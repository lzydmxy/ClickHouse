...
if (GFLAGS_NAMESPACE_SECONDARY)
  ...
  foreach (ns IN LISTS GFLAGS_NAMESPACE_SECONDARY)
    ...
    set (gflags_ns_h "${_gFlags_BINARY_DIR}/include/${GFLAGS_INCLUDE_DIR}/gflags_${ns}.h")
  ...
  if (BUILD_${TYPE}_LIBS)
    ...
    foreach (opts IN ITEMS "" _nothreads)
      if (BUILD_gflags${opts}_LIB)
        ...
        set (include_dirs "$<BUILD_INTERFACE:${_gFlags_BINARY_DIR}/include>")
        if (INSTALL_HEADERS)
          list (APPEND include_dirs "$<INSTALL_INTERFACE:${INCLUDE_INSTALL_DIR}>")
        endif ()
        target_include_directories (${target_name}
          PUBLIC  "${include_dirs}"
          PRIVATE "${_gFlags_BINARY_DIR}/src;${_gFlags_BINARY_DIR}/include/${GFLAGS_INCLUDE_DIR}"
        )
...
configure_file (${_gFlags_SOURCE_DIR}/cmake/config.cmake.in  "${_gFlags_BINARY_DIR}/${PACKAGE_NAME}-config-install.cmake" @ONLY)
configure_file (${_gFlags_SOURCE_DIR}/cmake/version.cmake.in "${_gFlags_BINARY_DIR}/${PACKAGE_NAME}-config-version.cmake" @ONLY)
...
if (INSTALL_HEADERS)
  ...
  install (
    FILES "${_gFlags_BINARY_DIR}/${PACKAGE_NAME}-config-install.cmake"
    RENAME ${PACKAGE_NAME}-config.cmake
    DESTINATION ${CONFIG_INSTALL_DIR}
  )
  install (
    FILES "${_gFlags_BINARY_DIR}/${PACKAGE_NAME}-config-version.cmake"
    DESTINATION ${CONFIG_INSTALL_DIR}
  )
  ...
  if (PKGCONFIG_INSTALL_DIR)
    configure_file ("cmake/package.pc.in" "${_gFlags_BINARY_DIR}/${PACKAGE_NAME}.pc" @ONLY)
    install (FILES "${_gFlags_BINARY_DIR}/${PACKAGE_NAME}.pc" DESTINATION "${PKGCONFIG_INSTALL_DIR}")
  endif ()
endif ()
...
if (EXPORT_NAMESPACE_SET)
  export (
    TARGETS ${TARGETS}
    NAMESPACE ${PACKAGE_NAME}::
    FILE "${_gFlags_BINARY_DIR}/${EXPORT_NAME}.cmake"
  )
endif ()
if (EXPORT_NONAMESPACE_SET)
  export (
    TARGETS ${TARGETS}
    FILE "${_gFlags_BINARY_DIR}/${PACKAGE_NAME}-nonamespace-targets.cmake"
  )
endif ()
...
configure_file (${_gFlags_SOURCE_DIR}/cmake/config.cmake.in "${_gFlags_BINARY_DIR}/${PACKAGE_NAME}-config.cmake" @ONLY)
...
if (BUILD_PACKAGING)
  ...
  if (NOT CPACK_PROJECT_CONFIG_FILE)
    configure_file (
      ...
      "${_gFlags_BINARY_DIR}/${PACKAGE_NAME}-package.cmake" @ONLY
    )
    set (CPACK_PROJECT_CONFIG_FILE "${_gFlags_BINARY_DIR}/${PACKAGE_NAME}-package.cmake")
  endif ()
