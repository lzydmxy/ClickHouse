...
public:
    ...
    ExecutableFunctionPtr prepare(const ColumnsWithTypeAndName & sample_arguments) const override
    {
        ...
        {
            throw Exception(ErrorCodes::LOGICAL_ERROR, "unexpected sample block size");
        }
    ...
    bool isSuitableForShortCircuitArgumentsExecution(const DataTypesWithConstInfo & /*arguments*/) const override { return true; } // TODO wujianchao true?
...
public:
    ...
    FunctionBasePtr buildImpl(const DB::ColumnsWithTypeAndName & arguments, const DB::DataTypePtr & result_type) const override
    {
        if (arguments.size() != 2)
        {
            throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "unmatched size of arguments: must be 2");
        }
        if (arguments[0].type->getTypeId() != TypeIndex::String)
        {
            throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "the first arguments must be base64 format of bucket bounds");
        }
...
#undef HANDLE_CASE
            default:
                throw Exception(ErrorCodes::ILLEGAL_TYPE_OF_ARGUMENT, "unimplemented data type");
        }
    }
