...
#define APPLY_FOR_BUILTIN_ERROR_CODES(M) \
    ...
    \
    /** JD query optimizer*/ \
    M(2000, QUERY_OPTIMIZER_EXCEPTION) \
    M(2001, BRPC_PROTOCOL_VERSION_UNSUPPORT) \
    M(2002, BRPC_CONNECT_ERROR) \
    M(2003, BRPC_CANNOT_INIT_CHANNEL) \
    M(2004, BRPC_NO_METHOD) \
    M(2005, BRPC_HOST_DOWN) \
    M(2006, BRPC_TIMEOUT) \
    M(2007, BRPC_EXCEPTION) \
    M(2008, EXCHANGE_DATA_TRANS_EXCEPTION) \
    M(2009, BSP_EXCHANGE_DATA_DISK_LIMIT_EXCEEDED) \
    M(2010, BSP_CLEANUP_PREVIOUS_SEGMENT_INSTANCE_FAILED) \
    M(2011, BSP_WRITE_DATA_FAILED) \
    M(2012, DISTRIBUTE_STAGE_QUERY_EXCEPTION) \
    M(2013, QUERY_WAS_CANCELLED_INTERNAL) \
    M(2014, TOO_MANY_PLAN_SEGMENTS) \
    M(2015, PROJECTION_SELECTION_ERROR) \
    M(2016, QUERY_CPU_TIMEOUT_EXCEEDED) \
    M(2017, INVALID_IMPLICIT_COLUMN_NAME) \
    M(2018, INVALID_IMPLICIT_COLUMN_FILE_NAME) \
    M(2019, INVALID_MAP_SEPARATOR) \
    M(2020, PLAN_BUILD_ERROR) \
    M(2021, REMOVE_SUBQUERY_ERROR) \
    M(2022, ILLEGAL_ENFORCE) \
    M(2023, OPTIMIZER_TIMEOUT) \
    M(2024, OPTIMIZER_TEST_FAILED) \
    M(2025, NO_AVAILABLE_MATERIALIZED_VIEW) \
    M(2026, TOO_MANY_JOINS) \
