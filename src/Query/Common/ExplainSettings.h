...
struct QueryPipelineSettings
{
    ...
    std::unordered_map<std::string, std::reference_wrapper<bool>> boolean_settings
        = {{"header", query_pipeline_options.header}, {"graph", graph}, {"compact", compact}, {"stats", stats}};
...
struct ExplainSettings : public Settings
{
    ...
    bool has(const std::string & name_) const { return boolean_settings.count(name_) > 0 || uint_settings.count(name_) > 0; }
    ...
    for (const auto & change : set_query.changes)
    {
        ...
        if (change.value.getType() != Field::Types::UInt64)
            throw Exception(
                ...
                "Invalid type {} for setting \"{}\" only boolean and UInt64 settings are supported",
                std::string(change.value.getTypeName()),
                change.name);
        ...
        if (!has_setting)
            throw Exception(ErrorCodes::LOGICAL_ERROR, "Unknown setting for ExplainSettings: {}", change.name);
    }
