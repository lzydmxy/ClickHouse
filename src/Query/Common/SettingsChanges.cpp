...
bool SettingsChanges::insertSetting(std::string_view name, const Field & value)
{
    ...
    if (it != end())
        return false;
    emplace_back(DB::SettingChange(name, value));
    return true;
}
...
void SettingsChanges::toProto(DB::Protos::SettingsChanges & proto) const
{
    for (const auto & element : *this)
        element.toProto(*proto.add_settings_changes());
}

void SettingsChanges::fillFromProto(const DB::Protos::SettingsChanges & proto)
{
