    ...
    const bool is_user_defined_expression)
{
    if (split_number <= 0 && is_with_range)
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Unexpected operation. SPLIT_NUMBER is required for WITH_RANGE ");

    if (is_user_defined_expression && (split_number > 0 || is_with_range))
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Unexpected operation. SPLIT_NUMBER and WITH_RANGE not allowed for user defined CLUSTER BY expression");
    ...
    for (size_t i = 0; i < num_rows; i++)
    {
        ...
        if (unlikely(bucket_number_from_expression >= total_shard_num))
        {
            throw Exception(ErrorCodes::LOGICAL_ERROR
                , "User defined expression for cluster by resulted in a row's BUCKET NUMBER[{}] >= TOTAL BUCKET NUMBER[{}]"
                , bucket_number_from_expression, total_shard_num);
        }
