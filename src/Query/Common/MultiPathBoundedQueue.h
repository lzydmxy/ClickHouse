...
public:
    ...
    void decrease(const MultiPathDataPacket & packet) 
    {
        amount.fetch_sub(calculate(packet), std::memory_order_relaxed); 
    }

    void logPeakMemoryUsage() const 
    { 
        LOG_TRACE(getLogger("MemoryController"), "Peak memory usage: {}", ReadableSize(peak)); 
    }
private:
