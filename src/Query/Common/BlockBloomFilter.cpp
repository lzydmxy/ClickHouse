...
#else
    throw Exception(ErrorCodes::LOGICAL_ERROR, "AVX2 is not supported!");
#endif
}
...
void BlockBloomFilter::deserialize(ReadBuffer & istr)
{
    readBinary(ndv, istr);
    init(ndv, true);
    auto size = istr.read(reinterpret_cast<char *>(data.get()), bytes_in_slot * slots);
    if (size < bytes_in_slot * slots)
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Read data error.");
}
...
void BlockBloomFilter::mergeInplace( BlockBloomFilter && bf)
{
    if (this->slots != bf.slots)
    {
        if (!isPowerOf2(this->slots) || !isPowerOf2(bf.slots))
            throw Exception(ErrorCodes::LOGICAL_ERROR, "Cannot merge bloom filters with none power of two bit size");
