...
PartitionTopNTransformExt::PartitionTopNTransformExt(
    Block header_, size_t topN_, ColumnNumbers partition_by_column_, ColumnNumbers order_by_columns_, TopNModel model_, bool reverse_)
    : IProcessor(InputPorts{header_}, OutputPorts{header_})
    , model(model_)
    , topN(topN_)
    , partition_by_columns(std::move(partition_by_column_))
    , order_by_columns(std::move(order_by_columns_))
    , reverse(reverse_)
    , hash(0)
{
    if (order_by_columns.size() != 1)
        throw Exception(ErrorCodes::LOGICAL_ERROR, "currently order by columns size only support one");
    if (model > TopNModel::DENSE_RANK)
        throw Exception(ErrorCodes::LOGICAL_ERROR, "invalid data");
}

IProcessor::Status PartitionTopNTransformExt::prepare()
{
...
void PartitionTopNTransformExt::work()
{
    if (start_output_chunk)
    {
            ...
            if (partition_to_heap.find(hash_value) == partition_to_heap.end())
            {
                if (!reverse)
                    ...
                    partition_to_heap_reverse.emplace(
                        std::make_pair(hash_value, std::priority_queue<Field, std::vector<Field>, std::greater<>>()));
    ...
    if (receive_all_data)
    {
        ...
        for (auto & iter : partition_to_map)
        {
            for (auto & field_iter : iter.second)
            {
        ...
        for (auto & iter : filter_map)
        {
            ...
            for (auto & current_column : source_chunks_columns)
