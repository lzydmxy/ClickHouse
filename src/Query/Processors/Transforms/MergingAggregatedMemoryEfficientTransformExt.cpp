...
void GroupingAggregatedTransformExt::readFromAllInputs()
{
...
void GroupingAggregatedTransformExt::pushData(Chunks chunks, Int32 bucket, bool is_overflows)
{
...
bool GroupingAggregatedTransformExt::tryPushTwoLevelData()
{
...
bool GroupingAggregatedTransformExt::tryPushSingleLevelData()
{
...
bool GroupingAggregatedTransformExt::tryPushOverflowData()
{
...
IProcessor::Status GroupingAggregatedTransformExt::prepare()
{
    /// Check can output.
    auto & output = outputs.front();
    ...
    else
    {
        if (!all_inputs_finished) // -V547
            throw Exception(
                ...
                "GroupingAggregatedTransform should have read all chunks for single level aggregation, but not all of the inputs are "
                "finished.");
...
void GroupingAggregatedTransformExt::addChunk(Chunk chunk, size_t input)
{
    ...
    if (!agg_info)
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Chunk should have AggregatedChunkInfo in GroupingAggregatedTransform.");
...
void GroupingAggregatedTransformExt::work()
{
    ...
    if (!single_level_chunks.empty())
    {
        ...
        auto blocks = params->aggregator_ext.convertBlockToTwoLevel(block);
...
MergingAggregatedBucketTransformExt::MergingAggregatedBucketTransformExt(AggregatingTransformParamsExtPtr params_)
...
void MergingAggregatedBucketTransformExt::transform(Chunk & chunk)
{
    ...
    auto header = params->aggregator_ext.getHeader(false);
    ...
    for (auto & cur_chunk : *chunks_to_merge->chunks)
    {
        ...
        if (!agg_info)
            throw Exception(ErrorCodes::LOGICAL_ERROR, "Chunk should have AggregatedChunkInfo in MergingAggregatedBucketTransform.");
    ...
    auto block = params->aggregator_ext.mergeBlocks(blocks_list, params->final, is_cancelled);
    size_t num_rows = block.rows();
    chunk.setColumns(block.getColumns(), num_rows);
}


SortingAggregatedTransformExt::SortingAggregatedTransformExt(size_t num_inputs_, AggregatingTransformParamsExtPtr params_)
...
bool SortingAggregatedTransformExt::tryPushChunk()
{
    auto & output = outputs.front();
...
void SortingAggregatedTransformExt::addChunk(Chunk chunk, size_t from_input)
{
    const auto & info = chunk.getChunkInfo();
    if (!info)
    ...
    const auto * agg_info = typeid_cast<const AggregatedChunkInfo *>(info.get());
    if (!agg_info)
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Chunk should have AggregatedChunkInfo in SortingAggregatedTransform.");
    ...
    {
        if (chunks[bucket])
            throw Exception(ErrorCodes::LOGICAL_ERROR, "SortingAggregatedTransform already got bucket with number {}", toString(bucket));
...
IProcessor::Status SortingAggregatedTransformExt::prepare()
{
    /// Check can output.
    auto & output = outputs.front();
...
void addMergingAggregatedMemoryEfficientTransform(Pipe & pipe, AggregatingTransformParamsExtPtr params, size_t num_merging_processors)
{
    pipe.addTransform(std::make_shared<GroupingAggregatedTransformExt>(pipe.getHeader(), pipe.numOutputPorts(), params));

    if (num_merging_processors <= 1)
    {
        /// --> GroupingAggregated --> MergingAggregatedBucket -->
        pipe.addTransform(std::make_shared<MergingAggregatedBucketTransformExt>(params));
        return;
    }
    ...
    pipe.addSimpleTransform([params](const Block &) { return std::make_shared<MergingAggregatedBucketTransformExt>(params); });

    pipe.addTransform(std::make_shared<SortingAggregatedTransformExt>(num_merging_processors, params));
}
