...
#include <Query/Common/ProfileElementConsumer.h>
...
ExplainAnalyzeTransformExt::ExplainAnalyzeTransformExt(
...
void ExplainAnalyzeTransformExt::transform(Chunk & chunk)
{
    ...
    auto scheduler = context->getOptimizerContext()->getSegmentScheduler();
    UInt64 time_out = context->getOptimizerContext()->getSettingsRef().operator_profile_receive_timeout;
    ...
    String explain;
    if ((kind == ASTExplainQueryExt::ExplainKindExt::LogicalAnalyze || kind == ASTExplainQueryExt::ExplainKindExt::DistributedAnalyze))
    {
        ...
        if (kind == ASTExplainQueryExt::ExplainKindExt::LogicalAnalyze)
        {
            ...
            //     explain = PlanPrinter::jsonLogicalPlan(
            //         *query_plan_ptr, plan_cost, CostModel(*context), step_agg_operator_profiles, costs, settings);
            // }
            // else
...
ISimpleTransform::Status ExplainAnalyzeTransformExt::prepare()
{
    /// Check can output.
...
void ExplainAnalyzeTransformExt::getRemoteProcessorProfiles(
    std::unordered_map<size_t, std::unordered_map<String, ProcessorProfiles>> & segment_profiles)
{
    ...
    auto consumer = context->getOptimizerContext()->getProcessorProfileElementConsumer();
...
void ExplainAnalyzeTransformExt::getProcessorProfiles(
    ProcessorsSet & processors_set, ProcessorProfiles & profiles, const IProcessor * processor)
{
    ...
    for (const auto & input : inputs)
    {
        ...
        if (processors_set.find(from) == processors_set.end())
        {
            ...
            // child->step_id = from->getStepId(); /// useless, only for log
            child->segment_id = 0;
