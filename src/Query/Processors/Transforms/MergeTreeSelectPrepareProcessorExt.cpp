...
MergeTreeSelectPrepareProcessorExt::MergeTreeSelectPrepareProcessorExt(
...
IProcessor::Status MergeTreeSelectPrepareProcessorExt::prepare()
{
    if (inputs.empty())
    {
        if (start_expand)
...
void MergeTreeSelectPrepareProcessorExt::work()
{
    if (!poll_done)
    {
        if (timing.elapsed() > rf_wait_time_ns)
        {
            ...
            for (const auto & rf : runtime_filters)
            {
                ...
                {
                     LOG_DEBUG(
                        getLogger("MergeTreeSelectPrepareProcessorExt"),
                        "wait time out:{} rf:{}",
                        timing.elapsed(),
                        rf);
                }
        ...
        else
        {
            ...
            if (all_ready)
            {
                //                LOG_DEBUG(getLogger("MergeTreeSelectPrepareProcessorExt"), "cost time:{} thread:{}", timing.elapsed(),
...
Processors MergeTreeSelectPrepareProcessorExt::expandPipeline()
{
