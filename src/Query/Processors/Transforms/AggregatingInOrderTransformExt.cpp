...
AggregatingInOrderTransformExt::~AggregatingInOrderTransformExt() = default;

void AggregatingInOrderTransformExt::consume(Chunk chunk)
{
    ...
    AggregatorExt::NestedColumnsHolder nested_columns_holder;
    AggregatorExt::AggregateFunctionInstructions aggregate_function_instructions;
    params->aggregator_ext.prepareAggregateInstructions(chunk.getColumns(), aggregate_columns, materialized_columns, aggregate_function_instructions, nested_columns_holder);
    ...
    if (!cur_block_size)
    {
        ...
        params->aggregator_ext.createStatesAndFillKeyColumnsWithSingleKey(variants, key_columns, key_begin, res_key_columns);
        params->aggregator_ext.addArenasToAggregateColumns(variants, res_aggregate_columns);
        ++cur_block_size;
    }
    ...
    while (key_end != rows)
    {
        ...
        if (key_begin != key_end)
            params->aggregator_ext.executeOnIntervalWithoutKeyImpl(variants.without_key, key_begin, key_end, aggregate_function_instructions.data(), variants.aggregates_pool);

        /// We finalize last key aggregation state if a new key found.
        if (key_end != rows)
        {
            params->aggregator_ext.addSingleKeyToAggregateColumns(variants, res_aggregate_columns);
            ...
            if (cur_block_size == res_block_size)
            {
                ...
                if (params->final)
                {
                    if (variants.aggregates_pools.size() != 1)
                        throw Exception(ErrorCodes::LOGICAL_ERROR, "Too much arenas");
            ...
            params->aggregator_ext.createStatesAndFillKeyColumnsWithSingleKey(variants, key_columns, key_end, res_key_columns);
            ++cur_block_size;
        }
...
void AggregatingInOrderTransformExt::work()
{
...
IProcessor::Status AggregatingInOrderTransformExt::prepare()
{
...
void AggregatingInOrderTransformExt::generate()
{
