...
namespace DB
{
FinalSampleTransformExt::FinalSampleTransformExt(const Block & header_, size_t sample_size_, size_t max_chunk_size_, size_t num_streams_)
...
IProcessor::Status FinalSampleTransformExt::prepare()
{
...
void FinalSampleTransformExt::work()
{
...
size_t FinalSampleTransformExt::SampledChunk::sample(const size_t sample_size_, const size_t total_rows)
{
    ...
    {
        size_t base = 10;
        size_t window = size_t(step * base);
    ...
    {
        size_t window = size_t(step * 10 / 10);
        for (size_t idx = 0; idx < rows; idx += window)
            filter_vec[idx] = 1;
    }
...
void FinalSampleTransformExt::SampledChunk::merge(const SampledChunk & rhs_sampled_chunk)
{
...
bool FinalSampleTransformExt::SampledChunks::add(Chunk && chunk)
{
    if (!chunk)
        return false;
...
FinalSampleTransformExt::SampledChunk FinalSampleTransformExt::SampledChunks::getResult()
{
    if (chunks.empty())
    {
        Chunk empty;
        FinalSampleTransformExt::SampledChunk empty_sample(std::move(empty));
        return empty_sample;
    }
