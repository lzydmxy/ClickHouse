...
TopNFilteringTransformExt::TopNFilteringTransformExt(
    ...
    : ISimpleTransform(header_, header_, true), sort_description(std::move(sort_description_)), size(size_), model(model_)
{
    if (sort_description.empty())
        throw Exception(ErrorCodes::LOGICAL_ERROR, "TopN sort description is empty");

    if (!size)
        throw Exception(ErrorCodes::LOGICAL_ERROR, "TopN size must be greater than 0");

    if (model != TopNModel::DENSE_RANK && model != TopNModel::ROW_NUMBER)
        throw Exception(ErrorCodes::NOT_IMPLEMENTED, "only support DENSE_RANK");
}

void TopNFilteringByLimitingTransformExt::transform(Chunk & chunk)
{
...
TopNFilteringByHeapTransformExt::TopNFilteringByHeapTransformExt(
    const Block & header_, SortDescription sort_description_, UInt64 size_, TopNModel model_)
    : TopNFilteringTransformExt(header_, std::move(sort_description_), size_, model_)
{
...
void TopNFilteringByHeapTransformExt::transform(Chunk & chunk)
{
