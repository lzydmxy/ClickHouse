...
QueryPipelineBuilderPtr QueryPlanExt::buildQueryPipeline(
    const QueryPlanOptimizationSettings & optimization_settings, const BuildQueryPipelineSettings & build_pipeline_settings, bool need_ck_optimizations)
{
    checkInitialized();
    // normally we need disable ClickHouse RBO rules,
    // but when the plan is generated by view (actually generated by ck planner),
    // we should enable ck rbo rules;
    if (need_ck_optimizations)
    ...
    const QueryPlanOptimizationSettings & optimization_settings, const BuildQueryPipelineSettings & build_pipeline_settings, bool need_ck_optimizations)
{
    ...
    return QueryPipelineBuilder::getPipe(std::move(*buildQueryPipeline(optimization_settings, build_pipeline_settings, need_ck_optimizations)), resources);
}
