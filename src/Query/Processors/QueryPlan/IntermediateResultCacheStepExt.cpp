...
IntermediateResultCacheStepExt::IntermediateResultCacheStepExt(
...
void IntermediateResultCacheStepExt::updateInputStream(DataStream input_stream_)
{
    input_streams.clear();
    input_streams.emplace_back(std::move(input_stream_));
}

std::shared_ptr<IQueryPlanStep> IntermediateResultCacheStepExt::copy(ContextPtr) const
{
    return std::make_shared<IntermediateResultCacheStepExt>(input_streams[0], /*cache_param,*/ aggregator_params);
}
