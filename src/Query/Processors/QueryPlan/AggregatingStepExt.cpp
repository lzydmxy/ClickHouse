...
bool hasNonParallelAggregateFunctions(const AggregateDescriptions & aggregates)
{
    for (const auto & agg : aggregates)
    {
        ...
        if (startsWith(agg_name, "pathCount") || startsWith(agg_name, "attributionCorrelation") || startsWith(agg_name, "funnelRep"))
            return true;
    }
    ...
    for (const auto & grouping : groupings)
    {
        ...
        if (!grouping_set_params.empty()) // GROUPING SETS, ROLLUP, CUBE
            outputs.push_back(&actions->addFunction(
                std::make_shared<FunctionToOverloadResolverAdaptor>(std::make_shared<FunctionGroupingForGroupingSets>(
                    std::move(arguments_indexes), grouping_sets_indices, force_grouping_standard_compatibility)),
                children,
...
AggregatingStepExt::createParams(Block header_before_aggregation, AggregateDescriptions aggregates, Names group_by_keys, bool overflow_row)
{
    ...
    for (auto & descr : aggregates)
    {
        ...
        {
            bool is_before_argument_uint64
                = descr.function->getArgumentTypes().size() == 1 && descr.function->getArgumentTypes()[0]->getTypeId() == TypeIndex::UInt64;
        ...
        descr.function = AggregateFunctionFactory::instance().get(
...
{
    bool is_prepared = std::any_of(
        grouping_sets_params.cbegin(),
        grouping_sets_params.cend(),
    ...
    GroupingSetsParamsExtList result;
    ...
    for (const auto & param : grouping_sets_params)
    {
        ...
        for (size_t i = 0; i < all_keys_index.size(); ++i)
            if (!used_indexes_set.contains(all_keys_index[i]))
...
AggregatingStepExt::AggregatingStepExt(
...
void AggregatingStepExt::transformPipeline(QueryPipelineBuilder & pipeline, const BuildQueryPipelineSettings & build_settings)
{
    ...
    if (group_by_info)
    {
        bool need_finish_sorting = (group_by_info->sort_description_for_merging.size() < group_by_sort_description.size());
...
void AggregatingStepExt::describeActions(FormatSettings & settings) const
{
    params.explain(settings.out, settings.offset);
}

void AggregatingStepExt::describeActions(JSONBuilder::JSONMap & map) const
{
    params.explain(map);
}

void AggregatingStepExt::describePipeline(FormatSettings & settings) const
{
