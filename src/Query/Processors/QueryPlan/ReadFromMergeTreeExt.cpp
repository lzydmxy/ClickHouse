...
void ReadFromMergeTreeExt::initializePipeline(QueryPipelineBuilder & pipeline, const BuildQueryPipelineSettings & build_context)
{
    ...
    if (build_context.getBuildQueryPipelineSettingsExt().context->getOptimizerContext()->getSettings()->report_segment_profiles)
    ...
    if (!select.final() && result.sampling.use_sampling &&
     !build_context.getBuildQueryPipelineSettingsExt().context->getOptimizerContext()->getSettings()->enable_sample_by_range &&
     !build_context.getBuildQueryPipelineSettingsExt().context->getOptimizerContext()->getSettings()->enable_deterministic_sample_by_range)
    {
    ...
    else if ((settings.optimize_read_in_order || settings.optimize_aggregation_in_order) && input_order_info)
    {
        ...
        can_read_in_partition_order = (build_context.getBuildQueryPipelineSettingsExt().context->getOptimizerContext()->getSettings()->optimize_read_in_partition_order ||
                                       build_context.getBuildQueryPipelineSettingsExt().context->getOptimizerContext()->getSettings()->force_read_in_partition_order);
    ...
    if (build_context.getBuildQueryPipelineSettingsExt().context->getOptimizerContext()->getSettings()->force_read_in_partition_order &&
    ...
    if (map_column_keys_column_queried)
    {
        if (build_context.getBuildQueryPipelineSettingsExt().context->getOptimizerContext()->getSettings()->early_limit_for_map_virtual_columns > 0)
        {
            pipe.addSimpleTransform([&](const Block & header) {
                return std::make_shared<LimitTransform>(header, build_context.getBuildQueryPipelineSettingsExt().context->getOptimizerContext()->getSettings()->early_limit_for_map_virtual_columns, 0);
            });
        }
