...
FinishAggregatingInOrderAlgorithmExt::State::State(
    const Chunk & chunk, const SortDescriptionWithPositions & desc)
...
FinishAggregatingInOrderAlgorithmExt::FinishAggregatingInOrderAlgorithmExt(
    const Block & header_,
    size_t num_inputs_,
    ...
    , max_block_size(max_block_size_)
{
    ...
    for (auto & column_description : description)
    {
        ...
        {
            column_description.column_number = header_.getPositionByName(column_description.base.column_name);
            column_description.base.column_name.clear();
        }
    }
}

void FinishAggregatingInOrderAlgorithmExt::initialize(Inputs inputs)
{
...
void FinishAggregatingInOrderAlgorithmExt::consume(Input & input, size_t source_num)
{
    states[source_num] = State{input.chunk, description};
}

IMergingAlgorithm::Status FinishAggregatingInOrderAlgorithmExt::merge()
{
...
Chunk FinishAggregatingInOrderAlgorithmExt::aggregate()
{
    auto aggregated = params->aggregator_ext.mergeBlocks(blocks, false, is_cancelled);
    blocks.clear();
    accumulated_rows = 0;
    return {aggregated.getColumns(), aggregated.rows()};
}

void FinishAggregatingInOrderAlgorithmExt::addToAggregation()
{
    for (size_t i = 0; i < num_inputs; ++i)
    {
