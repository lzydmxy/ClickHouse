...
ColumnSketchBinary::ColumnSketchBinary(const ColumnSketchBinary & src)
    : COWHelper<IColumnHelper<ColumnSketchBinary>, ColumnSketchBinary>(src),
...
void ColumnSketchBinary::updateWeakHash32(WeakHash32 & hash) const
{
    ...
    if (hash.getData().size() != s)
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Size of WeakHash32 does not match size of column: column size is {}, hash size is {}", s, hash.getData().size());
...
void ColumnSketchBinary::expand(const IColumn::Filter & mask, bool inverted)
{
...
ColumnPtr ColumnSketchBinary::permute(const Permutation & perm, size_t limit) const
{
    ...
    if (perm.size() < limit)
        throw Exception(ErrorCodes::SIZES_OF_COLUMNS_DOESNT_MATCH, "Size of permutation is less than required.");
...
void ColumnSketchBinary::insertManyFrom(const IColumn & src, size_t position, size_t length)
{
    const ColumnSketchBinary & src_concrete = assert_cast<const ColumnSketchBinary &>(src);
...
void ColumnSketchBinary::collectSerializedValueSizes(PaddedPODArray<UInt64> & sizes, const UInt8 * is_null) const
{
    if (empty())
        return;
...
char * ColumnSketchBinary::serializeValueIntoMemory(size_t n, char * memory) const
{
    size_t string_size = sizeAt(n);
    size_t offset = offsetAt(n);
...
void ColumnSketchBinary::shrinkToFit()
{
    chars.shrink_to_fit();
