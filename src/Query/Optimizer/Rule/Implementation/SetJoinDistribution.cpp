...
TransformResult SetJoinDistribution::transformImpl(PlanNodePtr node, const Captures &, RuleContext & context)
{
    ...
    // auto left_stats = context.context->getOptimizerContext()->getMemo().getGroupById(left_group_id)->getStatistics();
    // auto right_group_id = dynamic_cast<const AnyStepExt *>(node->getChildren()[1]->getStep().get())->getGroupId();
    // auto right_stats = context.context->getOptimizerContext()->getMemo().getGroupById(right_group_id)->getStatistics();
