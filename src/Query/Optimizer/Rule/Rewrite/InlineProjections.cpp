...
TransformResult InlineProjections::transformImpl(PlanNodePtr node, const Captures &, RuleContext & rule_context)
{
    ...
    const auto & step = dynamic_cast<const ProjectionStepExt &>(*node->getStep());
    const auto & child_step = dynamic_cast<const ProjectionStepExt &>(*child->getStep());
...
{
    auto *parent = dynamic_cast<ProjectionStepExtNode *>(parent_node.get());
    auto *child = dynamic_cast<ProjectionStepExtNode *>(child_node.get());
    if (!parent || !child)
        return {};
    ...
    {
        auto new_child_step = std::make_shared<ProjectionStepExt>(
            child->getChildren()[0]->getStep()->getOutputStream(), new_child_assignments, new_child_types, false, child_step.isIndexProject());
        new_child_node = std::make_shared<ProjectionStepExtNode>(child->getId(), std::move(new_child_step), PlanNodes{child->getChildren()[0]});
    }

    auto new_parent_step = std::make_shared<ProjectionStepExt>(
        new_child_node->getStep()->getOutputStream(),
        new_parent_assignments,
        new_parent_types,
        parent_step.isFinalProject(),
        parent_step.isIndexProject());
    return std::make_shared<ProjectionStepExtNode>(parent->getId(), std::move(new_parent_step), PlanNodes{new_child_node});
}
 ...
 */
std::set<String> InlineProjections::extractInliningTargets(ProjectionStepExtNode * parent, ProjectionStepExtNode * child, ContextMutablePtr & context, bool inline_arraysetcheck)
{
...
TransformResult InlineProjectionIntoJoin::transformImpl(PlanNodePtr node, const Captures &, RuleContext & context)
{
    ...
    bool rewrite = false;
    if (getQueryPlanStepType(left->getStep()) == QueryPlanStepType::ProjectionStepExt)
    {
        const auto * step = dynamic_cast<const ProjectionStepExt *>(left->getStep().get());
    ...
    if (getQueryPlanStepType(right->getStep()) == QueryPlanStepType::ProjectionStepExt)
    {
        const auto * step = dynamic_cast<const ProjectionStepExt *>(right->getStep().get());
        if (Utils::isIdentity(*step))
        {
    ...
    PlanNodePtr new_join_node = std::make_shared<JoinStepExtNode>(context.context->getOptimizerContext()->nextNodeId(), std::move(new_join_step), PlanNodes{left, right});
...
ConstRefPatternPtr InlineProjectionOnJoinIntoJoin::getPattern() const
{
    static auto pattern = Patterns::project()
        .matchingStep<ProjectionStepExt>([](const auto & step) { return Utils::isIdentity(step); })
...
{
    const auto * projection_step = dynamic_cast<const ProjectionStepExt *>(node->getStep().get());
    const auto * join_step = dynamic_cast<const JoinStepExt *>(node->getChildren()[0]->getStep().get());
    ...
    return {PlanNodeBase::createPlanNode(context.context->getOptimizerContext()->nextNodeId(), new_join_step, node->getChildren()[0]->getChildren())};
}
