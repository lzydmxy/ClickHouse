...
public:
    ...
    {
        return context->getOptimizerContext()->getSettingsRef().enable_expand_distinct_optimization;
    }
...
private:
    ...
    static bool hasNoFilterOrMask(const AggregatingStepExt & step);
    static bool hasMultipleDistincts(const AggregatingStepExt & step);
    static bool hasMixedDistinctAndNonDistincts(const AggregatingStepExt & step);
    static bool hasNoUnSupportedFunc(const AggregatingStepExt & step);
     ...
     */
    static bool hasUniqueArgument(const AggregatingStepExt & step);
