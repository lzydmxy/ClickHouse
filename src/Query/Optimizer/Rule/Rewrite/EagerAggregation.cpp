...
//#include <brpc/src/butil/file_util.h>
...
ConstRefPatternPtr EagerAggregation::getPattern() const
{
    static auto pattern = Patterns::aggregating()
        .matchingStep<AggregatingStepExt>([](const AggregatingStepExt & s) { return s.isNormal() && !s.getKeys().empty(); })
        .result();
    return pattern;
}
...
static bool decomposeProjection(
    const ProjectionStepExt & projection_step,
    ...
    RuleContext & context)
{
    ...
    std::function<void(NameSet, PlanNodePtr, int, AggregateDescriptions, Names, int, std::unordered_map<String, String>)> find_bottom_join
        ...
        if (join->getChildren()[index]->getType() == QueryPlanStepType::ProjectionStepExt
            && join->getChildren()[index]->getChildren()[0]->getType() == QueryPlanStepType::JoinStepExt
            && proj_expr_to_origin_column.empty()) // try to push agg through projection
        {
            const auto & projection_step = dynamic_cast<const ProjectionStepExt &>(*join->getChildren()[index]->getStep());
            ...
            {
                const auto & second_join_step = dynamic_cast<const JoinStepExt &>(*next_join_node->getStep());
                ...
                auto second_names_and_types_from_left = next_join_node->getChildren()[0]->getCurrentDataStream().header.getNamesAndTypes();
                auto second_names_and_types_from_right = next_join_node->getChildren()[1]->getCurrentDataStream().header.getNamesAndTypes();
                auto second_names_from_left = ToNameSet(second_names_and_types_from_left);
                auto second_names_from_right = ToNameSet(second_names_and_types_from_right);
        ...
        if (join->getChildren()[index]->getType() != QueryPlanStepType::JoinStepExt || has_visit_first_join)
        {
            Names c1;
            ...
            {
                c1 = join->getChildren()[index]->getCurrentDataStream().header.getNames();
                const auto & proj_step = static_cast<const ProjectionStepExt &>(*projection->getStep());
        ...
        if (context.context->getOptimizerContext()->getSettingsRef().agg_push_down_every_join)
            has_visit_first_join = true;

        PlanNodePtr second_join = join->getChildren()[index];

        const auto & second_join_step = dynamic_cast<const JoinStepExt &>(*second_join->getStep());
        ...
        auto second_names_and_types_from_left = second_join->getChildren()[0]->getCurrentDataStream().header.getNamesAndTypes();
        auto second_names_and_types_from_right = second_join->getChildren()[1]->getCurrentDataStream().header.getNamesAndTypes();
        auto second_names_from_left = ToNameSet(second_names_and_types_from_left);
        auto second_names_from_right = ToNameSet(second_names_and_types_from_right);
        ...
        else
        {
            ...
            if (second_join->getChildren()[0]->getType()
                != QueryPlanStepType::AggregatingStepExt) // avoid push agg through join which child is already an aggregation node.
            {
            ...
            if (old_result_size == result.size())
            {
                if (second_join->getChildren()[1]->getType() != QueryPlanStepType::AggregatingStepExt)
                {
...
std::shared_ptr<AggregatingStepExt> createLocalAggregate(const DataStream & input_stream, const AggregateDescriptions & s0, const Names & g0, const ContextPtr &)
{
    LOG_DEBUG(getLogger("test"), "create local_agg={}, keys={}", formatS0(s0), fmt::join(g0, ","));
    return std::make_shared<AggregatingStepExt>(input_stream, g0, NameSet{}, s0, GroupingSetsParamsExtList{}, true);
}
    ...
    RuleContext & rule_context)
{
    ...
    std::function<PlanNodePtr(const PlanNodePtr &)> update_plan_node_until_bottom_join
        ...
        if (current_node->getType() == QueryPlanStepType::AggregatingStepExt)
        {
            if (has_visit_global_agg)
                return current_node;
            has_visit_global_agg = true;

            const auto & agg_step = dynamic_cast<const AggregatingStepExt &>(*aggregation->getStep());
            ...
            for (auto & agg_desc : new_global_agg_desc)
            {
                ...
                if (getClassOfAggFunc(agg_desc.function->getName()) == AggFuncClass::NEED_MERGE)
                {
                    ...
                    agg_desc.function = AggregateFunctionFactory::instance().get(getMergeName(agg_desc.function->getName()), NullsAction::EMPTY, arguments_types, agg_desc.parameters, properties);
                }
            }

            LOG_DEBUG(getLogger("test"), "create global_agg={}, keys={}", formatS0(new_global_agg_desc), fmt::join(agg_step.getKeys(), ","));

            auto new_global_agg_step = std::make_shared<AggregatingStepExt>(
            ...
            return AggregatingStepExtNode::createPlanNode(aggregation->getId(), std::move(new_global_agg_step), {child_node});
        }
        else if (current_node->getType() == QueryPlanStepType::ProjectionStepExt)
        {
            if (has_visit_join && !push_through_final_projection)
                return current_node;

            const auto & projection_step = dynamic_cast<const ProjectionStepExt &>(*current_node->getStep());

            if (current_node->getChildren()[0]->getType() != QueryPlanStepType::JoinStepExt)
            ...
            {
                auto child_name_to_type = BlockHelper::getNamesToTypes(child_node->getCurrentDataStream().header);
                ...
                return ProjectionStepExtNode::createPlanNode(rule_context.context->getOptimizerContext()->nextNodeId(), std::move(new_projection_step), {child_node});
            }
        ...
        else if (current_node->getType() == QueryPlanStepType::JoinStepExt)
        {
            has_visit_join = true;
            const auto & join = current_node;
            const auto & join_step = dynamic_cast<const JoinStepExt &>(*join->getStep());
            ...
            if (join->getId() == bottom_join_id)
            {
                ...
                if (push_projection)
                {
                    const auto & proj_step = static_cast<const ProjectionStepExt &>(*proj->getStep());
                    Assignments new_assignments;
                    NameToType new_name_to_type;
                    auto child_name_to_type = BlockHelper::getNamesToTypes(join->getChildren()[bottom_join_child_index]->getCurrentDataStream().header);
                    ...
                    auto new_proj_step = std::make_shared<ProjectionStepExt>(
                        ...
                        //proj_step.getHints());
                    //
                    node_below_local_agg = ProjectionStepExtNode::createPlanNode(
                        rule_context.context->getOptimizerContext()->nextNodeId(), std::move(new_proj_step), {join->getChildren()[bottom_join_child_index]});
                }
                else
                {
                ...
                for (auto & agg_desc : new_s1)
                {
                    ...
                    if (getClassOfAggFunc(agg_desc.function->getName()) == AggFuncClass::NEED_MERGE)
                    {
                        ...
                        for (const auto & argument_name : agg_desc.argument_names)
                            arguments_types.emplace_back(name_to_type.at(argument_name));
                        agg_desc.function = AggregateFunctionFactory::instance().get(getStateName(agg_desc.function->getName()), NullsAction::EMPTY, arguments_types, agg_desc.parameters, properties);
                    }
                }

                std::shared_ptr<AggregatingStepExt> local_agg_step
                ...
                {
                    left_child_node = AggregatingStepExtNode::createPlanNode(
                        rule_context.context->getOptimizerContext()->nextNodeId(), std::move(local_agg_step), {node_below_local_agg});
                }
                else
                {
                    right_child_node = AggregatingStepExtNode::createPlanNode(
                        rule_context.context->getOptimizerContext()->nextNodeId(), std::move(local_agg_step), {node_below_local_agg});
                }
            }
            else
            ...
            return JoinStepExtNode::createPlanNode(join->getId(), symbol_mapper.map(*new_join_step), {left_child_node, right_child_node});
        }
        else
        {
            return current_node;
...
bool canAggPushDown(const LocalGroupByTarget & target, RuleContext & context)
{
    ...
    auto settings = context.context->getOptimizerContext()->getSettingsRef();
...
TransformResult EagerAggregation::transformImpl(PlanNodePtr aggregation, const Captures &, RuleContext & rule_context)
{
    ...
    PlanNodePtr parent_of_first_join;
    {
        ...
        if (node->getChildren()[0]->getType() == QueryPlanStepType::ProjectionStepExt)
        {
            projection = node->getChildren()[0];
            node = projection;
        }
        if (node->getChildren()[0]->getType() != QueryPlanStepType::JoinStepExt)
            return {};
    ...
    auto names_and_types_from_left = join->getChildren()[0]->getCurrentDataStream().header.getNamesAndTypes();
    auto names_and_types_from_right = join->getChildren()[1]->getCurrentDataStream().header.getNamesAndTypes();
    auto names_from_left = ToNameSet(names_and_types_from_left);
    auto names_from_right = ToNameSet(names_and_types_from_right);
    ...
    const SymbolAllocatorPtr & symbol_allocator = rule_context.context->getOptimizerContext()->getSymbolAllocator();
    if (!decomposeAggJoin(
    ...
    if (projection)
    {
        const auto & projection_step = dynamic_cast<const ProjectionStepExt &>(*projection->getStep());
