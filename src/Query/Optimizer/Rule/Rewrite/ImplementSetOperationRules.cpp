...
TransformResult ImplementExceptRule::transformImpl(PlanNodePtr node, const Captures &, RuleContext & rule_context)
{
    ...
    if (!context.getOptimizerContext()->getSettingsRef().enable_setoperation_to_agg)
    {
        ...
        for (auto & child : node->getChildren())
        {
            ...
            auto projection_step = std::make_shared<ProjectionStepExt>(child->getStep()->getOutputStream(), assignments, name_to_type);
            auto projection_node = PlanNodeBase::createPlanNode(context.getOptimizerContext()->nextNodeId(), std::move(projection_step), PlanNodes{child});
        ...
        if(is_distinct)
        {
            ...
            auto distinct_step = std::make_unique<DistinctStepExt>(
                ...
                true,
                true);
            return PlanNodeBase::createPlanNode(context.getOptimizerContext()->nextNodeId(), std::move(distinct_step), PlanNodes{node_new});
        }

        auto step_new = std::make_shared<IntersectOrExceptStep>(input_streams, ASTSelectIntersectExceptQuery::Operator::EXCEPT_ALL);
        return PlanNodeBase::createPlanNode(context.getOptimizerContext()->nextNodeId(), std::move(step_new), new_children);
    }
    ...
    */
    ...
    auto filter_step = std::make_shared<FilterStepExt>(translator_result.plan_node->getStep()->getOutputStream(), remove_extra_rows);
    auto filter_node = PlanNodeBase::createPlanNode(context.getOptimizerContext()->getPlanNodeIdAllocator()->nextId(), std::move(filter_step), PlanNodes{translator_result.plan_node});
    ...
    auto project_step = std::make_shared<ProjectionStepExt>(filter_node->getStep()->getOutputStream(), assignments, name_to_type);
    auto project_node
        = PlanNodeBase::createPlanNode(context.getOptimizerContext()->getPlanNodeIdAllocator()->nextId(), std::move(project_step), PlanNodes{filter_node});
    return project_node;
}
