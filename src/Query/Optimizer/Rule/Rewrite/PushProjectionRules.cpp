...
{
    auto * projection = dynamic_cast<const ProjectionStepExt *>(node->getStep().get());
    size_t func_count = 0;
    
    auto tname_to_type = BlockHelper::getNamesToTypes(node->getChildren()[0]->getCurrentDataStream().header);
    ...
    auto output_names = BlockHelper::getNameSet(projection->getOutputStream().header);
    ...
    if (has_all)
    {
        ...
        auto filter_step = QueryPlanStepHelper::copyQueryPlanStep(node->getChildren()[0]->getStep(), rule_context.context);
        node->replaceChildren(node->getChildren()[0]->getChildren());
        filter_step->updateInputStreams({node->getCurrentDataStream()});
        return PlanNodeBase::createPlanNode(rule_context.context->getOptimizerContext()->nextNodeId(), filter_step, {node});
    }
    else
    {
        // before: ProjectionA -> Filter -> TableScan
        ...
        auto bottom_projection_node = [&] {
            ...
            auto step = std::make_shared<ProjectionStepExt>(projection->getInputStreams()[0], new_ass, name_to_type, projection->isFinalProject(), projection->isIndexProject());
            return PlanNodeBase::createPlanNode(rule_context.context->getOptimizerContext()->nextNodeId(), step, node->getChildren()[0]->getChildren());
        }();

        auto filter_step = QueryPlanStepHelper::copyQueryPlanStep(node->getChildren()[0]->getStep(), rule_context.context);
        filter_step->updateInputStreams({bottom_projection_node->getCurrentDataStream()});
        auto filter_node = PlanNodeBase::createPlanNode(rule_context.context->getOptimizerContext()->nextNodeId(), filter_step, {bottom_projection_node});
        ...
        auto top_projection_node = [&] {
            ...
            auto step = std::make_shared<ProjectionStepExt>(filter_node->getCurrentDataStream(), new_ass, name_to_type);

            return PlanNodeBase::createPlanNode(rule_context.context->getOptimizerContext()->nextNodeId(), step, {filter_node});
        }();
...
{
    auto * projection = dynamic_cast<const ProjectionStepExt *>(node->getStep().get());
    auto * bottom_projection = dynamic_cast<const ProjectionStepExt *>(node->getChildren()[0]->getStep().get());
    auto tname_to_type = BlockHelper::getNamesToTypes(node->getChildren()[0]->getCurrentDataStream().header);
    size_t func_count = 0;
    for (const auto & item : projection->getAssignments())
    {
    ...
    auto bottom_projection_node = [&] {
        ...
        auto step = std::make_shared<ProjectionStepExt>(bottom_projection->getInputStreams()[0], mapper.map(new_ass), mapper.map(name_to_type), projection->isFinalProject(), projection->isIndexProject());
        return PlanNodeBase::createPlanNode(rule_context.context->getOptimizerContext()->nextNodeId(), step, node->getChildren()[0]->getChildren());
    }();
    ...
    auto middle_projection_node = [&] {
        ...
        auto step = std::make_shared<ProjectionStepExt>(bottom_projection_node->getCurrentDataStream(), new_ass, name_to_type, bottom_projection->isFinalProject(), bottom_projection->isIndexProject());
        return PlanNodeBase::createPlanNode(rule_context.context->getOptimizerContext()->nextNodeId(), step, {bottom_projection_node});
    }();
