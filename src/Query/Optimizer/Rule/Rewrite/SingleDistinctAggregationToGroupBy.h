...
namespace DB
{
    ...
    public:
        ...
        {
            return context->getOptimizerContext()->getSettingsRef().enable_single_distinct_to_group_by;
        }
    ...
    protected:
        ...
        static bool allDistinctAggregates(const AggregatingStepExt & step);

        // All Aggregate Functions must have same arguments
        static bool hasSingleDistinctInput(const AggregatingStepExt & step);

        // All Aggregate Functions must can't contains mask
        static bool noMasks(const AggregatingStepExt & step);

        // All Count Aggregate Functions must have at most one argument.
        static bool allCountHasAtMostOneArguments(const AggregatingStepExt & step);
