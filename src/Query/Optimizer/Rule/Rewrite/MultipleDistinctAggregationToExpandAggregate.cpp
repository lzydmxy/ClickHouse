...
bool MultipleDistinctAggregationToExpandAggregate::hasNoFilterOrMask(const AggregatingStepExt & step)
{
...
bool MultipleDistinctAggregationToExpandAggregate::hasMultipleDistincts(const AggregatingStepExt & step)
{
...
bool MultipleDistinctAggregationToExpandAggregate::hasMixedDistinctAndNonDistincts(const AggregatingStepExt & step)
{
...
bool MultipleDistinctAggregationToExpandAggregate::hasUniqueArgument(const AggregatingStepExt & step)
{
    const AggregateDescriptions & agg_descs = step.getAggregates();
...
bool MultipleDistinctAggregationToExpandAggregate::hasNoUnSupportedFunc(const AggregatingStepExt & step)
{
    const AggregateDescriptions & agg_descs = step.getAggregates();
    for (const auto & agg_desc : agg_descs)
    {
...
TransformResult MultipleDistinctAggregationToExpandAggregate::transformImpl(PlanNodePtr node, const Captures &, RuleContext & rule_context)
{
    ...
    String group_id_symbol = rule_context.context->getOptimizerContext()->getSymbolAllocator()->newSymbol(ExpandStepExt::group_id);
    ...
    for (const auto & agg_desc : agg_descs)
    {
        ...
        if (distinct_func.contains(Poco::toLower(agg_desc.function->getName())) && agg_desc.mask_column.empty())
        {
            ...
            group_id_mask
                = rule_context.context->getOptimizerContext()->getSymbolAllocator()->newSymbol(ExpandStepExt::group_id_mask + std::to_string(distinct_group_id));
        ...
        else
        {
            ...
            {
                group_id_mask = rule_context.context->getOptimizerContext()->getSymbolAllocator()->newSymbol(
                    ExpandStepExt::group_id_mask + std::to_string(non_distinct_group_id));
    ...
    if (rule_context.context->getOptimizerContext()->getSettings().expand_mode == ExpandMode::EXPAND)
    {
        auto expand_step = std::make_shared<ExpandStepExt>(
            child->getStep()->getOutputStream(), assignments, name_type, group_id_symbol, group_id_value, group_id_non_null_symbol);
        expand_node = PlanNodeBase::createPlanNode(rule_context.context->getOptimizerContext()->nextNodeId(), std::move(expand_step), {child});
    }
    if (rule_context.context->getOptimizerContext()->getSettings().expand_mode == ExpandMode::UNION)
    {
    ...
    if (rule_context.context->getOptimizerContext()->getSettings().expand_mode == ExpandMode::CTE)
    {
    ...
    auto pre_agg_step = std::make_shared<AggregatingStepExt>(
    ...
    auto mask_step = std::make_shared<ProjectionStepExt>(pre_agg_node->getStep()->getOutputStream(), mask_assignments, mask_null_name_to_type);
    child = PlanNodeBase::createPlanNode(rule_context.context->getOptimizerContext()->nextNodeId(), std::move(mask_step), PlanNodes{pre_agg_node});
    ...
    if (!new_argument_assignments.empty())
    {
        ...
        {
            new_argument_assignments.emplace(input_column.name, std::make_shared<ASTIdentifier>(input_column.name));
            name_to_type.emplace(input_column.name, input_column.type);
        }
        auto new_argument_projection_step
            = std::make_shared<ProjectionStepExt>(child->getStep()->getOutputStream(), new_argument_assignments, name_to_type);
        child = PlanNodeBase::createPlanNode(rule_context.context->getOptimizerContext()->nextNodeId(), std::move(new_argument_projection_step), {child});
    }

    // step 4 : final aggregate
    auto count_agg_step = std::make_shared<AggregatingStepExt>(
    ...
    const AggregateDescription & agg_desc, String & mask_column, Assignments & new_argument_assignments, ContextMutablePtr context)
{
    ...
    if (fun_remove_distinct == "countIf" && agg_desc.argument_names.size() > 1)
    {
        ...
        for (const auto & argument : agg_desc.argument_names)
            argument_functions.emplace_back(makeASTFunction("isNotNull", std::make_shared<ASTIdentifier>(argument)));
        auto new_argument = PredicateUtils::combineConjuncts(argument_functions);
        auto new_argument_name = context->getOptimizerContext()->getSymbolAllocator()->newSymbol(new_argument);
    ...
    AggregateFunctionPtr new_agg_fun = AggregateFunctionFactory::instance().get(fun_remove_distinct, NullsAction::EMPTY, data_types, parameters, properties);
...
{
    DataTypes data_types;
    data_types.emplace_back(agg_desc.function->getResultType());
    data_types.emplace_back(std::make_shared<DataTypeUInt8>());
    ...
    AggregateFunctionPtr new_agg_fun = AggregateFunctionFactory::instance().get(fun, NullsAction::EMPTY, data_types, parameters, properties);
    Names argument_names;
    ...
    PlanNodePtr child)
{
    ...
    for (const auto & id : group_id_value)
    {
        ...
        for (const auto & assignment : assignments)
        {
            ...
            String column_symbol = rule_context.context->getOptimizerContext()->getSymbolAllocator()->newSymbol(assignment.first);
        ...
        String group_id_pre_group = rule_context.context->getOptimizerContext()->getSymbolAllocator()->newSymbol("group_id");
        ...
        auto expand_step
            = std::make_shared<ProjectionStepExt>(child->getStep()->getOutputStream(), assignments_pre_group, name_to_type_pre_group);
        auto expand_node = PlanNodeBase::createPlanNode(rule_context.context->getOptimizerContext()->nextNodeId(), std::move(expand_step), {child});
        children.emplace_back(expand_node);
        input_streams.emplace_back(expand_node->getStep()->getOutputStream());
    }
    auto step = std::make_shared<UnionStepExt>(input_streams, output_stream, output_to_inputs);
    return PlanNodeBase::createPlanNode(rule_context.context->getOptimizerContext()->nextNodeId(), std::move(step), children);
}
    ...
    PlanNodePtr child)
{
    ...
    for (const auto & id : group_id_value)
    {
        ...
        String group_id_pre_group = rule_context.context->getOptimizerContext()->getSymbolAllocator()->newSymbol(ExpandStepExt::group_id);
        assignments_pre_group.emplace(group_id_pre_group, std::make_shared<ASTLiteral>(id));
        name_to_type_pre_group[group_id_pre_group] = std::make_shared<DataTypeInt32>();
        ...
        PlanNodePtr cte_ref = PlanNodeBase::createPlanNode(
            rule_context.context->getOptimizerContext()->nextNodeId(), std::make_shared<CTERefStepExt>(output_stream, cte_id, output_columns, false));

        auto expand_step
            = std::make_shared<ProjectionStepExt>(cte_ref->getStep()->getOutputStream(), assignments_pre_group, name_to_type_pre_group);
        auto expand_node = PlanNodeBase::createPlanNode(rule_context.context->getOptimizerContext()->nextNodeId(), std::move(expand_step), {cte_ref});
        children.emplace_back(expand_node);
        input_streams.emplace_back(expand_node->getStep()->getOutputStream());
    }
    auto step = std::make_shared<UnionStepExt>(input_streams, output_stream, output_to_inputs);
