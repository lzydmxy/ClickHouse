...
bool MultipleDistinctAggregationToMarkDistinct::hasNoDistinctWithFilterOrMask(const AggregatingStepExt & step)
{
...
bool MultipleDistinctAggregationToMarkDistinct::hasMultipleDistincts(const AggregatingStepExt & step)
{
...
bool MultipleDistinctAggregationToMarkDistinct::hasMixedDistinctAndNonDistincts(const AggregatingStepExt & step)
{
...
{
    static auto pattern = Patterns::aggregating().matchingStep<AggregatingStepExt>([](const AggregatingStepExt & s) {
...
TransformResult MultipleDistinctAggregationToMarkDistinct::transformImpl(PlanNodePtr node, const Captures &, RuleContext & rule_context)
{
    ...
    for (const auto & agg_desc : agg_descs)
    {
        if (distinct_func.contains(Poco::toLower(agg_desc.function->getName())) && agg_desc.mask_column.empty())
        {
            ...
            {
                marker = rule_context.context->getOptimizerContext()->getSymbolAllocator()->newSymbol(last_name + "$distinct");
                ...
                auto mark_distinct_step
                    = std::make_shared<MarkDistinctStepExt>(child->getStep()->getOutputStream(), marker, distinct_symbols_list);
                child = PlanNodeBase::createPlanNode(rule_context.context->getOptimizerContext()->nextNodeId(), std::move(mark_distinct_step), PlanNodes{child});
            }
            ...
            if (fun_remove_distinct == "countIf" && agg_desc.argument_names.size() > 1)
            {
                ...
                auto new_argument_name = rule_context.context->getOptimizerContext()->getSymbolAllocator()->newSymbol(new_argument);
            ...
            AggregateFunctionPtr new_agg_fun
                = AggregateFunctionFactory::instance().get(fun_remove_distinct, NullsAction::EMPTY, data_types, parameters, properties);
    ...
    if (!new_argument_assignments.empty())
    {
        ...
        {
            new_argument_assignments.emplace(input.name, std::make_shared<ASTIdentifier>(input.name));
