...
TransformResult SumIfToCountIf::transformImpl(PlanNodePtr node, const Captures &, RuleContext & rule_context)
{
    ...
    auto & agg_step = dynamic_cast<AggregatingStepExt &>(*node->getStep());
    auto & projection_step = dynamic_cast<ProjectionStepExt &>(*projection_node->getStep());
    ...
    for (auto & aggregate : aggregates)
    {
        ...
        if (lower_name == "sumif")
        {
            ...
            if (func_arguments.size() == 2 && literal->value.get<UInt64>() == 1)
            {
                ...
                aggregate.function = AggregateFunctionFactory::instance().get("countIf", NullsAction::EMPTY, DataTypes{name_to_type.at(func_arguments[1])}, aggregate.parameters, properties);
        ...
        else if (lower_name == "sum")
        {
            ...
            if (first_literal && second_literal)
            {
                ...
                if (new_cond)
                {
                    ...
                    String new_cond_name = rule_context.context->getOptimizerContext()->getSymbolAllocator()->newSymbol(new_cond);

                    AggregateFunctionProperties properties;
                    aggregate.function = AggregateFunctionFactory::instance().get("countIf", NullsAction::EMPTY, DataTypes{cond_type}, aggregate.parameters, properties);
