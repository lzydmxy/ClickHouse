...
bool createTopNFilteringForAggLike( PlanNodePtr cur_node, const NameSet & agg_keys, SortDescription sort_desc, UInt64 limit_value, ContextMutablePtr context)
{
    ...
    {
        if (std::find_if(sort_desc.begin(), sort_desc.end(), [&](const auto & sort_column_desc) { return sort_column_desc.column_name == agg_key; })== sort_desc.end())
