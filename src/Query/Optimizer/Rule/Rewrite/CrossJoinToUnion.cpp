...
{
    static auto pattern = Patterns::join().matchingStep<JoinStepExt>([&](const JoinStepExt & s) { return s.isCrossJoin() && matchPattern(s); }).result();
    return pattern;
}
...
TransformResult CrossJoinToUnion::transformImpl(PlanNodePtr node, const Captures &, RuleContext & rule_context)
{
    ...
    auto disjuncts = PredicateUtils::extractDisjuncts(match_filter);
    ...
    for (size_t i = 0; i < copy_join_nodes.size(); i++)
    {
        ...
        const auto & child_step = dynamic_cast<const JoinStepExt &>(*child_step_ptr);
        ...
        auto child_disjuncts = PredicateUtils::extractDisjuncts(child_match_filter);
        ...
        if (i == 0)
        {
            ...
            PlanNodePtr join_node = std::make_shared<JoinStepExtNode>(context->getOptimizerContext()->nextNodeId(), std::move(join_step), child->getChildren());

            std::vector<ConstASTPtr> predicates = {child_un_match_filter, child_disjuncts.at(i)};
            auto filter_step
                = std::make_shared<FilterStepExt>(join_node->getStep()->getOutputStream(), PredicateUtils::combineConjuncts(predicates));
            PlanNodePtr filter_node = std::make_shared<FilterStepExtNode>(context->getOptimizerContext()->nextNodeId(), std::move(filter_step), PlanNodes{join_node});
    ...
    auto union_step = std::make_shared<UnionStepExt>(input_streams, output_stream, output_to_inputs);
    auto union_node = PlanNodeBase::createPlanNode(context->getOptimizerContext()->nextNodeId(), std::move(union_step), children);
    return union_node;
}

bool CrossJoinToUnion::matchPattern(const JoinStepExt & step)
{
    const auto & filter = step.getFilter();
    ...
    auto matched_disjunct = [&](const auto & disjunct) -> bool {
        if (disjunct && disjunct->template as<const ASTFunction>())
        {
            ...
            if (fun.name == "equals" && fun.arguments->children.size() == 2
                && getAstType(fun.arguments->children[0]) == ASTType::ASTIdentifier
                && getAstType(fun.arguments->children[1]) == ASTType::ASTIdentifier)
            {
                return true;
            }
        }
