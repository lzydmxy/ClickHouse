...
#include <Query/Common/AggregateBitmapExpression_fwd.h>
...
ConstRefPatternPtr ExtractBitmapImplicitFilter::getPattern() const
{
    static auto pattern = Patterns::aggregating()
        .matchingStep<AggregatingStepExt>([](const AggregatingStepExt & step, Captures & captures) {
...
TransformResult ExtractBitmapImplicitFilter::transformImpl(PlanNodePtr node, const Captures & captures, RuleContext & rule_context)
{
    ...
    */
    ...
    auto filter_step = std::make_shared<FilterStepExt>(node->getChildren().front()->getCurrentDataStream(), implicit_filter);
    auto filter_node = PlanNodeBase::createPlanNode(rule_context.context->getOptimizerContext()->nextNodeId(), std::move(filter_step), node->getChildren());
