...
ConstRefPatternPtr PushUnionThroughJoin::getPattern() const
{
    static auto pattern
        = Patterns::unionn()
              .withAll(Patterns::join().matchingStep<JoinStepExt>([](const JoinStepExt & step) {
                  return (isInner(step.getKind()) || isLeft(step.getKind()))
                      && (step.getStrictness() == JoinStrictness::All || step.getStrictness() == JoinStrictness::Semi);
              }))
              .result();
    return pattern;
}
...
PlanNodePtr createNodeForSignature(PlanNodePtr node, Names join_keys, ContextMutablePtr context)
{
    ...
    return PlanNodeBase::createPlanNode(
        context->getOptimizerContext()->nextNodeId(), std::make_shared<ProjectionStepExt>(node->getCurrentDataStream(), assigments, name_to_type), {node});
}

TransformResult PushUnionThroughJoin::transformImpl(PlanNodePtr node, const Captures &, RuleContext & context)
{
    const auto & union_step = dynamic_cast<const UnionStepExt &>(*node->getStep());
    ...
    const auto & first_join_step = dynamic_cast<const JoinStepExt &>(*node->getChildren()[0]->getStep());
    auto kind = first_join_step.getKind();
    auto strictness = first_join_step.getStrictness();

    for (auto & union_child : node->getChildren())
    {
        const auto & join_step = dynamic_cast<const JoinStepExt &>(*union_child->getStep());
    ...
    for (size_t key_index = 0; key_index < left_keys_mapped[0].size(); key_index++)
    {
        if (left_keys_mapped[0][key_index].empty())
        {
            ...
            auto out_symbol = context.context->getOptimizerContext()->getSymbolAllocator()->newSymbol(left_keys[0][key_index]);
    ...
    {
        const auto & join_step = dynamic_cast<const JoinStepExt &>(*node->getChildren()[i]->getStep());
                    ...
                    if (!expression_map.contains(name))
                    {
                        ...
                        if (!symbol_index_to_out.count(symbol_index))
                        {
                            symbol_index_to_out[symbol_index] = symbol_to_out.contains(symbol)
                                ...
                                : context.context->getOptimizerContext()->getSymbolAllocator()->newSymbol(symbol);
    ...
    auto new_union_node
        = PlanNodeBase::createPlanNode(context.context->getOptimizerContext()->nextNodeId(), std::make_shared<UnionStepExt>(inputs, output, new_out2ins), lefts);
    ...
    auto new_join_node = PlanNodeBase::createPlanNode(
        ...
        std::make_shared<JoinStepExt>(
            ...
            left_keys_mapped[0],
            dynamic_cast<const JoinStepExt &>(*node->getChildren()[0]->getStep()).getRightKeys(),
            dynamic_cast<const JoinStepExt &>(*node->getChildren()[0]->getStep()).getKeyIdsNullSafe(),
    ...
    return PlanNodeBase::createPlanNode(
        context.context->getOptimizerContext()->nextNodeId(),
        std::make_shared<ProjectionStepExt>(new_join_node->getCurrentDataStream(), assignments, name2type),
        {new_join_node});
}
...
TransformResult PushUnionThroughProjection::transformImpl(PlanNodePtr node, const Captures &, RuleContext & context)
{
    ...
    std::vector<ProjectionStepExt *> projections;
    PlanNodes new_children;
    DataStreams input_streams;
    for (auto & union_child : node->getChildren())
    {
        projections.emplace_back(dynamic_cast<ProjectionStepExt *>(union_child->getStep().get()));
    ...
    auto new_union_node = PlanNodeBase::createPlanNode(
        context.context->getOptimizerContext()->nextNodeId(), std::make_shared<UnionStepExt>(input_streams, output_stream, new_out2ins), new_children);

    return PlanNodeBase::createPlanNode(
        context.context->getOptimizerContext()->nextNodeId(),
        std::make_shared<ProjectionStepExt>(new_union_node->getCurrentDataStream(), new_assigment, name2type),
        {new_union_node});
}
...
TransformResult PushUnionThroughAgg::transformImpl(PlanNodePtr, const Captures &, RuleContext &)
{
    ...
    // auto new_union_node = PlanNodeBase::createPlanNode(
    //     context.context->nextNodeId(), std::make_shared<UnionStepExt>(input_streams, output_stream, new_out2ins), new_children);
