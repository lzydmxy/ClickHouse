...
#define VISITOR_DEF(TYPE) PlanNodePtr visit##TYPE##Node(TYPE##Node & node, ContextMutablePtr & context) override {return visitPlanNode(node, context);};
    APPLY_QUERY_PLAN_STEP_TYPES(VISITOR_DEF)
#undef VISITOR_DEF

#define APPLY_UNIFY_NULLABLE_TYPE_REWRITER_QUERY_PLAN_STEP(M) \
M(ProjectionStepExt) \
M(JoinStepExt) \
M(AggregatingStepExt) \
M(MergingAggregatedStepExt) \
M(UnionStepExt) \
M(ExchangeStepExt) \
M(CTERefStepExt) \
...
#define VISITOR_DEF(TYPE) PlanNodePtr visit##TYPE##NodeImpl(TYPE##Node & node, ContextMutablePtr & context);
