...
namespace DB
{
bool SortingOrderedSource::rewrite(QueryPlanExt & plan, ContextMutablePtr context) const
{
...
PlanAndPropConstants SortingOrderedSource::Rewriter::visitAggregatingStepExtNode(AggregatingStepExtNode & node, SortDescription & required)
{
    ...
    if (settings.optimize_aggregation_in_order /* && !settings.optimize_aggregate_function_type */)
    {
        ...
        SortDescriptionWithPositions prefix_sorting_with_positions;
        for (const auto & sort_column : prefix_sorting.toSortDesc())
            prefix_sorting_with_positions.emplace_back(SortColumnDescriptionWithColumnIndex(sort_column, 0));
        step->setGroupBySortDescription(prefix_sorting_with_positions);
...
PlanAndPropConstants SortingOrderedSource::Rewriter::visitCTERefStepExtNode(CTERefStepExtNode & node, SortDescription &)
{
...
PlanAndPropConstants SortingOrderedSource::Rewriter::visitTopNFilteringStepExtNode(TopNFilteringStepExtNode & node, SortDescription &)
{
...
PlanAndPropConstants SortingOrderedSource::Rewriter::visitTableScanStepExtNode(TableScanStepExtNode & node, SortDescription & required)
{
    auto & step = node.getStep();
...
PlanAndPropConstants SortingOrderedSource::Rewriter::visitFilterStepExtNode(FilterStepExtNode & node, SortDescription & required)
{
    auto result = VisitorUtil::accept(node.getChildren()[0], *this, required);
    Property any_prop;
    Property prop = PropertyDeriver::deriveProperty(node.getStep(), {result.property}, any_prop, context);
...
PlanAndPropConstants SortingOrderedSource::Rewriter::visitProjectionStepExtNode(ProjectionStepExtNode & node, SortDescription & require)
{
...
PlanNodePtr PruneSortingInfoRewriter::visitAggregatingStepExtNode(AggregatingStepExtNode & node, SortInfo &)
{
    auto prefix_desc = node.getStep()->getGroupBySortDescription();
    SortDescription sort_description;
    for (const auto & sort_description_with_position : prefix_desc)
    {
        sort_description.emplace_back(sort_description_with_position.base);
    }
    SortInfo s{sort_description, size_t{0}};
    return SimplePlanRewriter::visitPlanNode(node, s);
}
...
PlanNodePtr PruneSortingInfoRewriter::visitTableScanStepExtNode(TableScanStepExtNode & node, SortInfo & required)
{
    auto & step = node.getStep();
    ...
    SortDescription pruned_read_order;
    auto end = read_order.begin() + std::distance(it, read_order.rend());
    for (auto iter = read_order.begin(); iter != end; ++iter)
    {
        pruned_read_order.emplace_back(*iter);
...
PlanNodePtr PruneSortingInfoRewriter::visitProjectionStepExtNode(ProjectionStepExtNode & node, SortInfo & required)
{
    auto mappings = Utils::computeIdentityTranslations(node.getStep()->getAssignments());
