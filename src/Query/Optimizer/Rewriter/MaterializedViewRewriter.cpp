        ...
        if (verbose)
        {
            ...
            for (auto & item : explorer.failure_messages)
                for (auto & message : item.second)
                    LOG_DEBUG(log, "rewrite fail: plan node id: {}, type: {}, materializedView: {}, cause: {}", item.first->getId()
                        , item.first->getStep()->getName(), message.storage.getFullTableName(), message.message);
            for (auto & item : explorer.candidates)
                for (auto & candidate : item.second)
                    LOG_DEBUG(log, "rewrite success: plan node id: {}, type: {}, mview: {}.{}", item.first->getId(), item.first->getStep()->getName()
                        , candidate.target_database_and_table_name.getDatabaseName(), candidate.target_database_and_table_name.getTableName());
        }
...
protected:
    ...
    BaseTables visitTableScanStepExtNode(TableScanStepExtNode & node, std::unordered_set<String> &) override
    {
    ...
    BaseTables visitCTERefStepExtNode(CTERefStepExtNode & node, std::unordered_set<String> & required_columns) override
    {
    ...
    BaseTables process(PlanNodeBase & node, const std::unordered_set<String> & required_columns)
    {
        ...
        if (node.getType() == QueryPlanStepType::ProjectionStepExt)
        {
            auto & step = dynamic_cast<ProjectionStepExt &>(*node.getStep().get());
    ...
    void matches(PlanNodePtr query, PlanNodePtr origin, const std::unordered_set<StorageID> & tables, const std::unordered_set<String> & query_required_columns)
    {
        ...
        if (tables.size() > 1 && !context->getOptimizerContext()->getSettingsRef().enable_materialized_view_join_rewriting)
            return;

        std::vector<MaterializedViewStructurePtr> related_materialized_views;
        std::shared_ptr<const AggregatingStepExt> query_aggregate = extractTopAggregate(query);
        ...
        if (!query_map)
        {
            LOG_ERROR(logger, "skip plan node {}: duplicate symbol name.", query->getId());
            return;
        }
    ...
    std::optional<RewriterCandidate> match(
        ...
        const std::shared_ptr<const AggregatingStepExt> & query_aggregate,
        ...
        const std::shared_ptr<const AggregatingStepExt> & view_aggregate,
        ...
        const StorageID target_storage_id)
    {
        ...
        for (auto & query_to_view_table_mappings : generateFlatTableMappings(join_graph_match_result->query_to_view_table_multi_mappings))
        {
            ...
            {
                if (!context->getOptimizerContext()->getSettingsRef().enable_materialized_view_union_rewriting)
                {
            ...
            bool need_rollup = query_aggregate
                && (!async_materialized_view || context->getOptimizerContext()->getSettingsRef().enforce_materialized_view_union_rewriting || !view_aggregate
            ...
            if (empty_groupings && !context->getOptimizerContext()->getSettingsRef().enable_materialized_view_empty_grouping_rewriting) {
                continue;
            }
            ...
            for (const auto & view_output : view_outputs)
                view_output_columns_map.emplace(
                    normalizeExpression(std::make_shared<ASTIdentifier>(view_output), view_map, view_based_query_equivalences_map),
                    std::make_shared<ASTIdentifier>(view_output));
            ...
            for (const auto & name_and_type : query->getCurrentDataStream().header)
            {
                ...
                auto rewrite = rewriteExpressionContainsAggregates(
                    normalizeExpression(
                        std::make_shared<ASTIdentifier>(output_name), *query_map, query_to_view_table_mappings, view_based_query_equivalences_map),
            ...
            if (query_aggregate && need_rollup)
            {
                ...
                for (const auto & query_key : query_aggregate->getKeys())
                    view_based_query_keys.emplace(normalizeExpression(
                        std::make_shared<ASTIdentifier>(query_key), *query_map, query_to_view_table_mappings, view_based_query_equivalences_map));
            ...
            if (!PredicateUtils::isFalsePredicate(union_predicate) || context->getOptimizerContext()->getSettingsRef().enforce_materialized_view_union_rewriting)
            {
                if (query_aggregate && query->getType() != QueryPlanStepType::AggregatingStepExt)
                {
                ...
                for (const auto & plan_node : query_inner_sources)
                {
                    for (const auto & query_output : plan_node->getOutputNames())
                    {
                        ...
                        query_output_columns_map.emplace(
                            normalizeExpression(std::make_shared<ASTIdentifier>(query_output), *query_map), std::make_shared<ASTIdentifier>(query_output));
                    }
                }
            ...
            {
                const auto * table_scan = dynamic_cast<const TableScanStepExt *>(query_join_hyper_graph.getPlanNodes().at(0)->getStep().get());
    ...
    static std::shared_ptr<const DB::AggregatingStepExt> extractTopAggregate(PlanNodePtr query)
    {
        if (query->getType() == QueryPlanStepType::AggregatingStepExt)
            return dynamic_pointer_cast<const DB::AggregatingStepExt>(query->getStep());
    ...
    matchJoinSet(const JoinHyperGraph & query_graph, const JoinHyperGraph & view_graph, ContextMutablePtr context)
    {
        ...
        if (!context->getOptimizerContext()->getSettingsRef().enable_materialized_view_join_rewriting && query_nodes.size() > 1) {
            return {};
        }
        ...
        const JoinSetMatchResult & /*join_set_match_result*/)
    {
        ...
        bool all_inner = std::all_of(query_graph.getEdges().begin(), query_graph.getEdges().end(), [](const auto & edge) {
            return edge.join_step->getKind() == JoinKind::Inner;
        ...
        auto match_join_edge = [&](const JoinKind & query_kind,
                                   const JoinStrictness & query_strictness,
            ...
            JoinKind view_kind = view_edge.join_step->getKind();
            JoinStrictness view_strictness = view_edge.join_step->getStrictness();
            ...
            // left to right
            if ((query_kind == JoinKind::Inner && view_kind == JoinKind::Inner)
                || (query_kind == JoinKind::Left && view_kind == JoinKind::Right)
                || (query_kind == JoinKind::Right && view_kind == JoinKind::Left))
            {
        ...
        for (size_t i = 0; i < query_graph.getEdges().size(); i++)
        {
            ...
            JoinKind query_kind = query_edge.join_step->getKind();
            JoinStrictness query_strictness = query_edge.join_step->getStrictness();
            ...
            if (!matched && query_kind == JoinKind::Inner)
            {
                for (size_t j = 0; j < view_graph.getEdges().size(); j++)
                {
                    if (matched_view_edges.contains(i))
                    ...
                    if (view_edge.join_step->getKind() == JoinKind::Left)
                        inner_sources = view_edge.left_conditions_used_nodes;
                    else if (view_edge.join_step->getKind() == JoinKind::Right)
                    ...
                    if (matched)
                    {
                        compensation_predicates.emplace_back(makeASTFunction(
                            "isNotNull",
                            view_edge.join_step->getKind() == JoinKind::Left
                                ? std::make_shared<ASTIdentifier>(view_edge.join_step->getLeftKeys()[0])
                                : std::make_shared<ASTIdentifier>(view_edge.join_step->getRightKeys()[0])));
                        matched_view_edges.emplace(i);
                        break;
                    }
                }
...
{
public:
    static bool rewrite(QueryPlanExt & plan, ContextMutablePtr context_, std::unordered_map<PlanNodePtr, RewriterCandidates> & match_results)
    {
        Void c;
...
protected:
    ...
    PlanNodePtr visitPlanNode(PlanNodeBase & node, Void & c) override
    {
        if (auto it = match_results.find(node.shared_from_this()); it != match_results.end())
        {
            ...
            try
            {
                ...
                context->getOptimizerContext()->getOptimizerMetrics()->addMaterializedView(candidate_it->view_database_and_table_name);
                return plan;
            }
            catch (...)
            {
    ...
    PlanNodePtr constructEquivalentPlan(const PlanNodePtr & query, const RewriterCandidate & candidate)
    {
        ...
        if (!PredicateUtils::isTruePredicate(candidate.compensation_predicate))
            plan = PlanNodeBase::createPlanNode(
                context->getOptimizerContext()->nextNodeId(), std::make_shared<FilterStepExt>(plan->getCurrentDataStream(), candidate.compensation_predicate), {plan});
        ...
        if (candidate.need_rollup)
        {
            ...
            if (!PredicateUtils::isFalsePredicate(candidate.union_predicate)
                || context->getOptimizerContext()->getSettingsRef().enforce_materialized_view_union_rewriting)
                plan = planUnionBeforeAggragte(plan, query, candidate.union_predicate, assignments);

            plan = PlanNodeBase::createPlanNode(
                context->getOptimizerContext()->nextNodeId(),
                std::make_shared<ProjectionStepExt>(plan->getCurrentDataStream(), assignments, candidate.name_to_type),
                {plan});
            return plan;
        }
        else
        {
            ...
            plan = PlanNodeBase::createPlanNode(
                context->getOptimizerContext()->nextNodeId(),
                std::make_shared<ProjectionStepExt>(plan->getCurrentDataStream(), candidate.assignments, candidate.name_to_type),
                {plan});
            ...
            plan = PlanNodeBase::createPlanNode(
                context->getOptimizerContext()->nextNodeId(),
                std::make_shared<ProjectionStepExt>(plan->getStep()->getOutputStream(), assignments, candidate.name_to_type),
                {plan});
            return plan;
        }
    }
    ...
    PlanNodePtr checkOutputTypes(PlanNodePtr plan)
    {
        ...
        for(size_t i = 0; i < plan->getStep()->getOutputStream().header.columns(); i++)
        {
            ...
            if (single_aggregate_function)
            {
                assignments.emplace(
                    ...
                    makeASTFunction(
                        "cast",
                        std::make_shared<ASTIdentifier>(column.name),
                        std::make_shared<ASTLiteral>(single_aggregate_function->getFunction()->getResultType()->getName())));
                name_to_type.emplace(column.name, single_aggregate_function->getFunction()->getResultType());
            }
            else
            {
                assignments.emplace(column.name, std::make_shared<ASTIdentifier>(column.name));
        ...
        if (!Utils::isIdentity(assignments))
        {
            plan = PlanNodeBase::createPlanNode(
                    context->getOptimizerContext()->nextNodeId(),
                    std::make_shared<ProjectionStepExt>(plan->getCurrentDataStream(), assignments, name_to_type),
                    {plan});
        }
        return plan;
    }
        ...
        ASTPtr prewhere_expr)
    {
        ...
        return PlanNodeBase::createPlanNode(
            context->getOptimizerContext()->nextNodeId(),
            std::make_shared<TableScanStepExt>(
                context,
    ...
    {
        if (getQueryPlanStepType(query->getStep()) != QueryPlanStepType::AggregatingStepExt || getQueryPlanStepType(view->getStep()) != QueryPlanStepType::AggregatingStepExt)
            throw Exception(ErrorCodes::LOGICAL_ERROR, "union rewrite failed: view & query root node expectes as aggregate");

        const auto & view_step = dynamic_cast<const AggregatingStepExt &>(*view->getStep().get());
        const auto & query_step = dynamic_cast<const AggregatingStepExt &>(*query->getStep().get());

        OutputToInputs output_to_inputs;
        for (const auto & key : query_step.getKeys())
        {
            const auto & view_key = assignments.at(key);
            if (getAstType(view_key) != ASTType::ASTIdentifier)
                throw Exception(ErrorCodes::LOGICAL_ERROR, "union rewrite failed: view_key expected as identifier");
        ...
        for (const auto & aggregate : query_step.getAggregates())
        {
            ...
            if (getAstType(view_aggregate_output) != ASTType::ASTIdentifier)
                throw Exception(ErrorCodes::LOGICAL_ERROR, "union rewrite failed: view_aggregate_output expected as identifier");
            ...
            if (view_aggreagte->function->getName().ends_with("Merge"))
            {
                ...
                rewrite.function = AggregateFunctionFactory::instance().get(
                    function_name, NullsAction::EMPTY, aggregate.function->getArgumentTypes(), aggregate.function->getParameters(), properties);
            }

            // direct rollup
            if (view_aggreagte->function->getArgumentTypes().size() == 1
                && rewrite.function->getResultType()->equals(*view_aggreagte->function->getArgumentTypes()[0]))
            {
            ...
            throw Exception(ErrorCodes::LOGICAL_ERROR, "aggregate not support");
        }
        ...
        query_with_filter = PlanNodeBase::createPlanNode(
            context->getOptimizerContext()->nextNodeId(),
            std::make_shared<AggregatingStepExt>(
        ...
        return PlanNodeBase::createPlanNode(context->getOptimizerContext()->nextNodeId(), view->getStep(), {unionn});
    }
    ...
    planAggregate(PlanNodePtr plan, bool need_rollup, const std::vector<ASTPtr> & rollup_keys, const Assignments & assignments)
    {
        ...
        {
            auto new_symbol = context->getOptimizerContext()->getSymbolAllocator()->newSymbol(key);
        ...
        if (!agg_rewriter.aggregates.empty() || !keys.empty())
        {
            if (!Utils::isIdentity(arguments))
                plan = PlanNodeBase::createPlanNode(
                    context->getOptimizerContext()->nextNodeId(),
                    std::make_shared<ProjectionStepExt>(plan->getCurrentDataStream(), arguments, arguments_types),
                    {plan});

            plan = PlanNodeBase::createPlanNode(
                context->getOptimizerContext()->nextNodeId(),
                std::make_shared<AggregatingStepExt>(plan->getCurrentDataStream(), keys, NameSet{}, agg_rewriter.aggregates, GroupingSetsParamsExtList{}, true),
                {plan});
        }
        ...
        ASTPtr visitASTFunction(ASTPtr & node, Void & c) override
        {
            ...
            if (agg_it == aggregates_map.end())
            {
                ...
                for (auto & argument : function.arguments->children)
                {
                    ...
                    {
                        auto new_symbol = context->getOptimizerContext()->getSymbolAllocator()->newSymbol(argument);
                ...
                auto output_column = context->getOptimizerContext()->getSymbolAllocator()->newSymbol(node);
                AggregateDescription aggregate_description;
                AggregateFunctionProperties properties;
                aggregate_description.function = AggregateFunctionFactory::instance().get(
                    function.name, NullsAction::EMPTY, agg_argument_types, parameters, properties);
            ...
            if (!predicate_added)
            {
                for (const auto & symbol : node.getOutputNames())
                {
                    if (required_symbols.contains(symbol))
                    {
                        ...
                        return PlanNodeBase::createPlanNode(
                            context->getOptimizerContext()->nextNodeId(),
                            std::make_shared<FilterStepExt>(node.getStep()->getOutputStream(), predicate),
...
bool MaterializedViewRewriter::rewrite(QueryPlanExt & plan, ContextMutablePtr context) const
{
    try
    {
    ...
    {
        if (context->getOptimizerContext()->getSettingsRef().enforce_materialized_view_rewrite)
            throw;
...
bool MaterializedViewRewriter::rewriteImpl(QueryPlanExt & plan, ContextMutablePtr context) const
{
    bool enforce = context->getOptimizerContext()->getSettingsRef().enforce_materialized_view_rewrite;
    bool verbose = context->getOptimizerContext()->getSettingsRef().enable_materialized_view_rewrite_verbose_log;

    auto materialized_views = getRelatedMaterializedViews(plan, context);
    if (materialized_views.empty())
    {
        if (enforce)
            throw Exception(ErrorCodes::NO_AVAILABLE_MATERIALIZED_VIEW, "no related materialized views");
        return false;
    }
    ...
    {
        if (enforce)
            throw Exception(ErrorCodes::NO_AVAILABLE_MATERIALIZED_VIEW, "no materialized view candidates");
        return false;
    }
...
LinkedHashMap<MaterializedViewStructurePtr, PartitionCheckResult> MaterializedViewRewriter::getRelatedMaterializedViews(QueryPlanExt & plan, ContextMutablePtr context) const
{
