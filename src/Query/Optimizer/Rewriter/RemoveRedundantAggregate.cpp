#include <Query/Optimizer/Rewriter/RemoveRedundantAggregate.h>
...
namespace DB
{
bool RemoveRedundantDistinct::rewrite(QueryPlanExt & plan, ContextMutablePtr context) const
{
...
PlanNodePtr RemoveRedundantAggregateVisitor::visitLimitByStepNode(LimitByStepNode & node, RemoveRedundantAggregateContext & ctx)
{
...
PlanNodePtr RemoveRedundantAggregateVisitor::visitFilterStepExtNode(FilterStepExtNode & node, RemoveRedundantAggregateContext & ctx)
{
    auto child = VisitorUtil::accept(node.getChildren()[0], *this, ctx);
    PlanNodes children{child};
    return resetChildren(node, children, ctx);
}
PlanNodePtr RemoveRedundantAggregateVisitor::visitDistinctStepExtNode(DistinctStepExtNode & node, RemoveRedundantAggregateContext & ctx)
{
...
PlanNodePtr RemoveRedundantAggregateVisitor::visitAggregatingStepExtNode(AggregatingStepExtNode & node, RemoveRedundantAggregateContext & ctx)
{
    auto step = node.getStep();
...
PlanNodePtr RemoveRedundantAggregateVisitor::visitProjectionStepExtNode(ProjectionStepExtNode & node, RemoveRedundantAggregateContext & ctx)
{
    auto step = node.getStep();
    ...
    for (auto & assignment : step->getAssignments())
    {
        if (input_symbols.find(assignment.first) == input_symbols.end())
        {
            ...
            if(getAstType(ast) == ASTType::ASTIdentifier)
            {
...
PlanNodePtr RemoveRedundantAggregateVisitor::visitCTERefStepExtNode(CTERefStepExtNode & node, RemoveRedundantAggregateContext & ctx)
{
...
PlanNodePtr RemoveRedundantAggregateVisitor::visitJoinStepExtNode(JoinStepExtNode & node, RemoveRedundantAggregateContext & ctx)
{
    auto step  = node.getStep();
    ...
    if (kind == JoinKind::Left)
    {
        ...
        {
            if (step->getStrictness() == JoinStrictness::Any || right_distinct_flag == true)
    ...
    else if (kind == JoinKind::Right)
    {
        ...
        {
            if (step->getStrictness() == JoinStrictness::Any || left_distinct_flag == true)
...
PlanNodePtr RemoveRedundantAggregateVisitor::visitTableScanStepExtNode(TableScanStepExtNode & node, RemoveRedundantAggregateContext & ctx)
{
    const auto * step = dynamic_cast<const TableScanStepExt *>(node.getStep().get());
...
std::set<std::string> RemoveRedundantAggregateVisitor::extractSymbol(const ConstASTPtr & node)
{
    ...
    if(getAstType(node) == ASTType::ASTIdentifier)
    {
    ...
    else if(getAstType(node) == ASTType::ASTFunction)
    {
        ...
        if(function.name == "tuple")
        {
            ...
            if(getAstType(child_node) == ASTType::ASTExpressionList)
            {
                ...
                {
                    if(getAstType(child) == ASTType::ASTIdentifier)
                    {
