#include <Query/Optimizer/Rewriter/ShareCommonExpression.h>
...
namespace
{
    ...
    {
        assert(node != nullptr);
        auto type = getQueryPlanStepType(node->getStep());

        // for now simply skip this complex cases, see ASSERT-386
        if (node->getChildren().size() > 1)
            return false;

        if (type == QueryPlanStepType::ProjectionStepExt)
        {
            const auto & projection = dynamic_cast<ProjectionStepExt &>(*node->getStep());
        ...
        if (type == QueryPlanStepType::FilterStepExt)
        {
            const auto & filter = dynamic_cast<FilterStepExt &>(*node->getStep());
            return !ExpressionDeterminism::canChangeOutputRows(filter.getFilter(), context);
        }

        if (type == QueryPlanStepType::TableScanStepExt)
        {
            // const auto & table_scan = dynamic_cast<TableScanStepExt &>(*node->getStep());
            return false;
        }

        const static std::unordered_set<QueryPlanStepType> sharable_steps{
            ...
            QueryPlanStepType::ExchangeStepExt,
        };
    ...
    std::optional<int> FindSharableExpressionASTVisitor::visitASTFunction(ASTPtr & node, const Void & context)
    {
        ...
        if (RuntimeFilterUtils::isInternalRuntimeFilter(node) || ctx->getOptimizerContext()->isNonDeterministicFunction(function.name))
            return {};
    ...
    public:
        ...
        void visitTableScanStepExtNode(TableScanStepExtNode & node, const Void &) override;
        void visitFilterStepExtNode(FilterStepExtNode & node, const Void &) override;
        void visitProjectionStepExtNode(ProjectionStepExtNode & node, const Void &) override;
    ...
    void FindSharableExpressionPlanVisitor::visitTableScanStepExtNode(TableScanStepExtNode & node, const Void &)
    {
        if (context->getOptimizerContext()->getSettingsRef().enable_common_expression_sharing_for_prewhere)
        {
            auto & step = dynamic_cast<TableScanStepExt &>(*node.getStep().get());
    ...
    void FindSharableExpressionPlanVisitor::visitFilterStepExtNode(FilterStepExtNode & node, const Void &)
    {
        auto & step = dynamic_cast<FilterStepExt &>(*node.getStep().get());
    ...
    void FindSharableExpressionPlanVisitor::visitProjectionStepExtNode(ProjectionStepExtNode & node, const Void &)
    {
        ...
        PlanNodeBase * bottom, ExpressionInfos & expression_infos, SymbolTranslationMap & symbol_translation, ContextMutablePtr context)
    {
        ...
        auto & table_scan = dynamic_cast<TableScanStepExt &>(*bottom->getStep().get());
        auto & symbol_allocator = context->getOptimizerContext()->getSymbolAllocator();
    ...
    public:
        ...
        PlanNodePtr visitProjectionStepExtNode(ProjectionStepExtNode & node, PlanNodePtr & cur_node) override;
        PlanNodePtr visitFilterStepExtNode(FilterStepExtNode & node, PlanNodePtr & cur_node) override;
    ...
    PlanNodePtr FoldExpressionPlanVisitor::visitProjectionStepExtNode(ProjectionStepExtNode & node, PlanNodePtr & cur_node)
    {
        ...
        auto input_type_name = cur_node->getCurrentDataStream().header.getColumnsWithTypeAndName();
        NameToType input_types;
        for (const auto & type_name : input_type_name)
        {
            input_types.emplace(type_name.name, type_name.type);
        }
    ...
    {
        auto & filter_step = dynamic_cast<FilterStepExt &>(*node.getStep().get());
    ...
    while (!stack.empty())
    {
        ...
        do
        {
            ...
            pruneSharableExpressions(
                expression_infos, std::max(context->getOptimizerContext()->getSettingsRef().common_expression_sharing_threshold.value, UInt64{1}));
        ...
        if (!expression_infos.empty())
        {
            ...
            if (bottom->getType() == QueryPlanStepType::TableScanStepExt)
            ...
            size_t next_node_idx = bottom->getType() == QueryPlanStepType::TableScanStepExt ? stack.size() - 1 : stack.size();
...
bool ShareCommonExpression::rewrite(QueryPlanExt & plan, ContextMutablePtr context) const
{
