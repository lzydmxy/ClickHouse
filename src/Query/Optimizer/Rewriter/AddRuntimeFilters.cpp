#include <Query/Optimizer/Rewriter/AddRuntimeFilters.h>
...
namespace DB
{
bool AddRuntimeFilters::rewrite(QueryPlanExt & plan, ContextMutablePtr context) const
{
    ...
    auto print_graphviz = [&](const String & hint)
    {
        ...
        GraphvizPrinter::printLogicalPlan(
            plan, context, fmt::format("{}{}ms", context->getOptimizerContext()->getRuleId(), rule_watch.elapsedMilliseconds()));
        rule_watch.restart();
    };
    ...
    if (!context->getOptimizerContext()->getSettingsRef().enable_runtime_filter_pipeline_poll || !plan.getCTEInfo().empty())
    {
...
PlanPropEquivalences AddRuntimeFilters::AddRuntimeFilterRewriter::visitCTERefStepExtNode(CTERefStepExtNode & node, Void & c)
{
...
PlanPropEquivalences AddRuntimeFilters::AddRuntimeFilterRewriter::visitJoinStepExtNode(JoinStepExtNode & node, Void & c)
{
    ...
    if (join.getKind() != JoinKind::Inner && join.getKind() != JoinKind::Right
        && join.getStrictness() != JoinStrictness::Semi)
...
bool AddRuntimeFilters::AddRuntimeFilterRewriter::rewrite(QueryPlanExt & plan)
{
    Void c;
...
RuntimeFilterContext AddRuntimeFilters::RuntimeFilterInfoExtractor::extract(QueryPlanExt & plan, ContextMutablePtr & context)
{
...
InheritedRuntimeFilters AddRuntimeFilters::RuntimeFilterInfoExtractor::visitExchangeStepExtNode(
    ExchangeStepExtNode & node, std::unordered_set<RuntimeFilterId> & local_runtime_filter)
{
    const auto * exchange_step = dynamic_cast<const ExchangeStepExt *>(node.getStep().get());
    if (exchange_step->getExchangeMode() == RExchangeMode::LOCAL_MAY_NEED_REPARTITION
        || exchange_step->getExchangeMode() == RExchangeMode::LOCAL_NO_NEED_REPARTITION)
...
InheritedRuntimeFilters AddRuntimeFilters::RuntimeFilterInfoExtractor::visitProjectionStepExtNode(
    ProjectionStepExtNode & node, std::unordered_set<RuntimeFilterId> & local_runtime_filter)
{
    ...
    {
        if (getAstType(assignment.second) == ASTType::ASTIdentifier && result.contains(assignment.first))
...
AddRuntimeFilters::RuntimeFilterInfoExtractor::visitJoinStepExtNode(JoinStepExtNode & node, std::unordered_set<RuntimeFilterId> & local_runtime_filter)
{
...
InheritedRuntimeFilters
AddRuntimeFilters::RuntimeFilterInfoExtractor::visitCTERefStepExtNode(CTERefStepExtNode & node, std::unordered_set<RuntimeFilterId> &)
{
    const auto * cte = dynamic_cast<const CTERefStepExt *>(node.getStep().get());
    std::unordered_set<RuntimeFilterId> empty;
    cte_helper.accept(cte->getId(), *this, empty);
    return {};
}

InheritedRuntimeFilters AddRuntimeFilters::RuntimeFilterInfoExtractor::visitFilterStepExtNode(
    FilterStepExtNode & node, std::unordered_set<RuntimeFilterId> & local_runtime_filter)
{
    auto result = VisitorUtil::accept(node.getChildren()[0], *this, local_runtime_filter);
...
PlanNodePtr AddRuntimeFilters::RemoveUnusedRuntimeFilterProbRewriter::visitFilterStepExtNode(
    FilterStepExtNode & node, std::unordered_set<RuntimeFilterId> & allowed_runtime_filters)
{
    auto child = VisitorUtil::accept(*node.getChildren()[0], *this, allowed_runtime_filters);

    String black_list = context->getOptimizerContext()->getSettingsRef().runtime_filter_black_list;
    ...
    if (!child_stats || child_stats.value()->getRowCount() < context->getOptimizerContext()->getSettingsRef().runtime_filter_min_filter_rows)
    {
    ...
    for (auto & runtime_filter : filters.first)
    {
        ...
        double min_filter_factor = is_table_scan_filter ? context->getOptimizerContext()->getSettingsRef().runtime_filter_min_filter_factor
                                                        : context->getOptimizerContext()->getSettingsRef().runtime_filter_min_filter_factor_for_non_table_scan;
...
PlanNodePtr AddRuntimeFilters::RemoveUnusedRuntimeFilterProbRewriter::visitJoinStepExtNode(
    JoinStepExtNode & node, std::unordered_set<RuntimeFilterId> & allowed_runtime_filters)
{
    const auto * join_step = dynamic_cast<const JoinStepExt *>(node.getStep().get());
...
PlanNodePtr AddRuntimeFilters::RemoveUnusedRuntimeFilterProbRewriter::visitCTERefStepExtNode(
    CTERefStepExtNode & node, std::unordered_set<RuntimeFilterId> & allowed_runtime_filters)
{
    const auto * cte = dynamic_cast<const CTERefStepExt *>(node.getStep().get());
...
PlanNodePtr AddRuntimeFilters::RemoveUnusedRuntimeFilterBuildRewriter::visitJoinStepExtNode(JoinStepExtNode & node, Void & c)
{
    const auto * join_step = dynamic_cast<const JoinStepExt *>(node.getStep().get());

    std::unordered_map<String, RuntimeFilter> runtime_filter_builders;
    for (const auto & runtime_filter : join_step->getRuntimeFilterBuilders())
    {
        if (effective_runtime_filters.contains(runtime_filter.second.id))
        {
            ...
            RRuntimeFilter::Enum distribution = runtime_filter.second.distribution;
            if (distribution == RRuntimeFilter::UNKNOWN)
                distribution = runtime_filter_context.distributed_runtime_filters.contains(runtime_filter.second.id)
                    ? RRuntimeFilter::DISTRIBUTED
                    : RRuntimeFilter::LOCAL;

            runtime_filter_builders.emplace(runtime_filter.first, RuntimeFilter{runtime_filter.second.id, distribution});
        }
    }
    ...
    auto new_join_step = std::make_shared<JoinStepExt>(
        ...
        join_step->isOrdered(),
        join_step->isSimpleReordered(),
        LinkedHashMap<String, RuntimeFilter>{runtime_filter_builders.begin(), runtime_filter_builders.end()});

    node.setStep(new_join_step);
    return visitPlanNode(node, c);
}

PlanNodePtr AddRuntimeFilters::RemoveUnusedRuntimeFilterBuildRewriter::visitCTERefStepExtNode(CTERefStepExtNode & node, Void & c)
{
    const auto * cte = dynamic_cast<const CTERefStepExt *>(node.getStep().get());
...
PlanNodePtr AddRuntimeFilters::AddExchange::visitExchangeStepExtNode(ExchangeStepExtNode & node, std::unordered_set<RuntimeFilterId> &)
{
    std::unordered_set<RuntimeFilterId> need_exchange;
    return SimplePlanRewriter::visitPlanNode(node, need_exchange);
}

PlanNodePtr AddRuntimeFilters::AddExchange::visitJoinStepExtNode(JoinStepExtNode & node, std::unordered_set<RuntimeFilterId> & need_exchange)
{
...
PlanNodePtr AddRuntimeFilters::AddExchange::visitBufferStepExtNode(BufferStepExtNode & node, std::unordered_set<RuntimeFilterId> & need_exchange)
{
    auto res = SimplePlanRewriter::visitBufferStepExtNode(node, need_exchange);
...
PlanNodePtr AddRuntimeFilters::AddExchange::visitCTERefStepExtNode(CTERefStepExtNode & node, std::unordered_set<RuntimeFilterId> &)
{
    std::unordered_set<RuntimeFilterId> need_exchange;
    return SimplePlanRewriter::visitCTERefStepExtNode(node, need_exchange);
}

PlanNodePtr AddRuntimeFilters::AddExchange::visitFilterStepExtNode(FilterStepExtNode & node, std::unordered_set<RuntimeFilterId> & need_exchange)
{
    if (need_exchange.empty() || context->getOptimizerContext()->getSettingsRef().enable_runtime_filter_pipeline_poll)
        return SimplePlanRewriter::visitPlanNode(node, need_exchange);
