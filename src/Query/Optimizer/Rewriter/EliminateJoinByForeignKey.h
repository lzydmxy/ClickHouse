...
public:
    ...
    FPKeysAndOrdinaryKeys visitJoinStepExtNode(JoinStepExtNode &, JoinInfo &) override;
    FPKeysAndOrdinaryKeys visitTableScanStepExtNode(TableScanStepExtNode &, JoinInfo &) override;
    ...
    FPKeysAndOrdinaryKeys visitFilterStepExtNode(FilterStepExtNode &, JoinInfo &) override;
    FPKeysAndOrdinaryKeys visitUnionStepExtNode(UnionStepExtNode &, JoinInfo &) override;
    ...
    void collectEliminableJoin(const Pairs & winners)
    {
        ...
        {
            const auto & bottom_joins = winner.second.bottom_joins;
            if (context->getOptimizerContext()->getSettingsRef().enable_eliminate_complicated_pk_fk_join && bottom_joins.size() > 1)
            {
...
public:
    ...
    PlanNodePtr visitProjectionStepExtNode(ProjectionStepExtNode &, JoinEliminationContext &) override;
    PlanNodePtr visitJoinStepExtNode(JoinStepExtNode &, JoinEliminationContext &) override;
    PlanNodePtr visitTableScanStepExtNode(TableScanStepExtNode &, JoinEliminationContext &) override;
    PlanNodePtr visitFilterStepExtNode(FilterStepExtNode &, JoinEliminationContext &) override;
    PlanNodePtr visitAggregatingStepExtNode(AggregatingStepExtNode &, JoinEliminationContext &) override;
    PlanNodePtr visitUnionStepExtNode(UnionStepExtNode &, JoinEliminationContext &) override;
    PlanNodePtr visitCTERefStepExtNode(CTERefStepExtNode &, JoinEliminationContext &) override;
