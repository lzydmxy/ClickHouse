...
static ASTPtr
getBoundComparisons(const String & left, const String & minValue, const String & maxValue, const ASTQuantifiedComparisonExt & qc_ast);
static bool shouldCompareValueWithLowerBound(const ASTQuantifiedComparisonExt & qc_ast);
...
bool RemoveCorrelatedScalarSubquery::rewrite(QueryPlanExt & plan, ContextMutablePtr context) const
{
...
PlanNodePtr CorrelatedScalarSubqueryVisitor::visitApplyStepExtNode(ApplyStepExtNode & node, Void & v)
{
    auto apply_ptr = visitPlanNode(node, v);
    const auto & apply_step = *node.getStep();

    if (apply_step.getSubqueryType() != ApplyStepExt::SubqueryType::SCALAR)
    {
        return apply_ptr;
    }
    ...
    if (getQueryPlanStepType(subquery_step_ptr) == QueryPlanStepType::EnforceSingleRowStepExt)
    {
        ...
        if (!result.has_value())
        {
            throw Exception(ErrorCodes::REMOVE_SUBQUERY_ERROR,
                "Correlated Scalar subquery de-correlation error, correlation filter not exists: ");
        }
        ...
        String unique = context->getOptimizerContext()->getSymbolAllocator()->newSymbol("assign_unique_id_symbol");
        auto unique_id_step = std::make_unique<AssignUniqueIdStepExt>(join_left->getStep()->getOutputStream(), unique);
        auto unique_id_node = std::make_shared<AssignUniqueIdStepExtNode>(context->getOptimizerContext()->nextNodeId(), std::move(unique_id_step), PlanNodes{join_left});

        // step 3 : construct a Left JoinStepExtNode to replace ApplyStepExtNode
        ...
        {
            output.emplace_back(item.type, item.name);
        }
        ...
        PlanNodePtr join_node
            = std::make_shared<JoinStepExtNode>(context->getOptimizerContext()->nextNodeId(), std::move(join_step), PlanNodes{unique_id_node, join_right});

        String marker = context->getOptimizerContext()->getSymbolAllocator()->newSymbol("is_distinct");
        ...
        auto mark_distinct_step
            = std::make_shared<MarkDistinctStepExt>(join_node->getStep()->getOutputStream(), marker, distinct_symbols_list);
        auto mark_distinct_node = PlanNodeBase::createPlanNode(context->getOptimizerContext()->nextNodeId(), std::move(mark_distinct_step), PlanNodes{join_node});

        auto check_subquery = makeASTFunction("check_subquery_return_single_row", ASTs{std::make_shared<ASTIdentifier>(marker)});

        auto filter_step = std::make_shared<FilterStepExt>(join_node->getStep()->getOutputStream(), check_subquery);
        PlanNodePtr filter_node
            = std::make_shared<FilterStepExtNode>(context->getOptimizerContext()->nextNodeId(), std::move(filter_step), PlanNodes{mark_distinct_node});
    ...
    if (getQueryPlanStepType(subquery_step_ptr) == QueryPlanStepType::AggregatingStepExt)
    {
        const auto & step = dynamic_cast<const AggregatingStepExt &>(*subquery_step_ptr);
    ...
    if (getQueryPlanStepType(subquery_step_ptr) == QueryPlanStepType::ProjectionStepExt)
    {
        const auto & expression_step = dynamic_cast<const ProjectionStepExt &>(*subquery_step_ptr);
        ...
        if (getQueryPlanStepType(child_step_ptr) == QueryPlanStepType::AggregatingStepExt)
        {
            const auto & step = dynamic_cast<const AggregatingStepExt &>(*child_step_ptr);
            const auto & keys = step.getKeys();
            if (keys.empty())
            {
                match = true;
    ...
    if (!match)
    {
        throw Exception(ErrorCodes::REMOVE_SUBQUERY_ERROR, "Un-matched pattern for Correlated Scalar subquery");
    }
    ...
    {
        throw Exception(
            ErrorCodes::REMOVE_SUBQUERY_ERROR, "Correlated Scalar subquery de-correlation error, correlation filter not exists: ");
    }
    ...
    String non_null = context->getOptimizerContext()->getSymbolAllocator()->newSymbol("build_side_non_null_symbol");
    ...
    auto non_null_step
        = std::make_shared<ProjectionStepExt>(join_right->getStep()->getOutputStream(), null_value_assignments, null_value_name_to_type);
    auto non_null_node = std::make_shared<ProjectionStepExtNode>(context->getOptimizerContext()->nextNodeId(), std::move(non_null_step), PlanNodes{join_right});

    // step 4 : construct a Left JoinStepExtNode to replace ApplyStepExtNode
    const DataStream & left_data_stream = unique_id_node->getStep()->getOutputStream();
    ...
    auto join_step = std::make_shared<JoinStepExt>(
       ...
       JoinStrictness::All,
        ...
        DistributionType::UNKNOWN);
    PlanNodePtr join_node
        = std::make_shared<JoinStepExtNode>(context->getOptimizerContext()->nextNodeId(), std::move(join_step), PlanNodes{unique_id_node, non_null_node});

    // step 5 : construct a AggregateNode with group-by columns (replace the scalar aggregation)
    const auto & scalar_agg_step = dynamic_cast<const AggregatingStepExt &>(*scalar_agg->getStep());
    Names keys;
    for (const auto & item : left_header)
    {
    ...
    for (const auto & desc : descs)
    {
        ...
        AggregateFunctionPtr fun_with_mask = AggregateFunctionFactory::instance().get(fun_name + "If", NullsAction::EMPTY, types, desc.parameters, properties);
    ...
    auto group_agg_step = std::make_shared<AggregatingStepExt>(
        join_node->getStep()->getOutputStream(), keys, NameSet{}, descs_with_mask, GroupingSetsParamsExtList{}, true);
    auto group_agg_node = std::make_shared<AggregatingStepExtNode>(context->getOptimizerContext()->nextNodeId(), std::move(group_agg_step), PlanNodes{join_node});
    ...
    auto expression_step = std::make_shared<ProjectionStepExt>(group_agg_node->getStep()->getOutputStream(), assignments, name_to_type);
    auto expression_node = std::make_shared<ProjectionStepExtNode>(context->getOptimizerContext()->nextNodeId(), std::move(expression_step), PlanNodes{group_agg_node});
    return expression_node;
}

bool RemoveUnCorrelatedScalarSubquery::rewrite(QueryPlanExt & plan, ContextMutablePtr context) const
{
...
PlanNodePtr UnCorrelatedScalarSubqueryVisitor::visitApplyStepExtNode(ApplyStepExtNode & node, Void & v)
{
    ...
    {
        if (apply_step.getApplyType() == ApplyStepExt::ApplyType::CROSS)
        {
            return subquery_ptr;
        }
    }
    ...
    if (context->getOptimizerContext()->getSettingsRef().enable_execute_uncorrelated_subquery)
    {
        ...
        int rule_id = context->getOptimizerContext()->getRuleId();
        String sub_query_id = std::to_string(context->getOptimizerContext()->incAndGetSubQueryId());
        String subquery_name_prefix = fmt::format("{}_RemoveUnCorrelatedScalarSubquery_ExecuteSubQuery_{}_", rule_id, sub_query_id );

        context->getOptimizerContext()->setExecuteSubQueryPath(subquery_name_prefix);
        ...
        context->getOptimizerContext()->removeExecuteSubQueryPath();
        context->setCurrentQueryId(query_id);
        context->getOptimizerContext()->setRuleId(rule_id);
        ...
        auto scalar_step = std::make_shared<ProjectionStepExt>(input_ptr->getStep()->getOutputStream(), assignments, types);
        return PlanNodeBase::createPlanNode(context->getOptimizerContext()->nextNodeId(), std::move(scalar_step), PlanNodes{input_ptr});
    }
    ...
    return std::make_shared<JoinStepExtNode>(context->getOptimizerContext()->nextNodeId(), std::move(join_step), PlanNodes{input_ptr, subquery_ptr});
}

bool RemoveCorrelatedInSubquery::rewrite(QueryPlanExt & plan, ContextMutablePtr context) const
{
...
PlanNodePtr CorrelatedInSubqueryVisitor::visitApplyStepExtNode(ApplyStepExtNode & node, Void & v)
{
    PlanNodePtr apply_ptr = visitPlanNode(node, v);
    const auto & apply_step = *node.getStep();

    if (apply_step.getSubqueryType() != ApplyStepExt::SubqueryType::IN)
    {
        return apply_ptr;
    }
    ...
    String unique = context->getOptimizerContext()->getSymbolAllocator()->newSymbol("assign_unique_id_symbol");
    auto unique_id_step = std::make_unique<AssignUniqueIdStepExt>(input_ptr->getStep()->getOutputStream(), unique);
    auto unique_id_node = std::make_shared<AssignUniqueIdStepExtNode>(context->getOptimizerContext()->nextNodeId(), std::move(unique_id_step), PlanNodes{input_ptr});
    ...
    auto expression_step = std::make_shared<ProjectionStepExt>(decorrelation_source->getStep()->getOutputStream(), assignments_new, name_to_type);
    auto expression_node
        = std::make_shared<ProjectionStepExtNode>(context->getOptimizerContext()->nextNodeId(), std::move(expression_step), PlanNodes{decorrelation_source});

    // step 3 : construct a JoinStepExtNode to replace ApplyStepExtNode
    ...
    auto join_step = std::make_shared<JoinStepExt>(
       ...
       JoinStrictness::All,
        ...
        ASOFJoinInequality::GreaterOrEquals,
        DistributionType::UNKNOWN);
    PlanNodePtr join_node = std::make_shared<JoinStepExtNode>(context->getOptimizerContext()->nextNodeId(), std::move(join_step), PlanNodes{join_left, join_right});

    // step 6 project match_condition_symbol, null_match_condition_symbol
    String match_condition_symbol = context->getOptimizerContext()->getSymbolAllocator()->newSymbol("system_match_condition_symbol");
    ...
    String null_match_condition_symbol = context->getOptimizerContext()->getSymbolAllocator()->newSymbol("system_null_match_condition_symbol");
    ...
    auto pre_expression_step = std::make_shared<ProjectionStepExt>(join_node->getStep()->getOutputStream(), pre_assignments, pre_name_to_type);
    auto pre_expression_node
        = std::make_shared<ProjectionStepExtNode>(context->getOptimizerContext()->nextNodeId(), std::move(pre_expression_step), PlanNodes{join_node});
    ...
    AggregateFunctionProperties properties;
    AggregateFunctionPtr agg_fun = AggregateFunctionFactory::instance().get("countIf", NullsAction::EMPTY, types, parameters, properties);
    AggregateDescription aggregate_desc_1;
    aggregate_desc_1.column_name = context->getOptimizerContext()->getSymbolAllocator()->newSymbol("count_matches");
    ...
    AggregateFunctionPtr agg_fun_2 = AggregateFunctionFactory::instance().get("countIf",  NullsAction::EMPTY, types_2, parameters_2, properties);
    AggregateDescription aggregate_desc_2;
    aggregate_desc_2.column_name = context->getOptimizerContext()->getSymbolAllocator()->newSymbol("count_null_matches");
    ...
    auto count_step = std::make_shared<AggregatingStepExt>(
        pre_expression_node->getStep()->getOutputStream(), keys, NameSet{}, aggregate_descs, GroupingSetsParamsExtList{}, true);
    auto count_node = std::make_shared<AggregatingStepExtNode>(context->getOptimizerContext()->nextNodeId(), std::move(count_step), PlanNodes{pre_expression_node});
    ...
    auto multi_if_expression_step
        = std::make_shared<ProjectionStepExt>(count_node->getStep()->getOutputStream(), multi_if_assignments, multi_if_pre_name_to_type);
    auto multi_if_expression_node
        = std::make_shared<ProjectionStepExtNode>(context->getOptimizerContext()->nextNodeId(), std::move(multi_if_expression_step), PlanNodes{count_node});
...
bool RemoveUnCorrelatedInSubquery::rewrite(QueryPlanExt & plan, ContextMutablePtr context) const
{
...
PlanNodePtr UnCorrelatedInSubqueryVisitor::visitApplyStepExtNode(ApplyStepExtNode & node, Void & v)
{
    PlanNodePtr apply_ptr = visitPlanNode(node, v);
    const auto & apply_step = *node.getStep();
    ...
    if (context->getOptimizerContext()->getSettingsRef().enable_execute_uncorrelated_subquery)
    {
        ...
        if (stats.has_value() && stats.value()->getRowCount() <= context->getOptimizerContext()->getSettingsRef().execute_uncorrelated_in_subquery_size)
        {
            const Assignment & apply_assignment = apply_step.getAssignment();
            String sub_query_column_name = apply_assignment.first;
            DataTypePtr sub_query_column_type = apply_step.getAssignmentDataType();
            ...
            int rule_id = context->getOptimizerContext()->getRuleId();
            String sub_query_id = std::to_string(context->getOptimizerContext()->incAndGetSubQueryId());
            String subquery_name_prefix = fmt::format("{}_RemoveUnCorrelatedInSubquery_ExecuteSubQuery_{}_", rule_id, sub_query_id, "_");
            ...
            auto in_step = std::make_shared<ProjectionStepExt>(input_ptr->getStep()->getOutputStream(), assignments_in, types);
            return PlanNodeBase::createPlanNode(context->getOptimizerContext()->nextNodeId(), std::move(in_step), PlanNodes{input_ptr});
        }
    }
    ...
    auto distinct_step = std::make_unique<DistinctStepExt>(
        ...
        true,
        true);
    auto distinct_node = std::make_shared<DistinctStepExtNode>(context->getOptimizerContext()->nextNodeId(), std::move(distinct_step), PlanNodes{subquery_ptr});
    ...
    auto expression_step = std::make_shared<ProjectionStepExt>(distinct_node->getStep()->getOutputStream(), assignments, name_to_type);
    auto expression_node = std::make_shared<ProjectionStepExtNode>(context->getOptimizerContext()->nextNodeId(), std::move(expression_step), PlanNodes{distinct_node});
    ...
    auto join_step = std::make_shared<JoinStepExt>(
        streams,
        DataStream{.header = ToColumnsWithTypeAndName(output)},
        JoinKind::Left,
       JoinStrictness::All,
        context->getSettingsRef().max_threads,
        context->getSettingsRef().optimize_read_in_order,
    ...
    PlanNodePtr join_node = std::make_shared<JoinStepExtNode>(context->getOptimizerContext()->nextNodeId(), std::move(join_step), PlanNodes{input_ptr, expression_node});
    ...
    auto project_step = std::make_shared<ProjectionStepExt>(join_node->getStep()->getOutputStream(), in_assignments, in_name_to_type);
    auto project_node = std::make_shared<ProjectionStepExtNode>(context->getOptimizerContext()->nextNodeId(), std::move(project_step), PlanNodes{join_node});
...
bool RemoveCorrelatedExistsSubquery::rewrite(QueryPlanExt & plan, ContextMutablePtr context) const
{
...
PlanNodePtr CorrelatedExistsSubqueryVisitor::visitApplyStepExtNode(ApplyStepExtNode & node, Void & v)
{
    PlanNodePtr apply_ptr = visitPlanNode(node, v);
    const auto & apply_step = *node.getStep();

    if (apply_step.getSubqueryType() != ApplyStepExt::SubqueryType::EXISTS)
    {
        return apply_ptr;
    }
    ...
    {
        throw Exception(
            ErrorCodes::REMOVE_SUBQUERY_ERROR, "Correlated Exists subquery de-correlation error, correlation filter not exists: ");
    }
    ...
    if (filter.empty())
    {
        ...
        auto right_correlation_step = std::make_shared<ProjectionStepExt>(
        ...
        auto right_correlation_node
            = std::make_shared<ProjectionStepExtNode>(context->getOptimizerContext()->nextNodeId(), std::move(right_correlation_step), PlanNodes{subquery_ptr});
        ...
        auto distinct_step = std::make_unique<DistinctStepExt>(
            ...
            true);
        auto distinct_node
            = std::make_shared<DistinctStepExtNode>(context->getOptimizerContext()->nextNodeId(), std::move(distinct_step), PlanNodes{right_correlation_node});

        // step 3 : add extra non-null symbol, default value = 1.
        Assignments non_null_assignments;
        NameToType non_null_name_to_type;
        String non_null_symbol = context->getOptimizerContext()->getSymbolAllocator()->newSymbol("build_side_non_null_symbol");
        ...
        auto non_null_step
            = std::make_shared<ProjectionStepExt>(subquery_ptr->getStep()->getOutputStream(), non_null_assignments, non_null_name_to_type);
        auto non_null_node = std::make_shared<ProjectionStepExtNode>(context->getOptimizerContext()->nextNodeId(), std::move(non_null_step), PlanNodes{distinct_node});
        ...
        auto exists_step
            = std::make_shared<ProjectionStepExt>(join_node->getStep()->getOutputStream(), exists_assignments, exists_name_to_type);
        auto exists_node = std::make_shared<ProjectionStepExtNode>(context->getOptimizerContext()->nextNodeId(), std::move(exists_step), PlanNodes{join_node});
        return exists_node;
    }
    ...
    auto non_null_step
        = std::make_shared<ProjectionStepExt>(subquery_ptr->getStep()->getOutputStream(), non_null_assignments, non_null_name_to_type);
    auto non_null_node = std::make_shared<ProjectionStepExtNode>(context->getOptimizerContext()->nextNodeId(), std::move(non_null_step), PlanNodes{subquery_ptr});
    ...
    auto filter_step = std::make_shared<ProjectionStepExt>(join_node->getStep()->getOutputStream(), filter_assignments, filter_name_to_type);
    auto filter_node = std::make_shared<ProjectionStepExtNode>(context->getOptimizerContext()->nextNodeId(), std::move(filter_step), PlanNodes{join_node});
    ...
    auto remove_null_step
        = std::make_shared<ProjectionStepExt>(join_node->getStep()->getOutputStream(), remove_null_assignments, remove_null_name_to_type);
    auto remove_null_node = std::make_shared<ProjectionStepExtNode>(context->getOptimizerContext()->nextNodeId(), std::move(remove_null_step), PlanNodes{join_node});
    ...
    String count_non_null_value = context->getOptimizerContext()->getSymbolAllocator()->newSymbol("count_non_null_value");
    ...
    auto count_non_null_step = std::make_shared<AggregatingStepExt>(
        remove_null_node->getStep()->getOutputStream(), keys, NameSet{}, aggregate_descs, GroupingSetsParamsExtList{}, true);
    auto count_non_null_node
        = std::make_shared<AggregatingStepExtNode>(context->getOptimizerContext()->nextNodeId(), std::move(count_non_null_step), PlanNodes{remove_null_node});
    ...
    auto exists_step
        = std::make_shared<ProjectionStepExt>(count_non_null_node->getStep()->getOutputStream(), exist_assignments, exist_name_to_type);
    auto exists_node = std::make_shared<ProjectionStepExtNode>(context->getOptimizerContext()->nextNodeId(), std::move(exists_step), PlanNodes{count_non_null_node});
    return exists_node;
}

bool RemoveUnCorrelatedExistsSubquery::rewrite(QueryPlanExt & plan, ContextMutablePtr context) const
{
...
PlanNodePtr UnCorrelatedExistsSubqueryVisitor::visitApplyStepExtNode(ApplyStepExtNode & node, Void & v)
{
    PlanNodePtr apply_ptr = visitPlanNode(node, v);
    const auto & apply_step = *node.getStep();
    ...
    Array parameters = Array();
    AggregateFunctionProperties properties;
    AggregateFunctionPtr agg_fun = AggregateFunctionFactory::instance().get("count", NullsAction::EMPTY,types, parameters, properties);
    AggregateDescription aggregate_desc;
    aggregate_desc.column_name = context->getOptimizerContext()->getSymbolAllocator()->newSymbol("count");
    aggregate_desc.parameters = parameters;
    aggregate_desc.function = agg_fun;
    AggregateDescriptions aggregate_descs{aggregate_desc};
    auto count_subquery_step = std::make_shared<AggregatingStepExt>(
        subquery_ptr->getStep()->getOutputStream(), keys, NameSet{}, aggregate_descs, GroupingSetsParamsExtList{}, true);
    auto count_subquery_node
        = std::make_shared<AggregatingStepExtNode>(context->getOptimizerContext()->nextNodeId(), std::move(count_subquery_step), PlanNodes{subquery_ptr});
    ...
    auto project_exists_symbol_step
        = std::make_shared<ProjectionStepExt>(count_subquery_node->getStep()->getOutputStream(), assignments, name_to_type);
    auto project_exists_symbol_node
        = std::make_shared<ProjectionStepExtNode>(context->getOptimizerContext()->nextNodeId(), std::move(project_exists_symbol_step), expression_children);
    ...
    if (context->getOptimizerContext()->getSettingsRef().enable_execute_uncorrelated_subquery)
    {
        ...
        String subquery_name_prefix = fmt::format("{}_RemoveUnCorrelatedExistsSubquery_ExecuteSubQuery_{}_", rule_id, sub_query_id);
        ...
        auto step_exists = std::make_shared<ProjectionStepExt>(input_ptr->getStep()->getOutputStream(), assignments_exists, types_new);
        return PlanNodeBase::createPlanNode(context->getOptimizerContext()->nextNodeId(), std::move(step_exists), PlanNodes{input_ptr});
    }
    ...
    // if exists values is true, then condition match, else not match.
    auto join_node
        = std::make_shared<JoinStepExtNode>(context->getOptimizerContext()->nextNodeId(), std::move(join_step), PlanNodes{input_ptr, project_exists_symbol_node});
...
bool RemoveUnCorrelatedQuantifiedComparisonSubquery::rewrite(QueryPlanExt& plan, ContextMutablePtr context) const
{
...
PlanNodePtr UnCorrelatedQuantifiedComparisonSubqueryVisitor::visitApplyStepExtNode(ApplyStepExtNode & node, Void & v)
{
    PlanNodePtr apply_ptr = visitPlanNode(node, v);
    auto & apply_step = *node.getStep();

    if (apply_step.getSubqueryType() != ApplyStepExt::SubqueryType::QUANTIFIED_COMPARISON)
    {
        return apply_ptr;
    }
    ...
    auto & qc_ast = qc_assignment.second->as<ASTQuantifiedComparisonExt &>();
    ...
    Names keys;
    auto agg_step
        = std::make_shared<AggregatingStepExt>(right_data_stream, keys, NameSet{}, aggregate_descriptions, GroupingSetsParamsExtList{}, true);
    auto agg_node = std::make_shared<AggregatingStepExtNode>(context->getOptimizerContext()->nextNodeId(), std::move(agg_step), PlanNodes{subquery_ptr});
    ...
    auto join_step = std::make_shared<JoinStepExt>(
       ...
       JoinStrictness::Unspecified,
        ...
        ASOFJoinInequality::GreaterOrEquals,
        DistributionType::UNKNOWN);
    PlanNodePtr join_node = std::make_shared<JoinStepExtNode>(context->getOptimizerContext()->nextNodeId(), std::move(join_step), PlanNodes{input_ptr, agg_node});
    ...
    auto project_step = std::make_shared<ProjectionStepExt>(join_node->getStep()->getOutputStream(), qc_assignments, qc_name_to_type);
    auto project_node = std::make_shared<ProjectionStepExtNode>(context->getOptimizerContext()->nextNodeId(), std::move(project_step), PlanNodes{join_node});
    return project_node;
}

bool RemoveCorrelatedQuantifiedComparisonSubquery::rewrite(QueryPlanExt& plan, ContextMutablePtr context) const
{
...
PlanNodePtr CorrelatedQuantifiedComparisonSubqueryVisitor::visitApplyStepExtNode(ApplyStepExtNode & node, Void & v)
{
    PlanNodePtr apply_ptr = visitPlanNode(node, v);
    auto & apply_step = *node.getStep();
    ...
    auto expression_step = std::make_shared<ProjectionStepExt>(decorrelation_source->getStep()->getOutputStream(), assignments, name_to_type);
    auto expression_node
        = std::make_shared<ProjectionStepExtNode>(context->getOptimizerContext()->nextNodeId(), std::move(expression_step), PlanNodes{decorrelation_source});
    ...
    auto & qc_assignment = apply_step.getAssignment();
    auto & quantified_comparison = qc_assignment.second->as<ASTQuantifiedComparisonExt &>();
    ...
    auto count_step = std::make_shared<AggregatingStepExt>(
        join_node->getStep()->getOutputStream(), keys, NameSet{}, aggregate_descriptions, GroupingSetsParamsExtList{}, true);
    auto count_node = std::make_shared<AggregatingStepExtNode>(context->getOptimizerContext()->nextNodeId(), std::move(count_step), PlanNodes{join_node});
    ...
    auto project_step = std::make_shared<ProjectionStepExt>(count_node->getStep()->getOutputStream(), qc_assignments, qc_name_to_type);
    auto project_node = std::make_shared<ProjectionStepExtNode>(context->getOptimizerContext()->nextNodeId(), std::move(project_step), PlanNodes{count_node});
    return project_node;
}
...
ASTPtr getBoundComparisons(const String & left, const String & minValue, const String & maxValue, const ASTQuantifiedComparisonExt & qc_ast)
{
...
bool shouldCompareValueWithLowerBound(const ASTQuantifiedComparisonExt & qc_ast)
{
...
void makeAggDescriptionsMinMaxCountCount2(
    ...
    Names & qc_right)
{
    min_value = context->getOptimizerContext()->getSymbolAllocator()->newSymbol("min_value");
    max_value = context->getOptimizerContext()->getSymbolAllocator()->newSymbol("max_value");
    count_all_value = context->getOptimizerContext()->getSymbolAllocator()->newSymbol("count_all_value");
    count_non_null_value = context->getOptimizerContext()->getSymbolAllocator()->newSymbol("count_non_null_value");
...
ConstRefPatternPtr UnnestingWithWindow::getPattern() const
{
    ...
    static auto pattern = Patterns::filter()
        .with(Patterns::apply()
                  .matchingStep<ApplyStepExt>([](const ApplyStepExt & s) {
                      return s.getSubqueryType() == ApplyStepExt::SubqueryType::SCALAR && !s.getCorrelation().empty();
                  })
                  .with(Patterns::any(), Patterns::aggregating().matchingStep<AggregatingStepExt>([](const AggregatingStepExt & agg) {
...
TransformResult UnnestingWithWindow::transformImpl(PlanNodePtr filter_node, const Captures &, RuleContext & rule_context)
{
    ...
    auto apply_step = dynamic_cast<ApplyStepExtNode *>(apply_node.get())->getStep();
    auto left_node = apply_node->getChildren()[0];
    auto right_node = apply_node->getChildren()[1];
    AggregatingStepExt * agg_step = nullptr;
    ProjectionStepExt * proj_step = nullptr;
    if (getQueryPlanStepType(right_node->getStep()) == QueryPlanStepType::AggregatingStepExt)
    {
        agg_step = dynamic_cast<AggregatingStepExtNode *>(right_node.get())->getStep().get();
    }
    else
    {
        proj_step = dynamic_cast<ProjectionStepExtNode *>(right_node.get())->getStep().get();
    ...
    std::set<QueryPlanStepType> left_support_step_types{
        QueryPlanStepType::ProjectionStepExt,
        QueryPlanStepType::TableScanStepExt,
        QueryPlanStepType::JoinStepExt,
    };
    std::set<QueryPlanStepType> right_support_step_types{
        QueryPlanStepType::TableScanStepExt,
        QueryPlanStepType::JoinStepExt,
    ...
    std::unordered_map<StorageID, TableScanStepExt *> outer_id_to_plan_node;
    std::vector<StorageID> left_tables;
    for (const auto & node : left_nodes)
    {
        // check children type
        auto step_type = getQueryPlanStepType(node->getStep());
        if (!left_support_step_types.count(step_type))
            return {};
        if (step_type == QueryPlanStepType::JoinStepExt)
        {
            // todo support more join type (inner)
            auto join_step = dynamic_cast<JoinStepExtNode *>(node.get())->getStep();
            if (!join_step->isCrossJoin())
                return {};
        }
        if (step_type == QueryPlanStepType::TableScanStepExt)
        {
            auto table_scan = dynamic_cast<TableScanStepExtNode *>(node.get())->getStep();
            left_tables.emplace_back(table_scan->getStorageID());
            outer_id_to_plan_node[table_scan->getStorageID()] = table_scan.get();
        }
    }

    std::unordered_map<StorageID, TableScanStepExt *> sub_id_to_plan_node;
    std::vector<StorageID> right_tables;
    size_t filter_count = 0;
    FilterStepExt * sub_filter_step = nullptr;
    for (const auto & node : right_nodes)
    {
        ...
        if (step_type == QueryPlanStepType::FilterStepExt)
        {
            filter_count++;
            sub_filter_step = dynamic_cast<FilterStepExtNode *>(node.get())->getStep().get();
            // todo
    ...
    auto output_names = ToNameSet(correlation_table_step->getTableOutputStream().header.getNamesAndTypes());
    ...
    auto filter_step = dynamic_cast<FilterStepExtNode *>(filter_node.get())->getStep();
    ...
    auto extract_symbol_mapping = [](const std::unordered_map<StorageID, TableScanStepExt *> & id_to_plan_node) {
    ...
    desc.window_name = rule_context.context->getOptimizerContext()->getSymbolAllocator()->newSymbol("__window_name");
    for (const auto & col : apply_step->getCorrelation())
    {
    ...
    auto window_step = std::make_shared<WindowStep>(new_filter_node->getCurrentDataStream(), desc, std::vector<WindowFunctionDescription>{}, false);
    ...
    if (proj_step)
    {
        ...
        auto new_proj_step = std::make_shared<ProjectionStepExt>(left_node->getCurrentDataStream(), assignments, name_to_type);
        window_node = PlanNodeBase::createPlanNode(rule_context.context->getOptimizerContext()->nextNodeId(), new_proj_step, {window_node});
    }

    auto after_filter_step = std::make_shared<FilterStepExt>(left_node->getCurrentDataStream(), correlation_filter);
    auto after_filter_node = PlanNodeBase::createPlanNode(rule_context.context->getOptimizerContext()->nextNodeId(), after_filter_step, {window_node});
...
ConstRefPatternPtr UnnestingWithProjectionWindow::getPattern() const
{
    ...
    static auto pattern = Patterns::filter()
        .with(Patterns::apply()
                  ...
                  .with(
                      Patterns::any(),
                      Patterns::project().with(Patterns::aggregating().matchingStep<AggregatingStepExt>([](const AggregatingStepExt & agg) {
                          return agg.getAggregates().size() == 1 && agg_white_list.count(agg.getAggregates()[0].function->getName());
                      }))))
        .result();
    return pattern;
...
ConstRefPatternPtr ExistsToSemiJoin::getPattern() const
{
    static auto pattern = Patterns::apply()
        .matchingStep<ApplyStepExt>([](const ApplyStepExt & s) {
            return s.supportSemiAnti() && s.getSubqueryType() == ApplyStepExt::SubqueryType::EXISTS && !s.getCorrelation().empty();
        })
        .result();
    return pattern;
}
...
TransformResult ExistsToSemiJoin::transformImpl(PlanNodePtr node, const Captures &, RuleContext & rule_context)
{
    ...
    if (context->getOptimizerContext()->getSettingsRef().enable_unnesting_subquery_with_semi_anti_join)
    {
        auto * apply = dynamic_cast<ApplyStepExtNode *>(node.get());
        ...
        auto strictness =JoinStrictness::Unspecified;
        ...
        for (const auto & column : subquery_ptr->getStep()->getOutputStream().header)
        {
            ...
            auto join_step = std::make_shared<JoinStepExt>(
                ...
                DistributionType::UNKNOWN);
            PlanNodePtr join_node
                = std::make_shared<JoinStepExtNode>(context->getOptimizerContext()->nextNodeId(), std::move(join_step), PlanNodes{join_left, join_right});
        ...
        // Exists function has been rewritten into "exists" symbols
        auto exists_step = std::make_shared<ProjectionStepExt>(join_node->getStep()->getOutputStream(), exist_assignments, exist_name_to_type);
            PlanNodePtr exists_node = std::make_shared<ProjectionStepExtNode>(context->getOptimizerContext()->nextNodeId(), std::move(exists_step), PlanNodes{join_node});
            return exists_node;
            }
...
ConstRefPatternPtr InToSemiJoin::getPattern() const
{
     static auto pattern = Patterns::apply()
        .matchingStep<ApplyStepExt>([](const ApplyStepExt & s) {
            return s.supportSemiAnti() && s.getSubqueryType() == ApplyStepExt::SubqueryType::IN && s.getCorrelation().empty();
        })
        .result();
    return pattern;
}
