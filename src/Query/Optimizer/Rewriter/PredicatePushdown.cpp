...
bool PredicatePushdown::rewrite(QueryPlanExt & plan, ContextMutablePtr context) const
{
...
PlanNodePtr PredicateVisitor::visitPlanNode(PlanNodeBase & node, PredicateContext & predicate_context)
{
    ...
    if (!PredicateUtils::isTruePredicate(predicate_context.predicate))
    {
        ...
        auto cols_with_type_and_name = rewritten->getStep()->getOutputStream().header.getColumnsWithTypeAndName();
        ...
        auto filter_step = std::make_shared<FilterStepExt>(rewritten->getStep()->getOutputStream(), remaining_filter);
        auto filter_node = PlanNodeBase::createPlanNode(context->getOptimizerContext()->nextNodeId(), std::move(filter_step), PlanNodes{rewritten});
        return filter_node;
    }
    return rewritten;
}

PlanNodePtr PredicateVisitor::visitProjectionStepExtNode(ProjectionStepExtNode & node, PredicateContext & predicate_context)
{
    const auto & step = *node.getStep();
    const auto & assignments = step.getAssignments();
    if (!predicate_context.context->getOptimizerContext()->getSettingsRef().enable_pushdown_filter_through_stateful)
    {
    ...
    if (!pushdown_predicate->as<ASTLiteral>())
        pushdown_predicate
            = ExpressionInterpreter::optimizePredicate(pushdown_predicate, name_to_type, context);
...
PlanNodePtr PredicateVisitor::visitFilterStepExtNode(FilterStepExtNode & node, PredicateContext & predicate_context)
{
    const auto & step = *node.getStep();

    // handle in function has large value list
    UInt64 limit = predicate_context.context->getOptimizerContext()->getSettingsRef().max_in_value_list_to_pushdown;
    std::pair<ConstASTPtr, ConstASTPtr> split_in_filter = FilterStepExt::splitLargeInValueList(step.getFilter(), limit);
    ...
    if (getQueryPlanStepType(rewritten->getStep()) != QueryPlanStepType::FilterStepExt)
    {
        ...
        {
            auto filter_step = std::make_shared<FilterStepExt>(rewritten->getStep()->getOutputStream(), split_in_filter.second);
            return std::make_shared<FilterStepExtNode>(context->getOptimizerContext()->nextNodeId(), std::move(filter_step), PlanNodes{rewritten});
        }
        return rewritten;
    }
    ...
    if (getQueryPlanStepType(rewritten->getStep()) != QueryPlanStepType::FilterStepExt)
    {
        ...
        const auto & rewritten_step = dynamic_cast<const FilterStepExt &>(*rewritten_step_ptr);
...
PlanNodePtr PredicateVisitor::visitAggregatingStepExtNode(AggregatingStepExtNode & node, PredicateContext & predicate_context)
{
    const auto & step = *node.getStep();
    ...
    if (!post_aggregation_conjuncts.empty())
    {
        ...
        output = PlanNodeBase::createPlanNode(context->getOptimizerContext()->nextNodeId(), std::move(filter_step), PlanNodes{output});
    }
    return output;
}

PlanNodePtr PredicateVisitor::visitJoinStepExtNode(JoinStepExtNode & node, PredicateContext & predicate_context)
{
    ...
    {
        throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Unsupported join type : Comma");
    }
    ...
    for (auto & conjunct : PredicateUtils::extractConjuncts(new_join_predicate))
    {
        if (PredicateUtils::isJoinClause(conjunct, left_symbols, right_symbols, context))
        {
            ...
            std::set<std::string> left_equality_symbols = SymbolsExtractor::extract(equality.arguments->children[0]);
            std::set<std::string> right_equality_symbols = SymbolsExtractor::extract(equality.arguments->children[1]);
            ...
            ASTPtr & left_expression = (left_aligned_comparison && right_aligned_comparison) ? equality.arguments->children[0]
                                                                                             : equality.arguments->children[1];
            ASTPtr & right_expression = (left_aligned_comparison && right_aligned_comparison) ? equality.arguments->children[1]
                                                                                              : equality.arguments->children[0];
            String left_symbol = left_expression->as<ASTIdentifier>()
                ? left_expression->getColumnName()
                : context->getOptimizerContext()->getSymbolAllocator()->newSymbol(left_expression->getColumnName());
            ...
            String right_symbol = right_expression->as<ASTIdentifier>()
                ...
                : context->getOptimizerContext()->getSymbolAllocator()->newSymbol(right_expression->getColumnName());
    ...
    if (kind == JoinKind::Cross && !join_clauses.empty() /*inner join*/)
    {
        Names left_keys;
        Names right_keys;
    ...
    {
        auto left_expression_step = std::make_shared<ProjectionStepExt>(left->getStep()->getOutputStream(), left_assignments, left_types);
        auto left_expression_node
            = std::make_shared<ProjectionStepExtNode>(context->getOptimizerContext()->nextNodeId(), std::move(left_expression_step), PlanNodes{left});

        auto right_expression_step = std::make_shared<ProjectionStepExt>(right->getStep()->getOutputStream(), right_assignments, right_types);
        auto right_expression_node
            = std::make_shared<ProjectionStepExtNode>(context->getOptimizerContext()->nextNodeId(), std::move(right_expression_step), PlanNodes{right});
    ...
    else
    {
        auto left_source_expression_step
            = std::make_shared<ProjectionStepExt>(left_source->getStep()->getOutputStream(), left_assignments, left_types);
        left_source_expression_node
            = std::make_shared<ProjectionStepExtNode>(context->getOptimizerContext()->nextNodeId(), std::move(left_source_expression_step), PlanNodes{left_source});
    }
    ...
    else
    {
        auto right_source_expression_step
            = std::make_shared<ProjectionStepExt>(right_source->getStep()->getOutputStream(), right_assignments, right_types);
        right_source_expression_node
            = std::make_shared<ProjectionStepExtNode>(context->getOptimizerContext()->nextNodeId(), std::move(right_source_expression_step), PlanNodes{right_source});
    }
    ...
    ColumnsWithTypeAndName output = step->getOutputStream().header.getColumnsWithTypeAndName();
    ...
    Names right_keys;
    {
        ...
        const bool allow_extended_type_conversion = context->getOptimizerContext()->getSettingsRef().allow_extended_type_conversion;
        const bool enable_implicit_arg_type_convert = context->getOptimizerContext()->getSettingsRef().enable_implicit_arg_type_convert;
            ...
            if (!JoinCommon::isJoinCompatibleTypes(left_type, right_type))
            {
                ...
                catch (DB::Exception & ex)
                {
                    throw Exception(ErrorCodes::TYPE_MISMATCH,
                        "Type mismatch of columns to JOIN by: {} at left, {} at right. Can't get supertype: {}", left_type->getName(), right_type->getName(), ex.message());
    ...
    std::shared_ptr<JoinStepExt> join_step;
    ...
    {
        auto filter_step = std::make_shared<FilterStepExt>(join_node->getStep()->getOutputStream(), post_join_predicate);
        auto filter_node = PlanNodeBase::createPlanNode(context->getOptimizerContext()->nextNodeId(), std::move(filter_step), PlanNodes{join_node});
    ...
    if (join_node_output != output_node_output)
    {
        ...
        auto output_expression_step
            = std::make_shared<ProjectionStepExt>(output_node->getStep()->getOutputStream(), std::move(output_assignments), std::move(output_types));
        auto output_expression_node
            = std::make_shared<ProjectionStepExtNode>(context->getOptimizerContext()->nextNodeId(), std::move(output_expression_step), PlanNodes{output_node});
        output_node = output_expression_node;
    }
    return output_node;
}

PlanNodePtr PredicateVisitor::visitArrayJoinStepNode(ArrayJoinStepNode & node, PredicateContext & predicate_context)
{
    const auto & step = *node.getStep();
    ...
    for (const auto & input : step.arrayJoin()->columns)
    {
...
PlanNodePtr PredicateVisitor::visitExchangeStepExtNode(ExchangeStepExtNode & node, PredicateContext & predicate_context)
{
...
PlanNodePtr PredicateVisitor::visitWindowStepNode(WindowStepNode & node, PredicateContext & predicate_context)
{
    ...
    if (!PredicateUtils::isTruePredicate(non_push_down_predicate))
    {
        ...
        auto filter_step = std::make_shared<FilterStepExt>(rewritten->getStep()->getOutputStream(), extra_predicate);
        auto filter_node = PlanNodeBase::createPlanNode(context->getOptimizerContext()->nextNodeId(), std::move(filter_step), PlanNodes{rewritten});
        return filter_node;
    }
    return rewritten;
...
PlanNodePtr PredicateVisitor::visitMarkDistinctStepExtNode(MarkDistinctStepExtNode & node, PredicateContext & predicate_context)
{
    auto & step_ptr = node.getStep();
    auto & step = dynamic_cast<MarkDistinctStepExt &>(*step_ptr);
...
PlanNodePtr PredicateVisitor::visitMergeSortingStepExtNode(MergeSortingStepExtNode & node, PredicateContext & predicate_context)
{
...
PlanNodePtr PredicateVisitor::visitUnionStepExtNode(UnionStepExtNode & node, PredicateContext & predicate_context)
{
    const auto & step = *node.getStep();
    PlanNodes children;
...
PlanNodePtr PredicateVisitor::visitDistinctStepExtNode(DistinctStepExtNode & node, PredicateContext & predicate_context)
{
    return processChild(node, predicate_context);
}

PlanNodePtr PredicateVisitor::visitAssignUniqueIdStepExtNode(AssignUniqueIdStepExtNode & node, PredicateContext & predicate_context)
{
    ...
    {
        throw Exception(ErrorCodes::NOT_IMPLEMENTED, "UniqueId in predicate is not yet supported");
    }
    return processChild(node, predicate_context);
}

PlanNodePtr PredicateVisitor::visitCTERefStepExtNode(CTERefStepExtNode & node, PredicateContext & predicate_context)
{
    auto * cte_step = dynamic_cast<CTERefStepExt *>(node.getStep().get());
    ...
    if (cte_refs.size() == cte_reference_counts.at(cte_step->getId()))
    {
        ...
        if (pushdown_filter_into_cte)
        {
            ...
            const auto types_and_names = cte_def->getStep()->getOutputStream().header.getColumnsWithTypeAndName();
            NameToType name_to_type;
            for (const auto & type_and_name : types_and_names)
            {
                name_to_type.emplace(type_and_name.name, type_and_name.type);
            }

            optimized_expression = ExpressionInterpreter::optimizePredicate(
                optimized_expression, name_to_type, context);
    ...
    std::set<String> & right_symbols)
{
    ...
    if (!inherited_predicate)
        throw Exception(ErrorCodes::PLAN_BUILD_ERROR, "Unexpected error in predicate pushdown");
...
void PredicateVisitor::tryNormalizeOuterToInnerJoin(JoinStepExtNode & node, const ConstASTPtr & inherited_predicate, ContextMutablePtr context)
{
    auto & step = *node.getStep();
    auto kind = step.getKind();
    auto strictness = step.getStrictness();
    ...
    auto columns_with_types_and_name = step.getOutputStream().header.getColumnsWithTypeAndName();
    NameToType column_types;
    for (const auto & col : columns_with_types_and_name)
    {
...
bool PredicateVisitor::isRegularJoin(const JoinStepExt & step)
{
    auto strictness = step.getStrictness();

    return strictness == JoinStrictness::All || strictness == JoinStrictness::Any;
}
...
ASTPtr EffectivePredicateVisitor::visitLimitStepExtNode(LimitStepExtNode & node, ContextMutablePtr & context)
{
    return process(node, context);
}

ASTPtr EffectivePredicateVisitor::visitEnforceSingleRowStepExtNode(EnforceSingleRowStepExtNode & node, ContextMutablePtr & context)
{
    // EnforceSingleRowNode will produce at lease one row, can't propagate predicate.
    return visitPlanNode(node, context);
}

ASTPtr EffectivePredicateVisitor::visitProjectionStepExtNode(ProjectionStepExtNode & node, ContextMutablePtr & context)
{
...
ASTPtr EffectivePredicateVisitor::visitFilterStepExtNode(FilterStepExtNode & node, ContextMutablePtr & context)
{
    ASTPtr underlying_predicate = process(node, context);
    ...
    NameToType name_types;
    for (const auto & col : step.getOutputStream().header.getColumnsWithTypeAndName())
        name_types.emplace(col.name, col.type);
    ...
    auto is_inconsistent = [&](ConstASTPtr & ptr) {
        if (ptr->as<ASTFunction>())
        {
            ...
            {
                auto left = fun.arguments->children[0];
                auto right = fun.arguments->children[1];
    ...
    std::vector<ConstASTPtr> removed_large_in_value_list
        = FilterStepExt::removeLargeInValueList(removed_inconsistent_type_filters, context->getOptimizerContext()->getSettingsRef().max_in_value_list_to_pushdown);
...
ASTPtr EffectivePredicateVisitor::visitAggregatingStepExtNode(AggregatingStepExtNode & node, ContextMutablePtr & context)
{
...
ASTPtr EffectivePredicateVisitor::visitJoinStepExtNode(JoinStepExtNode &, ContextMutablePtr &)
{
    return PredicateConst::TRUE_VALUE;
}

ASTPtr EffectivePredicateVisitor::visitExchangeStepExtNode(ExchangeStepExtNode &, ContextMutablePtr &)
{
    return PredicateConst::TRUE_VALUE;
}
...
ASTPtr EffectivePredicateVisitor::visitMergeSortingStepExtNode(MergeSortingStepExtNode & node, ContextMutablePtr & context)
{
    return process(node, context);
}

ASTPtr EffectivePredicateVisitor::visitUnionStepExtNode(UnionStepExtNode &, ContextMutablePtr &)
{
    return PredicateConst::TRUE_VALUE;
}

ASTPtr EffectivePredicateVisitor::visitTableScanStepExtNode(TableScanStepExtNode &, ContextMutablePtr &)
{
    return PredicateConst::TRUE_VALUE;
}

ASTPtr EffectivePredicateVisitor::visitDistinctStepExtNode(DistinctStepExtNode & node, ContextMutablePtr & context)
{
    return process(node, context);
}

ASTPtr EffectivePredicateVisitor::visitAssignUniqueIdStepExtNode(AssignUniqueIdStepExtNode & node, ContextMutablePtr & context)
{
    return process(node, context);
}
...
ASTPtr EffectivePredicateVisitor::visitCTERefStepExtNode(CTERefStepExtNode &, ContextMutablePtr &)
{
    return PredicateConst::TRUE_VALUE;
}
}
