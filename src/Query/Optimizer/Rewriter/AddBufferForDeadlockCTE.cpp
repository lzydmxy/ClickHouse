#include <Query/Optimizer/Rewriter/AddBufferForDeadlockCTE.h>
...
namespace
{
    ...
    public:
        ...
        void visitCTERefStepExtNode(CTERefStepExtNode & node, const JoinPath &) override;
        void visitJoinStepExtNode(JoinStepExtNode & node, const JoinPath &) override;
    ...
    public:
        ...
        void visitJoinStepExtNode(JoinStepExtNode & node, const Void &) override;
    ...
    public:
        ...
        void visitPlanNode(PlanNodeBase & node, const Void &) override;
        void visitCTERefStepExtNode(CTERefStepExtNode & node, const Void &) override;
        void visitJoinStepExtNode(JoinStepExtNode & node, const Void &) override;
    ...
    public:
        ...
        PlanNodePtr visitCTERefStepExtNode(CTERefStepExtNode & node, const Void & c) override;
    ...
    void FindDirectRightVisitor::visitCTERefStepExtNode(CTERefStepExtNode & node, const JoinPath & join_path)
    {
        visit_path.emplace_back(&node, join_path);
    ...
    void FindDirectRightVisitor::visitJoinStepExtNode(JoinStepExtNode & node, const JoinPath & join_path)
    {
        visit_path.emplace_back(&node, join_path);
    ...
    void UpdateParentExecuteOrderVisitor::visitJoinStepExtNode(JoinStepExtNode & node, const Void & ctx)
    {
    ...
    void UpdateLeftExecuteOrderVisitor::visitCTERefStepExtNode(CTERefStepExtNode & node, const Void & ctx)
    {
    ...
    void UpdateLeftExecuteOrderVisitor::visitJoinStepExtNode(JoinStepExtNode & node, const Void & ctx)
    {
        VisitorUtil::accept(*node.getChildren().at(0), *this, ctx);
    }

    PlanNodePtr AddBufferVisitor::visitCTERefStepExtNode(CTERefStepExtNode & node, const Void & c)
    {
        SimplePlanRewriter<const Void>::visitCTERefStepExtNode(node, c);
        auto cte_id = node.getStep()->getId();
         ...
         */
        Int64 max_buffer_size = context->getOptimizerContext()->getSettingsRef().max_buffer_size_for_deadlock_cte;
    ...
    public:
        ...
        std::unordered_set<CTEId> visitCTERefStepExtNode(CTERefStepExtNode & node, Void & c) override
        {
        ...
        std::unordered_set<CTEId> visitJoinStepExtNode(JoinStepExtNode & node, Void & c) override
        {
...
bool AddBufferForDeadlockCTE::rewrite(QueryPlanExt & plan, ContextMutablePtr context) const
{
    ...
    if (context->getOptimizerContext()->getSettings().enable_remove_remove_unnecessary_buffer)
    {
    ...
    if (context->getOptimizerContext()->getSettingsRef().max_buffer_size_for_deadlock_cte >= 0)
    {
