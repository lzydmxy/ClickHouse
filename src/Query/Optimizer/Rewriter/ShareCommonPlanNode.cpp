        ...
        if (auto signature = plan_node_to_signature.find(node_ptr); signature != plan_node_to_signature.end())
        {
            if (!plan_signature_to_cte_id.contains(signature->second))
            {
                ...
                return PlanNodeBase::createPlanNode(
                    context->getOptimizerContext()->nextNodeId(), std::make_shared<CTERefStepExt>(node.getStep()->getOutputStream(), cte_id, output_columns, false));
            }
            else
            {
    ...
    CTEId cte_id, const DataStream & output_stream, const DataStream & cte_output_stream, ContextMutablePtr context)
{
    ...
    return PlanNodeBase::createPlanNode(context->getOptimizerContext()->nextNodeId(), std::make_shared<CTERefStepExt>(output_stream, cte_id, output_columns, false));
}

bool ShareCommonPlanNode::rewrite(QueryPlanExt & plan, ContextMutablePtr context) const
{
    ...
    for (const auto & item : plan_node_to_signatures)
    {
        ...
        if (item.first->getChildren().empty())
            continue;
        if (item.first->getType() == QueryPlanStepType::ProjectionStepExt && item.first->getChildren()[0]->getChildren().empty())
            continue;
