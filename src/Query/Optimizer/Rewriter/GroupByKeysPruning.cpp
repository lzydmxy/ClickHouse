...
namespace DB
{
bool GroupByKeysPruning::rewrite(QueryPlanExt & plan, ContextMutablePtr context) const
{
...
PlanAndDataDependencyWithConstants GroupByKeysPruning::Rewriter::visitAggregatingStepExtNode(AggregatingStepExtNode & node, Void & v)
{
    ...
    if (has_eliminated_agg_by_constants)
    {
        ...
        auto new_projection_step
            = std::make_shared<ProjectionStepExt>(node_ptr->getCurrentDataStream(), new_assignments, new_name_to_type);
        node_ptr = ProjectionStepExtNode::createPlanNode(context->getOptimizerContext()->nextNodeId(), std::move(new_projection_step), {node_ptr});
    }
...
PlanAndDataDependencyWithConstants GroupByKeysPruning::Rewriter::visitCTERefStepExtNode(CTERefStepExtNode & node, Void & c)
{
