...
bool EliminateJoinByFK::rewrite(QueryPlanExt & plan, ContextMutablePtr context) const
{
    // todo: lizhuoyu5, other feat: need add foreign key for mergeTree
    // TableColumnInfo info(true);
    //
    // PlanNodes table_scan_nodes
    //     = CollectPlanNodeVisitor::collect(plan.getPlanNode(), {QueryPlanStepType::TableScanStepExt}, plan.getCTEInfo(), context);
    //
    // std::unordered_map<String, StorageMetadataPtr> metadatas;
    //
    // for (const PlanNodePtr & table_ptr : table_scan_nodes)
    // {
    //     String table_name = static_cast<TableScanStepExtNode &>(*table_ptr).getStep()->getStorageID().getTableName();
    //     if (metadatas.contains(table_name))
    //         continue;
    //     auto storage = static_cast<TableScanStepExtNode &>(*table_ptr).getStep()->getStorage();
...
FPKeysAndOrdinaryKeys EliminateJoinByFK::Rewriter::visitJoinStepExtNode(JoinStepExtNode & node, JoinInfo & join_info)
{
...
FPKeysAndOrdinaryKeys EliminateJoinByFK::Rewriter::visitTableScanStepExtNode(TableScanStepExtNode & node, JoinInfo &)
{
...
FPKeysAndOrdinaryKeys EliminateJoinByFK::Rewriter::visitCTERefStepExtNode(CTERefStepExtNode & node, JoinInfo & c)
{
...
FPKeysAndOrdinaryKeys EliminateJoinByFK::Rewriter::visitExchangeStepExtNode(ExchangeStepExtNode & node, JoinInfo & c)
{
...
FPKeysAndOrdinaryKeys EliminateJoinByFK::Rewriter::visitAggregatingStepExtNode(AggregatingStepExtNode & node, JoinInfo & c)
{
...
FPKeysAndOrdinaryKeys EliminateJoinByFK::Rewriter::visitLimitStepExtNode(LimitStepExtNode & node, JoinInfo & c)
{
    auto result = VisitorUtil::accept(node.getChildren()[0], *this, c);
...
FPKeysAndOrdinaryKeys EliminateJoinByFK::Rewriter::visitFilterStepExtNode(FilterStepExtNode & node, JoinInfo & c)
{
    auto result = VisitorUtil::accept(node.getChildren()[0], *this, c);
...
FPKeysAndOrdinaryKeys EliminateJoinByFK::Rewriter::visitUnionStepExtNode(UnionStepExtNode & node, JoinInfo & join_info)
{
    ...
    if (!result.getFPKeys().getPrimaryKeySet().empty() && context->getOptimizerContext()->getSettingsRef().enable_eliminate_complicated_pk_fk_join_without_top_join)
    {
        // NOTE: Currently `eliminate the join by fk without top join` only supports the union node, because corresponding eliminator must be special rewrite such as Eliminator::visitUnionStepExtNode.
...
PlanNodePtr EliminateJoinByFK::Eliminator::visitProjectionStepExtNode(ProjectionStepExtNode & node, JoinEliminationContext & c)
{
    ...
    if (c.in_eliminating)
    {
        ...
        auto expr_node = ProjectionStepExtNode::createPlanNode(context->getOptimizerContext()->nextNodeId(), std::move(expr_step), children);
        return expr_node;
    }
...
PlanNodePtr EliminateJoinByFK::Eliminator::visitJoinStepExtNode(JoinStepExtNode & node, JoinEliminationContext & c)
{
    PlanNodes children;
    DataStreams inputs;
    ...
    if (!c.in_eliminating)
    {
            ...
            iter != bottom_joins.end())
        {
            ...
            if (winner.second.winner->getType() != QueryPlanStepType::JoinStepExt)
            {
                if (is_fk_in_right == 0)
                {
            ...
            if ((kind != JoinKind::Left && kind != JoinKind::Right) || node.getStep()->getStrictness() == JoinStrictness::Semi)
            {
                auto filter_step = std::make_shared<FilterStepExt>(fk_node->getCurrentDataStream(), makeASTFunction("isNotNull", std::make_shared<ASTIdentifier>(current_fk_name)));
                child_node = FilterStepExtNode::createPlanNode(context->getOptimizerContext()->nextNodeId(), std::move(filter_step), {fk_node});
            }

            // extract filter from the join.
            if (auto join_filter = node.getStep()->getFilter(); join_filter && join_filter != PredicateConst::TRUE_VALUE)
            {
                auto filter_step = std::make_shared<FilterStepExt>(child_node->getCurrentDataStream(), join_filter);
                child_node = FilterStepExtNode::createPlanNode(context->getOptimizerContext()->nextNodeId(), std::move(filter_step), {child_node});
            }

            if (bottom_joins.begin()->first->getId() == winner.second.winner->getId()) // if can remove directly.
            {
                return createNewProjectionThenEnd(child_node, node.getCurrentDataStream().header.getNamesAndTypes(), c);
            }
    ...
    auto current_join_node = JoinStepExtNode::createPlanNode(node.getId(), std::move(join_step), children);
...
PlanNodePtr EliminateJoinByFK::Eliminator::visitTableScanStepExtNode(TableScanStepExtNode & node, JoinEliminationContext & c)
{
    NameToNameMap translation;
    for (const auto & item : node.getStep()->getColumnAlias())
        translation.emplace(item.first, item.second);
...
PlanNodePtr EliminateJoinByFK::Eliminator::visitFilterStepExtNode(FilterStepExtNode & node, JoinEliminationContext & c)
{
    return visitPlanNode(node, c);
}

PlanNodePtr EliminateJoinByFK::Eliminator::visitAggregatingStepExtNode(AggregatingStepExtNode & n, JoinEliminationContext & c)
{
    AggregatingStepExtNode & node = static_cast<AggregatingStepExtNode &>(*visitPlanNode(n, c));
    ...
    if (c.in_eliminating)
    {
        ...
        if (!c.additional_fk_for_multi_child_node.empty())
        {
            ...
            if (!ToNameToType(node.getCurrentDataStream().header.getColumnsWithTypeAndName()).contains(fk_name) && std::find(new_keys.begin(), new_keys.end(), fk_name) == new_keys.end())
            {
        ...
        return AggregatingStepExtNode::createPlanNode(context->getOptimizerContext()->nextNodeId(), std::move(agg_step), node.getChildren());
    }
...
PlanNodePtr EliminateJoinByFK::Eliminator::createNewProjectionThenEnd(PlanNodePtr child_node, const NamesAndTypes & expected_header, JoinEliminationContext & c)
{
    ...
    */
    ...
    auto new_projection_step
        = std::make_shared<ProjectionStepExt>(child_node->getCurrentDataStream(), new_assignments, new_name_to_type);

    c.clear();
    return ProjectionStepExtNode::createPlanNode(context->getOptimizerContext()->nextNodeId(), std::move(new_projection_step), {child_node});
}
...
PlanNodePtr EliminateJoinByFK::Eliminator::createNewJoinThenEnd(const String & fk_name, PlanNodePtr current_node, const NamesAndTypes & expected_header, JoinEliminationContext & c)
{
    ...
    */
    ...
    PlanNodes table_scan_nodes = CollectPlanNodeVisitor::collect(
        first_bottom_join.first, {QueryPlanStepType::TableScanStepExt}, cte_helper.getCTEInfo(), context);
    ...
    auto tb_step = static_cast<TableScanStepExtNode &>(*table_scan_ptr).getStep();

    NamesWithAliases column_alias;
    for (const auto & column : tb_step->getStorage()->getInMemoryMetadataPtr()->getColumns().getAll())
    {
        column_alias.emplace_back(column.name, context->getOptimizerContext()->getSymbolAllocator()->newSymbol(column.name));
    }

    auto new_table_scan_node = TableScanStepExtNode::createPlanNode(
    ...
    for (const auto & item : static_cast<const TableScanStepExt &>(*new_table_scan_node->getStep()).getColumnAlias())
    ...
    DataTypePtr left_type = ToNameToType(new_inputs[0].header.getColumnsWithTypeAndName()).at(fk_name);
    DataTypePtr right_type = ToNameToType(new_inputs[1].header.getColumnsWithTypeAndName()).at(pk_name);

    // 3-1. add new projection at pk side with cast function.
    if (!JoinCommon::isJoinCompatibleTypes(left_type, right_type))
    {
        auto common_type = getLeastSupertype(DataTypes{left_type, right_type});
        ...
        for (const auto & [name, type] : new_table_scan_node->getCurrentDataStream().header.getNamesAndTypes())
        {
            if (name == pk_name)
            {
        ...
        auto new_projection_step
            = std::make_shared<ProjectionStepExt>(new_table_scan_node->getCurrentDataStream(), new_assignments, new_name_to_type);

        new_table_scan_node = ProjectionStepExtNode::createPlanNode(context->getOptimizerContext()->nextNodeId(), std::move(new_projection_step), {new_table_scan_node});
    }
    ...
    auto new_join_step = std::make_shared<JoinStepExt>(
        ...
        JoinKind::Inner,
        JoinStrictness::All,
        static_cast<JoinStepExt & >(*first_bottom_join.first->getStep()).getMaxStreams(),
        static_cast<JoinStepExt & >(*first_bottom_join.first->getStep()).getKeepLeftReadInOrder(),
        Names{fk_name},
        Names{pk_name});

    auto new_join_node
        = JoinStepExtNode::createPlanNode(context->getOptimizerContext()->nextNodeId(), std::move(new_join_step), {current_node, new_table_scan_node});
    ...
    auto new_projection_step
        = std::make_shared<ProjectionStepExt>(new_join_node->getCurrentDataStream(), new_assignments, new_name_to_type);


    // end eliminating.
    c.clear();

    return ProjectionStepExtNode::createPlanNode(context->getOptimizerContext()->nextNodeId(), std::move(new_projection_step), {new_join_node});
}

PlanNodePtr EliminateJoinByFK::Eliminator::visitUnionStepExtNode(UnionStepExtNode & node, JoinEliminationContext & c)
{
    PlanNodes children;
    DataStreams inputs;
    ...
    if (c.in_eliminating)
    {
        ...
        if (winner.second.winner->getId() == node.getId())
        {
            ...
            if (!output_to_inputs.contains(header.name))
            {
                ...
                fk_name = context->getOptimizerContext()->getSymbolAllocator()->newSymbol(header.name);
        ...
        auto union_step = std::make_shared<UnionStepExt>(inputs, output_stream, output_to_inputs, step->getMaxThreads(), step->isLocal());
        ...
        auto current_union_node = UnionStepExtNode::createPlanNode(node.getId(), std::move(union_step), children);
...
PlanNodePtr EliminateJoinByFK::Eliminator::visitCTERefStepExtNode(CTERefStepExtNode & node, JoinEliminationContext & c)
{
    auto result = cte_helper.accept(node.getStep()->getId(), *this, c);
    ...
    if (c.in_eliminating)
    {
        ...
        auto new_cte_step = std::make_shared<CTERefStepExt>(
            DataStream{ToColumnsWithTypeAndName(result_columns)}, node.getStep()->getId(), output_columns, node.getStep()->hasFilter());
        return CTERefStepExtNode::createPlanNode(context->getOptimizerContext()->nextNodeId(), std::move(new_cte_step), {});
    }
    return node.shared_from_this();
}
