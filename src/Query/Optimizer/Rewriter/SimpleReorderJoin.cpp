#include <Query/Optimizer/Rewriter/SimpleReorderJoin.h>
...
namespace DB
{
bool SimpleReorderJoin::rewrite(QueryPlanExt & plan, ContextMutablePtr context) const
{
    if (context->getOptimizerContext()->getSettingsRef().heuristic_join_reorder_enumeration_times != 10)
        return false;
    if (!context->getOptimizerContext()->getSettingsRef().enable_join_reorder)
        return false;

    auto join_size = PlanPattern::maxJoinSize(plan, context);
    if (join_size <= context->getOptimizerContext()->getSettingsRef().max_graph_reorder_size)
        return false;
...
PlanNodePtr SimpleReorderJoinVisitor::visitJoinStepExtNode(JoinStepExtNode & node, Void & v)
{
    ...
    if (join_graph.size() < 2 || join_graph.size() <= context->getOptimizerContext()->getSettingsRef().max_graph_reorder_size)
        return visitPlanNode(node, v);
...
PlanNodePtr SimpleReorderJoinVisitor::getJoinOrder(JoinGraph & graph)
{
    ...
    for (const auto & item : graph.getEdges())
    {
        ...
        auto left_base_table = getQueryPlanStepType(left_node->getStep())== QueryPlanStepType::TableScanStepExt;
        ...
        for (const auto & edge : item.second)
        {
            ...
            auto right_base_table = getQueryPlanStepType(right_node->getStep())== QueryPlanStepType::TableScanStepExt;
    ...
    while (!selectivities.empty())
    {
        ...
        if (left_join_node_id != right_join_node_id)
        {
            ...
            auto new_join_node
                = PlanNodeBase::createPlanNode(context->getOptimizerContext()->nextNodeId(), std::move(new_join_step), PlanNodes{left_join_node, right_join_node});
    ...
    std::vector<String> & expected_output_symbols, JoinGraph & graph, PlanNodePtr join_node, ContextMutablePtr & context_ptr)
{
    ...
    if (!PredicateUtils::isTruePredicate(predicate))
    {
        ...
        result = std::make_shared<FilterStepExtNode>(context_ptr->getOptimizerContext()->nextNodeId(), std::move(filter_step), PlanNodes{result});
    }
    ...
    auto restricted_outputs_node
        = std::make_shared<ProjectionStepExtNode>(context_ptr->getOptimizerContext()->nextNodeId(), std::move(restricted_outputs_step), PlanNodes{result});
