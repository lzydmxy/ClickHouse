...
{
public:
    explicit DecorrelationVisitor(Names & correlation_) : correlation(correlation_) { }
    std::optional<DecorrelationResult> visitPlanNode(PlanNodeBase & node, Context & context) override;
    std::optional<DecorrelationResult> visitFilterStepExtNode(FilterStepExtNode & node, Context & context) override;
    std::optional<DecorrelationResult> visitProjectionStepExtNode(ProjectionStepExtNode & node, Context & context) override;
