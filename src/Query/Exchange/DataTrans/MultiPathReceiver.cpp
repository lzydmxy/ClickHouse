...
void MultiPathReceiver::registerToSendersAsync(UInt32 timeout_ms)
{
    ...
    else
    {
        ...
        if (!wait_register_cv.wait_for(lock, std::chrono::milliseconds(timeout_ms + 100), [&] { return inited.load(std::memory_order_acquire); }))
            throw Exception(ErrorCodes::EXCHANGE_DATA_TRANS_EXCEPTION, "Wait register timeout for {} for query {}", name, CurrentThread::getQueryId());
    }
}
...
void MultiPathReceiver::registerToSendersJoin()
{
    ...
    LOG_DEBUG(logger, "{} register to sender async successfully", name);
}
...
void MultiPathReceiver::registerToLocalSenders(UInt32 timeout_ms)
{
    ...
    LOG_DEBUG(logger, "{} register to local sender successfully", name);
}
...
void MultiPathReceiver::registerToSenders(UInt32 timeout_ms)
{
    ...
    if (registering.compare_exchange_strong(expected, true, std::memory_order_acq_rel, std::memory_order_acquire))
    {
        ...
        for (auto & res : async_results)
        {
            ...
            if (res.cntl->ErrorCode() == brpc::EREQUEST
                && boost::algorithm::ends_with(res.cntl->ErrorText(), "was closed before responded"))
            {
                LOG_INFO(logger,"Receiver register sender successfully but sender already finished, host: {} , request: {}",
                    butil::endpoint2str(res.cntl->remote_side()).c_str(), *res.request);
                continue;
            }
            res.channel->assertController(*res.cntl);
            LOG_TRACE(logger, "Receiver register sender successfully, host-{} , request: {}",
                butil::endpoint2str(res.cntl->remote_side()).c_str(), *res.request);
        }
        inited.store(true, std::memory_order_release);
        wait_register_cv.notify_all();
        LOG_DEBUG(logger, "{} register to sender successfully", name);
        if (enable_receiver_metrics)
            receiver_metrics.register_time_ms << register_s.elapsedMilliseconds();
    }
    else
    {
        std::unique_lock lock(wait_register_mutex);
        if (!wait_register_cv.wait_for(lock, std::chrono::milliseconds(timeout_ms + 100), [&] { return inited.load(std::memory_order_acquire); }))
            throw Exception(ErrorCodes::EXCHANGE_DATA_TRANS_EXCEPTION, "Wait register timeout for {}", name);
    }
}

RecvDataPacket MultiPathReceiver::recv(TimePoint timeout_tp)
{
    Stopwatch s;
    MultiPathDataPacket data_packet;
    if (!collector->tryPopUntil(data_packet, timeout_tp))
    {
        bool collector_closed = collector->closed();
        String error_msg = "Try pop receive collector for " + name;
        error_msg.append(
            collector_closed ? " interrupted" : " timeout at " + timeToString(timeout_tp));
    ...
    else
    {
        ...
        bool all_receiver_done = false;
        {
            ...
            if (unlikely(running_receiver_names.empty()))
                throw Exception(ErrorCodes::LOGICAL_ERROR, "{} receive unexpected sendDoneMark from  {}", name, receiver_name);
            else
            {
...
BroadcastStatus MultiPathReceiver::finish(BroadcastStatusCode status_code, String message)
{
    ...
    if (fin_status.compare_exchange_strong(current_status_ptr, new_status_ptr, std::memory_order_acq_rel, std::memory_order_acquire))
    {
        ...
        LOG_INFO(logger, "{} change finish status from {} to {} with message: {}, is_modifer: {}, subreceiver status: {}",
                getName(), current_status_ptr->code, status_code, message, is_modifer, old_status.code);
    ...
    else
    {
        ...
        LOG_TRACE(logger, "{} finished and can't change to status code {},  msg {} . Current status: {}, msg: {}",
                name, status_code, message, current_status_ptr->code, current_status_ptr->message);
