#include "LocalBroadcastChannel.h"
#include <optional>
...
RecvDataPacket LocalBroadcastChannel::recv(TimePoint timeout_tp)
{
    Stopwatch s;
    ...
    if (receive_queue->tryPopUntil(data_packet, timeout_tp))
    {
...
BroadcastStatus LocalBroadcastChannel::sendImpl(Chunk chunk)
{
    ...
    BroadcastStatus current_status = finish(BroadcastStatusCode::SEND_TIMEOUT,
        "Query send to local exchange channel " + name + " timeout after ms: " + timeToString(options.max_timeout_ts));
    return current_status;
}
...
{
    throw Exception(ErrorCodes::NOT_IMPLEMENTED, "merge is not implemented for LocalBroadcastChannel");
}
...
LocalBroadcastChannel::~LocalBroadcastChannel()
{
    ...
    try
    {
        ...
        if (status != &init_status)
            delete status;
        auto query_exchange_log = context ? context->getOptimizerContext()->getQueryExchangeLog() : nullptr;
