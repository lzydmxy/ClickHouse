...
DataTypePtr createBaseDataTypeFromTypeIndex(TypeIndex index)
{
    switch (index)
    {
        ...
        default:
            throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Cannot create data type in default");
    }
}
...
DataTypePtr deserializeDataTypeV1(ReadBuffer & buf)
{
    ...
    switch (index)
    {
        ...
        case TypeIndex::AggregateFunction: {
            ...
            auto func = factory.get(name, action, type_args, params, property);
...
void serializeDataTypeV1(const DataTypePtr & data_type, WriteBuffer & buf)
{
    ...
    #undef SERIALIZE_ENUM_TYPE
        ...
        default:
            throw Exception(ErrorCodes::NOT_IMPLEMENTED, "DataType {} cannot be serialized", data_type->getName());
    }
}
...
void serializeDataTypeToProto(const DataTypePtr & data_type, RDataType & proto)
{
    proto.set_name(data_type->getName());
}

DataTypePtr deserializeDataTypeFromProto(const RDataType & proto)
{
    const DataTypeFactory & data_type_factory = DataTypeFactory::instance();
    return data_type_factory.get(proto.name());
}
