...
public:
    ...
    PlanNodeAndMappings visitProjectionStepExtNode(ProjectionStepExtNode & project, Void & c) override
    {
    ...
    PlanNodeAndMappings visitJoinStepExtNode(JoinStepExtNode & join, Void & c) override
    {
        ...
        if (step->getKind() == JoinKind::Inner && step->getStrictness() != JoinStrictness::Asof)
        {
     ...
     * todo: lizhuoyu5 bc remove this restricts for Intersect/Except/IntersectOrExceptNode
     */
    PlanNodeAndMappings visitIntersectOrExceptStepNode(IntersectOrExceptStepNode & intersect, Void & c) override
    {
        ...
        if (!isCompatibleHeader(left.plan_node->getCurrentDataStream().header, right.plan_node->getCurrentDataStream().header))
        {
            ...
            if (!Utils::isIdentity(assignments))
            {
                auto projection_step
                    = std::make_shared<ProjectionStepExt>(right.plan_node->getStep()->getOutputStream(), assignments, name_to_type);
                right.plan_node = PlanNodeBase::createPlanNode(context->getOptimizerContext()->nextNodeId(), projection_step, {right.plan_node});
            }
        }
