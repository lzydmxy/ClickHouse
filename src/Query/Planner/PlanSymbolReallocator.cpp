...
PlanNodePtr PlanSymbolReallocator::unalias(const PlanNodePtr & plan, ContextMutablePtr & context)
{
    ...
    {
        auto projection_step = std::make_shared<ProjectionStepExt>(plan_node->getStep()->getOutputStream(), assignments, name_to_type);
        plan_node = PlanNodeBase::createPlanNode(context->getOptimizerContext()->nextNodeId(), projection_step, {plan_node});
    }
...
PlanNodeAndMappings PlanSymbolReallocator::reallocate(const PlanNodePtr & plan, ContextMutablePtr & context)
{
    ...
    SymbolReallocatorVisitor visitor{
        context, [&](auto & mapping) { return SymbolMapper::symbolReallocator(mapping, *context->getOptimizerContext()->getSymbolAllocator()); }};
    Void c;
    auto [plan_node, mappings] = VisitorUtil::accept(plan, visitor, c);
    auto symbol_mapper = SymbolMapper::symbolMapper(mappings);
