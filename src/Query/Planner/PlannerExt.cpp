...
public:
    ...
    RelationPlan visitASTSelectQueryExt(ASTPtr & node, const Void &) override;
...
private:
    ...
    /// plan FROM
    PlanBuilder planFrom(ASTSelectQueryExt &);
    PlanBuilder planWithoutTables(ASTSelectQueryExt & select_query);
    PlanBuilder planTables(ASTTablesInSelectQuery & tables_in_select, ASTSelectQueryExt & select_query);
    PlanBuilder planTableExpression(ASTTableExpression & table_expression, ASTSelectQueryExt & select_query);
    PlanBuilder planTable(ASTTableIdentifier & db_and_table, ASTSelectQueryExt & select_query);
    PlanBuilder planTableFunction(ASTFunction & table_function, ASTSelectQueryExt & select_query);
    ...
    RelationPlan planReadFromStorage(IAST & table_ast, ScopePtr table_scope, ASTSelectQueryExt & origin_query, bool is_table_function = false);

    void planArrayJoin(ASTArrayJoin & array_join, PlanBuilder & builder, ASTSelectQueryExt & select_query);
    void planFilter(PlanBuilder & builder, ASTSelectQueryExt & select_query, const ASTPtr & filter);
    void planAggregate(PlanBuilder & builder, ASTSelectQueryExt & select_query);
    void planWindow(PlanBuilder & builder, ASTSelectQueryExt & select_query);
    void planSelect(PlanBuilder & builder, ASTSelectQueryExt & select_query);
    void planDistinct(PlanBuilder & builder, ASTSelectQueryExt & select_query);
    void planOrderBy(PlanBuilder & builder, ASTSelectQueryExt & select_query);
    void planWithFill(PlanBuilder & builder, ASTSelectQueryExt & select_query);
    void planLimitBy(PlanBuilder & builder, ASTSelectQueryExt & select_query);
    void planTotalsAndHaving(PlanBuilder & builder, ASTSelectQueryExt & select_query);
    void planLimitAndOffset(PlanBuilder & builder, ASTSelectQueryExt & select_query);
    void planSampling(PlanBuilder & builder, ASTSelectQueryExt & select_query);

    RelationPlan planFinalSelect(PlanBuilder & builder, ASTSelectQueryExt & select_query);

    // the routine to plan expressions in most scenarios, which handle non-deterministic function & subqueries within expressions
    template <typename T>
    void planExpression(PlanBuilder & builder, ASTSelectQueryExt & select_query, const T & expressions);
    template <typename T>
    void planNonDeterministicFunction(PlanBuilder & builder, ASTSelectQueryExt & select_query, const T & expressions);
    ...
    template <typename T>
    PlanNodes planSubqueryExpression(PlanBuilder & builder, ASTSelectQueryExt & select_query, const T & expressions);
    void planScalarSubquery(PlanBuilder & builder, const ASTPtr & scalar_subquery);
    void planInSubquery(PlanBuilder & builder, const ASTPtr & node, ASTSelectQueryExt & select_query);
    void planExistsSubquery(PlanBuilder & builder, const ASTPtr & node);
    void planQuantifiedComparisonSubquery(PlanBuilder & builder, const ASTPtr & node, ASTSelectQueryExt & select_query);
    ...
    RelationPlan planSetOperation(ASTs & selects, SelectUnionMode union_mode);
    ...
    std::pair<UInt64, UInt64> getLimitLengthAndOffset(ASTSelectQueryExt & query);
    ...
    bool needAggregateOverflowRow(ASTSelectQueryExt & select_query) const;
};
...
namespace
{
    ...
    PlanNodePtr planOutput(RelationPlan & plan, ASTPtr & query, Analysis & analysis, ContextMutablePtr context)
    {
        ...
        if (context->getOptimizerContext()->getSettingsRef().final_order_by_all_direction || context->getSettingsRef().limit || context->getSettingsRef().offset)
        {
            // Ensure the order of output columns
            if (context->getOptimizerContext()->getSettingsRef().final_order_by_all_direction != 0)
            {
                auto output_step = std::make_shared<ProjectionStepExt>(old_root->getCurrentDataStream(), assignments, output_types, false);
                old_root = old_root->addStep(context->getOptimizerContext()->nextNodeId(), std::move(output_step), {});
            }
    ...
    void planExtremes(RelationPlan & plan, ContextMutablePtr context)
    {
        if (context->getSettingsRef().extremes)
        {
            ...
            auto extremes = plan.getRoot()->addStep(context->getOptimizerContext()->nextNodeId(), std::move(extremes_step), {});
    ...
    void planFinalResult(RelationPlan & plan, ContextMutablePtr context)
    {
        ...
        if (context->getSettingsRef().limit > 0)
        {
            ...
            auto limit_step = std::make_shared<LimitStepExt>(plan.getRoot()->getCurrentDataStream(), limit_length, limit_offset);
            auto limit_root = plan.getRoot()->addStep(context->getOptimizerContext()->nextNodeId(), std::move(limit_step), {});
        ...
        else if (context->getSettingsRef().offset > 0)
        {
            ...
            auto offset_root = plan.getRoot()->addStep(context->getOptimizerContext()->nextNodeId(), std::move(offset_step), {});
            plan.withNewRoot(offset_root);
        }
    }
}

QueryPlanExtPtr PlannerExt::plan(ASTPtr & query, Analysis & analysis, ContextMutablePtr context)
{
    CTERelationPlans cte_plans;
    ...
    return std::make_unique<QueryPlanExt>(plan_root, cte_info, context->getOptimizerContext()->getPlanNodeIdAllocator());
}

RelationPlan PlannerExt::planQuery(
...
RelationPlan QueryPlannerVisitor::visitASTSelectIntersectExceptQuery(ASTPtr & node, const Void &)
{
    ...
    auto operator_to_union_mode = [](ASTSelectIntersectExceptQuery::Operator op) -> SelectUnionMode {
        switch (op)
        {
            ...
            default:
                throw Exception(ErrorCodes::LOGICAL_ERROR, "Unrecognized set operator found");
        }
    };
...
RelationPlan QueryPlannerVisitor::visitASTSelectQueryExt(ASTPtr & node, const Void &)
{
...
RelationPlan QueryPlannerVisitor::visitASTSubquery(ASTPtr & node, const Void &)
{
    ...
    {
        if (cte_analysis->isSharable() || context->getOptimizerContext()->getSettingsRef().cte_mode == CTEMode::ENFORCED)
        {
            ...
            {
                auto new_name = context->getOptimizerContext()->getSymbolAllocator()->newSymbol(name_and_type.name);
            ...
            ColumnsWithTypeAndName name_with_type_and_name;
            for (const auto & item : mapped_name_and_types)
            {
                name_with_type_and_name.emplace_back(item.type, item.name);
            }
...
PlanBuilder QueryPlannerVisitor::planWithoutTables(ASTSelectQueryExt & select_query)
{
    PlanNodePtr node;
...

    {
        ...
        auto values_step = std::make_shared<ValuesStepExt>(name_with_type_and_name, data);
        node = PlanNodeBase::createPlanNode(context->getOptimizerContext()->nextNodeId(), values_step);
    }
...
PlanBuilder QueryPlannerVisitor::planTables(ASTTablesInSelectQuery & tables_in_select, ASTSelectQueryExt & select_query)
{
    ...
    for (size_t idx = 1; idx < tables_in_select.children.size(); ++idx)
    {
        ...
        if (table_element.table_expression)
        {
            ...
            if (!table_expression)
                throw Exception(ErrorCodes::LOGICAL_ERROR, "Invalid Table Expression");
...
PlanBuilder QueryPlannerVisitor::planTableExpression(ASTTableExpression & table_expression, ASTSelectQueryExt & select_query)
{
...
PlanBuilder QueryPlannerVisitor::planTable(ASTTableIdentifier & db_and_table, ASTSelectQueryExt & select_query)
{
    ...
    if (auto & alias_columns = analysis.getTableAliasColumns(db_and_table); !alias_columns.empty())
    {
        ...
        {
            auto alias_symbol = context->getOptimizerContext()->getSymbolAllocator()->newSymbol(alias_column->tryGetAlias());
...
PlanBuilder QueryPlannerVisitor::planTableFunction(ASTFunction & table_function, ASTSelectQueryExt & select_query)
{
...
void QueryPlannerVisitor::planCrossJoin(ASTTableJoin & table_join, PlanBuilder & left_builder, PlanBuilder & right_builder)
{
    // build join node
    {
        ...
        auto join_step = std::make_shared<JoinStepExt>(
            ...
            DataStream{.header = name_with_type_and_name},
            JoinKind::Cross,
...
{
    if (use_ansi_semantic && table_join.kind != JoinKind::Inner)
    {
...
void QueryPlannerVisitor::planJoinOn(ASTTableJoin & table_join, PlanBuilder & left_builder, PlanBuilder & right_builder)
{
    ...
    {
        auto filter_step = std::make_shared<FilterStepExt>(left_builder.getCurrentDataStream(), join_filter);
...
RelationPlan
QueryPlannerVisitor::planReadFromStorage(IAST & table_ast, ScopePtr table_scope, ASTSelectQueryExt & origin_query, bool is_table_function)
{
    ...
    for (size_t i = 0; i < primary_column_size; ++i)
    {
        ...
        if (used_columns.count(i) || (i < used_sub_columns.size() && !used_sub_columns[i].empty()))
        {
            ...
            {
                primary_column_symbol = context->getOptimizerContext()->getSymbolAllocator()->newSymbol(primary_column_name);
        ...
        if (i < used_sub_columns.size())
        {
            for (const auto & sub_column_id : used_sub_columns[i])
            {
                ...
                {
                    auto sub_column_symbol = context->getOptimizerContext()->getSymbolAllocator()->newSymbol(sub_column_name);
    ...
    auto rewritten_prewhere = analysis.tryGetPreWhere(origin_query);
    ...
    */
    ...
    auto table_step
        = std::make_shared<TableScanStepExt>(context, storage->getStorageID(), columns_with_aliases, query_info, max_block_size, alias);

    auto plan_node = PlanNodeBase::createPlanNode(context->getOptimizerContext()->nextNodeId(), table_step);
    if (rewritten_prewhere)
        plan_node = PlanNodeBase::createPlanNode(
            context->getOptimizerContext()->nextNodeId(), std::make_shared<FilterStepExt>(plan_node->getCurrentDataStream(), rewritten_prewhere, true), {plan_node});
    return {plan_node, field_symbols};
}


PlanBuilder QueryPlannerVisitor::planFrom(ASTSelectQueryExt & select_query)
{
...
void QueryPlannerVisitor::planArrayJoin(ASTArrayJoin & array_join, PlanBuilder & builder, ASTSelectQueryExt & select_query)
{
...
void QueryPlannerVisitor::planFilter(PlanBuilder & builder, ASTSelectQueryExt & select_query, const ASTPtr & filter)
{
    ...
    auto filter_step = std::make_shared<FilterStepExt>(builder.getCurrentDataStream(), builder.translate(filter));
    for (const auto & scalar : scalar_apply)
    {
        auto * apply_step = dynamic_cast<ApplyStepExt *>(scalar->getStep().get());
...
void QueryPlannerVisitor::planAggregate(PlanBuilder & builder, ASTSelectQueryExt & select_query)
{
    if (!analysis.needAggregate(select_query))
    {
        if (select_query.group_by_with_totals || select_query.group_by_with_rollup || select_query.group_by_with_cube)
            throw Exception(ErrorCodes::NOT_IMPLEMENTED, "WITH TOTALS, ROLLUP or CUBE are not supported without aggregation");
        return;
    }
    ...
    for (auto & agg_item : uniq_aggs)
    {
        ...
        agg_desc.column_name = context->getOptimizerContext()->getSymbolAllocator()->newSymbol(agg_item.expression);
    ...
    if (!context->getOptimizerContext()->getSettingsRef().only_full_group_by)
    {
        ...
        for (size_t i = 0; i < field_symbols_before_group.size(); ++i)
        {
            ...
            if (!field_symbol.empty() && visible_fields.at(i).primary_symbol.empty() && name_to_types_before_group_by.contains(field_symbol))
            {
                ...
                AggregateFunctionPtr any_agg_fun = AggregateFunctionFactory::instance().get("any", NullsAction::EMPTY, data_types, parameters, properties);
                AggregateDescription any_agg_func_desc;
                any_agg_func_desc.function = any_agg_fun;
                auto any_column_name = context->getOptimizerContext()->getSymbolAllocator()->newSymbol(field_symbol);
    ...
    if (select_query.group_by_with_totals)
    {
        ...
        Aggregator::Params params(keys_for_all_group, aggregate_descriptions, needAggregateOverflowRow(select_query), settings.max_threads, settings.max_block_size, settings.min_hit_rate_to_use_consecutive_keys_optimization);
...
void QueryPlannerVisitor::planTotalsAndHaving(PlanBuilder & builder, ASTSelectQueryExt & select_query)
{
    const auto & settings = context->getSettingsRef();
    AggregateDescriptions aggregate_descriptions;

    auto totals_having_step = std::make_shared<TotalsHavingStepExt>(
        builder.getCurrentDataStream(),
        aggregate_descriptions,
        needAggregateOverflowRow(select_query),
...
void QueryPlannerVisitor::planWindow(PlanBuilder & builder, ASTSelectQueryExt & select_query)
{
    ...
    for (auto & window_item : uniq_windows)
    {
        ...
        window_function.column_name = context->getOptimizerContext()->getSymbolAllocator()->newSymbol(window_item->expression);
...
void QueryPlannerVisitor::planSelect(PlanBuilder & builder, ASTSelectQueryExt & select_query)
{
    auto & select_expressions = analysis.getSelectExpressions(select_query);
    planExpression(builder, select_query, select_expressions);
}

void QueryPlannerVisitor::planDistinct(PlanBuilder & builder, ASTSelectQueryExt & select_query)
{
...
void QueryPlannerVisitor::planOrderBy(PlanBuilder & builder, ASTSelectQueryExt & select_query)
{
    ...
    for (auto & order_by_item : order_by_analysis)
    {
        ...
        if (order_by_item->getCollation())
            collator = std::make_shared<Collator>(order_by_item->getCollation()->as<ASTLiteral &>().value.get<String>());
    ...
    auto sorting_step = std::make_shared<SortingStep>(
        builder.getCurrentDataStream(), sort_description, limit, context->getSettingsRef().optimize_sorting_by_input_stream_properties);
    builder.addStep(std::move(sorting_step));
    // PRINT_PLAN(builder.plan, plan_order_by);
}

void QueryPlannerVisitor::planWithFill(PlanBuilder & builder, ASTSelectQueryExt & select_query)
{
    if (!select_query.orderBy())
        return;
    ...
    for (auto & order_by_item : analysis.getOrderByAnalysis(select_query))
    {
        ...
        FillColumnDescription fill_desc = getWithFillDescription(*order_by_item, context);

        SortColumnDescription sort_column_description(sort_symbol, order_by_item->direction, order_by_item->nulls_direction, collator, true, fill_desc);

        if (order_by_item->with_fill)
        {
            fill_description.emplace_back(sort_column_description);
        }

        sort_description.emplace_back(sort_column_description);
    }
    ...
    auto filling_step = std::make_shared<FillingStep>(builder.getCurrentDataStream(), std::move(sort_description),
...
void QueryPlannerVisitor::planLimitBy(PlanBuilder & builder, ASTSelectQueryExt & select_query)
{
    if (!select_query.limitBy())
        return;

    // project limit by keys
    auto & tmp_limit_by_expressions = analysis.getLimitByItem(select_query);

    ASTs limit_by_expressions;
    for (auto & limit_by_expression : tmp_limit_by_expressions)
    {
        limit_by_expressions.emplace_back(limit_by_expression);
    }
...
RelationPlan QueryPlannerVisitor::projectFieldSymbols(const RelationPlan & plan, const FieldSubColumnIDs & sub_column_positions)
{
    ...
    for (const auto & field_sub_col_id : sub_column_positions)
    {
        ...
        if (!sub_col_sym)
            throw Exception(ErrorCodes::LOGICAL_ERROR, "Sub column not found in union element.");
    ...
    auto project_step = std::make_shared<ProjectionStepExt>(old_root->getCurrentDataStream(), assignments, output_types);
    new_root = old_root->addStep(context->getOptimizerContext()->nextNodeId(), std::move(project_step), {});

    return {new_root, std::move(new_mappings)};
}

static bool hasWithTotalsInAnySubqueryInFromClause(const ASTSelectQueryExt & query)
{
        ...
        if (const auto * ast_union = query_table->as<ASTSelectWithUnionQuery>())
        {
            ...
            */
            std::function<bool(ASTPtr)> traverse_recursively = [&](ASTPtr child_ast) -> bool {
                if (const auto * select_child = child_ast->as<ASTSelectQueryExt>())
                {
                    if (hasWithTotalsInAnySubqueryInFromClause(select_child->as<ASTSelectQueryExt &>()))
                        return true;
                }
...
void QueryPlannerVisitor::planLimitAndOffset(PlanBuilder & builder, ASTSelectQueryExt & select_query)
{
...
void QueryPlannerVisitor::planSampling(PlanBuilder & builder, ASTSelectQueryExt & select_query)
{
    if (select_query.sampleSize() && context->getOptimizerContext()->getSettingsRef().enable_final_sample)
    {
        ...
        {
            auto sampling = std::make_unique<FinalSampleStepExt>(builder.getCurrentDataStream(), numerator, context->getSettingsRef().max_block_size);
            builder.addStep(std::move(sampling));
        }
    }
}

RelationPlan QueryPlannerVisitor::planFinalSelect(PlanBuilder & builder, ASTSelectQueryExt & select_query)
{
    ...
    auto select_column = [&](const auto & input_symbol) {
        ...
        if (existing_symbols.find(output_symbol) != existing_symbols.end())
            output_symbol = context->getOptimizerContext()->getSymbolAllocator()->newSymbol(output_symbol);

        assignments.emplace_back(output_symbol, toSymbolRef(input_symbol));
        if (!input_types.contains(input_symbol))
            throw Exception(ErrorCodes::PLAN_BUILD_ERROR, "Unknown column {}", input_symbol);
    ...
    auto project = std::make_shared<ProjectionStepExt>(builder.getCurrentDataStream(), assignments, output_types);
...
namespace
{
    ...
    protected:
        ...
        void visitQuantifiedComparisonSubquery(ASTPtr & node, ASTQuantifiedComparisonExt &, const Void &) override
        {
    ...
    protected:
        ...
        {
            if (context->getOptimizerContext()->isNonDeterministicFunction(func.name))
...
template <typename T>
PlanNodes QueryPlannerVisitor::planSubqueryExpression(PlanBuilder & builder, ASTSelectQueryExt & select_query, const T & expressions)
{
...
void QueryPlannerVisitor::planScalarSubquery(PlanBuilder & builder, const ASTPtr & scalar_subquery)
{
    ...
    auto subquery_plan = PlannerExt().planQuery(scalar_subquery, builder.translation, analysis, context, cte_plans);
    ...
    auto enforce_single_row_step = std::make_shared<EnforceSingleRowStepExt>(subquery_plan.getRoot()->getCurrentDataStream());
    auto enforce_single_row_node = subquery_plan.getRoot()->addStep(context->getOptimizerContext()->nextNodeId(), std::move(enforce_single_row_step), {});
    ...
    auto apply_step = std::make_shared<ApplyStepExt>(
        ...
        ApplyStepExt::ApplyType::CROSS,
        ApplyStepExt::SubqueryType::SCALAR,
        scalar_assignment,
        NameSet{},
...
void QueryPlannerVisitor::planInSubquery(PlanBuilder & builder, const ASTPtr & node, ASTSelectQueryExt & select_query)
{
    // filter out planned subqueries
    ...
    if (function.name == "nullIn" || function.name == "globalNullIn" || function.name == "notNullIn" || function.name == "globalNotNullIn")
        throw Exception(
            ErrorCodes::NOT_IMPLEMENTED, "nullIn,globalNullIn,notNullIn and globalNotNullIn are not implemented, when optimizer is opened");
    ...
    // Add Apply Step
    String apply_output_symbol = context->getOptimizerContext()->getSymbolAllocator()->newSymbol("_in_subquery");
...
void QueryPlannerVisitor::planExistsSubquery(PlanBuilder & builder, const ASTPtr & node)
{
    ...
    auto subquery_plan
        = PlannerExt().planQuery(exists_subquery.arguments->children.at(0), builder.translation, analysis, context, cte_plans);
    // Add Projection Step
    {
        ...
        auto expression_step = std::make_shared<ProjectionStepExt>(subquery_plan.getRoot()->getCurrentDataStream(), assignments, types);
        auto expression_node = subquery_plan.getRoot()->addStep(context->getOptimizerContext()->nextNodeId(), std::move(expression_step), {});
        subquery_plan = {expression_node, FieldSymbolInfos{{output_name}}};
    }

    // Add Apply Step
    String apply_output_symbol = context->getOptimizerContext()->getSymbolAllocator()->newSymbol(is_not_exist ? "_not_exists_subquery" : "_exists_subquery");
    ...
    auto apply_step = std::make_shared<ApplyStepExt>(
        ...
        builder.getOutputNames(),
        ApplyStepExt::ApplyType::CROSS,
        ApplyStepExt::SubqueryType::EXISTS,
        exist_assignment,
        NameSet{},
        analysis.subquery_support_semi_anti[node]);
...
void QueryPlannerVisitor::planQuantifiedComparisonSubquery(PlanBuilder & builder, const ASTPtr & node, ASTSelectQueryExt & select_query)
{
    if (builder.canTranslateToSymbol(node))
        return;

    auto & quantified_comparison = node->as<ASTQuantifiedComparisonExt &>();
    ...
    std::shared_ptr<ApplyStepExt> apply_step;
    // A = ANY B <=> A IN B
    // A <> ALL B <=> (A notIn B)
    ...
    {
        apply_output_symbol = context->getOptimizerContext()->getSymbolAllocator()->newSymbol("_quantified_comparison_subquery");
        ...
        apply_step = std::make_shared<ApplyStepExt>(
            DataStreams{builder.getCurrentDataStream(), rhs_plan.getRoot()->getCurrentDataStream()},
            builder.getOutputNames(),
            ApplyStepExt::ApplyType::CROSS,
            ApplyStepExt::SubqueryType::QUANTIFIED_COMPARISON,
    ...
    else
    {
        ...
        if (!comparator_is_equals)
            function_name = "notIn";
        apply_output_symbol = context->getOptimizerContext()->getSymbolAllocator()->newSymbol("_in_subquery");
...
RelationPlan QueryPlannerVisitor::combineSubqueryOutputsToTuple(const RelationPlan & plan, const ASTPtr & subquery)
{
    ...
    if (outputs.size() > 1)
    {
        ...
        auto tuple_func_symbol = context->getOptimizerContext()->getSymbolAllocator()->newSymbol(tuple_func_expr);
        Assignments assignments{{tuple_func_symbol, tuple_func_expr}};
        NameToType types{{tuple_func_symbol, subquery_type}};
        auto old_root = plan.getRoot();
        auto expression_step = std::make_shared<ProjectionStepExt>(old_root->getCurrentDataStream(), assignments, types);
        auto new_root = old_root->addStep(context->getOptimizerContext()->nextNodeId(), std::move(expression_step), {});
...
RelationPlan QueryPlannerVisitor::planSetOperation(ASTs & selects, SelectUnionMode union_mode)
{
    ...
    for (size_t i = 0; i < field_symbol_info.size(); ++i)
    {
        ...
        auto new_name = context->getOptimizerContext()->getSymbolAllocator()->newSymbol(col.name);
    ...
    auto set_operation_node = sub_plans[0].getRoot()->addStep(context->getOptimizerContext()->nextNodeId(), std::move(set_operation_step), source_nodes);
...
QueryPlannerVisitor::coerceTypesForSymbols(const PlanNodePtr & node, const NameToType & symbol_and_types, bool replace_symbol)
{
    ...
    for (const auto & input_symbol_and_type : node->getCurrentDataStream().header)
    {
        ...
        if (auto it = symbol_and_types.find(input_symbol); it != symbol_and_types.end() && it->second)
        {
            ...
            String output_symbol = context->getOptimizerContext()->getSymbolAllocator()->newSymbol(input_symbol);
    ...
    auto casting_step = std::make_shared<ProjectionStepExt>(node->getCurrentDataStream(), assignments, output_types);
    auto casting_node = node->addStep(context->getOptimizerContext()->nextNodeId(), std::move(casting_step), {});
...
PlanBuilder QueryPlannerVisitor::toPlanBuilder(const RelationPlan & plan, ScopePtr scope)
{
    ...
    return {analysis, context->getOptimizerContext()->getPlanNodeIdAllocator(), context->getOptimizerContext()->getSymbolAllocator(), plan.root, translation_map};
}
std::pair<UInt64, UInt64> QueryPlannerVisitor::getLimitLengthAndOffset(ASTSelectQueryExt & query)
{
    UInt64 length = 0;
    UInt64 offset = 0;
    ...
    ASTSelectQueryExt & select_query)
{
    ...
    rhs_plan = PlannerExt().planQuery(rhs_ast, builder.translation, analysis, context, cte_plans);
...
bool QueryPlannerVisitor::needAggregateOverflowRow(ASTSelectQueryExt & select_query) const
{
    const auto & settings = context->getSettingsRef();
...
template <typename T>
void QueryPlannerVisitor::planExpression(PlanBuilder & builder, ASTSelectQueryExt & select_query, const T & expressions)
{
...
template <typename T>
void QueryPlannerVisitor::planNonDeterministicFunction(PlanBuilder & builder, ASTSelectQueryExt & select_query, const T & expressions)
{
    if constexpr (std::is_same_v<T, ASTPtr>)
    {
