...
public:
    ...
    ColumnWithTypeAndName visitASTFieldReferenceExt(ASTPtr & node, AnalyzeContext &) override;
...
private:
    ...
    [[maybe_unused]] const bool enable_implicit_arg_type_convert;
    ...
    void enterLambda(ScopePtr lambda_scope)
    {
        if (!lambda_scope || lambda_scope->getType() != Scope::ScopeType::LAMBDA)
            throw Exception(ErrorCodes::LOGICAL_ERROR, "Provided scope is not a lambda scope");
    ...
    {
        if (scopes.size() <= 1)
            throw Exception(ErrorCodes::LOGICAL_ERROR, "No outer scope.");

        if (currentScope()->getType() != Scope::ScopeType::LAMBDA)
            throw Exception(ErrorCodes::LOGICAL_ERROR, "Current scope is not a lambda scope");
...
ColumnWithTypeAndName ExprAnalyzerVisitor::process(ASTPtr & node, AnalyzeContext & ac)
{
    ...
    if (!result.type)
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Can not determine expression type: {}", serializeAST(*node));
...
ColumnWithTypeAndName ExprAnalyzerVisitor::visitASTIdentifier(ASTPtr & node, AnalyzeContext &)
{
    ...
    if (!resolved)
        throw Exception(ErrorCodes::UNKNOWN_IDENTIFIER, "Can not resolve identifier: {}", identifier.name());

    return handleResolvedField(node, *resolved);
}

ColumnWithTypeAndName ExprAnalyzerVisitor::visitASTFieldReferenceExt(ASTPtr & node, AnalyzeContext &)
{
    // FieldReference is only used to refer to a relation field.
    auto & field_ref = node->as<ASTFieldReferenceExt &>();

    if (field_ref.field_index >= baseScope()->size())
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Illegal field reference node");
...
ColumnWithTypeAndName ExprAnalyzerVisitor::visitASTFunction(ASTPtr & node, AnalyzeContext & ac)
{
    ...
    Exception ex(ErrorCodes::UNKNOWN_FUNCTION, "Unknown function {}", function_ptr->name);
...
ColumnWithTypeAndName ExprAnalyzerVisitor::visitASTQuantifiedComparisonExt(ASTPtr & node, AnalyzeContext & ac)
{
    auto quantified_comparison = std::dynamic_pointer_cast<ASTQuantifiedComparisonExt>(node);
...
ColumnWithTypeAndName ExprAnalyzerVisitor::visitASTSubquery(ASTPtr & node, AnalyzeContext & ac)
{
    ...
    bool early_execute_subquery
        = context->getOptimizerContext()->getSettingsRef().early_execute_scalar_subquery
        || (isInLambda() && context->getOptimizerContext()->getSettingsRef().execute_subquery_in_lambda);
    ...
    if (isInLambda())
        throw Exception(ErrorCodes::SYNTAX_ERROR, "Subquery in lambda is not supported by ApplyStep");
...
ColumnWithTypeAndName ExprAnalyzerVisitor::analyzeOrdinaryFunction(ASTFunctionPtr & function, AnalyzeContext & ac)
{
    ...
    if (!function_base->isDeterministicInScopeOfQuery())
    {
        ...
        context->getOptimizerContext()->addNonDeterministicFunction(function->name, true);
    } else if (!function_base->isDeterministic())
    {
        context->getOptimizerContext()->addNonDeterministicFunction(function->name, false);
    }
...
ColumnWithTypeAndName ExprAnalyzerVisitor::analyzeAggregateFunction(ASTFunctionPtr & function)
{
    if (options.aggregate_support == ExprAnalyzerOptions::AggregateSupport::DISALLOWED)
        throw Exception(ErrorCodes::SYNTAX_ERROR, "Aggregate function is not supported in {}", options.statement_name);

    if (in_aggregate)
        throw Exception(ErrorCodes::ILLEGAL_AGGREGATION, "Nested aggregate function is not supported");
    ...
    in_aggregate = false;
    return {nullptr, aggregate_analysis.function->getResultType(), column_name};
}
...
ColumnWithTypeAndName ExprAnalyzerVisitor::analyzeWindowFunction(ASTFunctionPtr & function)
{
    if (options.window_support == ExprAnalyzerOptions::WindowSupport::DISALLOWED)
        throw Exception(ErrorCodes::SYNTAX_ERROR, "Window function is not supported in {}", options.statement_name);

    if (!options.select_query)
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Provide query node if window function is allowed");

    if (in_window)
        throw Exception(ErrorCodes::SYNTAX_ERROR, "Nested window function is not supported");

    if (in_aggregate)
        throw Exception(ErrorCodes::SYNTAX_ERROR, "Window function under an aggregate function is not supported");
    ...
    in_window = false;
    return {nullptr, aggregator->getResultType(), column_name};
}

ColumnWithTypeAndName ExprAnalyzerVisitor::analyzeGroupingOperation(ASTFunctionPtr & function)
{
    if (options.aggregate_support == ExprAnalyzerOptions::AggregateSupport::DISALLOWED)
        throw Exception(ErrorCodes::SYNTAX_ERROR, "Grouping operation is not supported in {}", options.statement_name);

    if (!options.select_query)
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Provide query node if grouping operation is allowed");

    if (in_aggregate)
        throw Exception(ErrorCodes::SYNTAX_ERROR, "Nested aggregate function is not supported");
...
ColumnWithTypeAndName ExprAnalyzerVisitor::analyzeExistsSubquery(ASTFunctionPtr & function, AnalyzeContext & ac)
{
    if (function->children.size() != 1)
        throw Exception(ErrorCodes::SYNTAX_ERROR, "Invalid exists subquery expression: {}", serializeAST(*function));
...
void ExprAnalyzerVisitor::processSubqueryArgsWithCoercion(ASTPtr & lhs_ast, ASTPtr & rhs_ast)
{
    ...
    bool early_execute_subquery
        = context->getOptimizerContext()->getSettingsRef().early_execute_in_subquery
        || (isInLambda() && context->getOptimizerContext()->getSettingsRef().execute_subquery_in_lambda);
    ...
    if (!JoinCommon::isJoinCompatibleTypes(lhs_type, rhs_type))
    {
        ...
        {
            if (context->getOptimizerContext()->getSettingsRef().convert_to_right_type_for_in_subquery)
            {
    ...
    for (auto & node : nodes)
    {
        if (auto * asterisk = node->as<ASTAsterisk>())
        {
            ...
            for (size_t field_index = 0; field_index < baseScope()->size(); ++field_index)
            {
                ...
                {
                    auto field_reference = std::make_shared<ASTFieldReferenceExt>(field_index);
...
std::tuple<AggregateFunctionPtr, Array, String> ExprAnalyzerVisitor::resolveAggregateFunction(ASTFunction & function)
{
    ...
    auto aggregate = AggregateFunctionFactory::instance().get(function.name, NullsAction::EMPTY, argument_types, parameters, properties);
...
DataTypePtr ExprAnalyzerVisitor::handleSubquery(const ASTPtr & subquery, bool use_explicit_named_tuple)
{
    if (auto * s = subquery->as<ASTSubquery>(); !s)
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Invalid subquery expression");

    if (options.subquery_support == ExprAnalyzerOptions::SubquerySupport::DISALLOWED)
        throw Exception(ErrorCodes::SYNTAX_ERROR, "Subquery is not supported in {}", options.statement_name);

    if (!options.select_query)
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Provide query node if subquery is allowed");
    ...
    else
    {
        ...
        auto checkTupleNames = [](const Strings& names) -> std::optional<Exception> {
            ...
            for (const auto& name : names) {
                ...
                if (!names_set.insert(name).second) {
                    return Exception(ErrorCodes::DUPLICATE_COLUMN,
                                   "Names of tuple elements must be unique. Duplicate name: {}", name);
                }
            }
            return {};
        };
