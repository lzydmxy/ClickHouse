...
#include <Query/Analyzer/tryEvaluateConstantExpression.h>
...
private:
    ...
    [[maybe_unused]] const bool allow_extended_conversion;
    const bool enable_subcolumn_optimization_through_union;
    [[maybe_unused]] const bool enable_implicit_arg_type_convert; // MySQL implicit cast rules
...
Void QueryAnalyzerVisitor::visitASTSelectWithUnionQuery(ASTPtr & node, const Void &)
{
    ...
    bool normalized = (select_with_union.is_normalized
                       && (select_with_union.union_mode == SelectUnionMode::UNION_ALL
                           || select_with_union.union_mode == SelectUnionMode::UNION_DISTINCT))
        || select_with_union.list_of_selects->children.size() == 1;
    if (!normalized)
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Invalid ASTSelectWithUnionQuery found");
...
Void QueryAnalyzerVisitor::visitASTSubquery(ASTPtr & node, const Void &)
{
    ...
    if (enable_shared_cte && !subquery.cte_name.empty())
    {
...
Void QueryAnalyzerVisitor::visitASTExplainQueryExt(ASTPtr & node, const Void &)
{
    ASTExplainQueryExt & explain = node->as<ASTExplainQueryExt &>();
    if (explain.getKind() != ASTExplainQueryExt::ExplainKindExt::LogicalAnalyze
        && explain.getKind() != ASTExplainQueryExt::ExplainKindExt::DistributedAnalyze
        && explain.getKind() != ASTExplainQueryExt::ExplainKindExt::PipelineAnalyze)
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Unexpected explain kind");
    if (!explain.getExplainedQuery())
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Explain query has syntax error ");

    process(const_cast<ASTPtr &>(explain.getExplainedQuery()));
    ...
    for (auto & select : selects)
    {
        ...
        if (column_size == 0)
            ...
            throw Exception(
                ...
                "Different number of columns for UNION ALL elements: {}",
                serializeAST(*node));
    }
            ...
            for (size_t column_idx = 0; column_idx < column_size; ++column_idx)
            {
                ...
                if (!input_type->equals(*output_type))
                {
                    ...
                    else
                    {
                        throw Exception(
                            ErrorCodes::UNION_ALL_RESULT_STRUCTURES_MISMATCH,
                            "Column type mismatch for UNION: {}, expect type: {}, but found type: {}",
                            serializeAST(*node),
                            output_type->getName(),
                            input_type->getName());
                    }
...
QueryAnalyzerVisitor::analyzeFrom(ASTTablesInSelectQuery & tables_in_select, ASTSelectQuery & select_query, Aliases & query_aliases)
{
    ...
    auto analyze_first_table_element = [&](ASTPtr & ast) -> ScopePtr {
        ...
        {
            throw Exception(ErrorCodes::LOGICAL_ERROR, "Invalid table table_element.");
        }
    };
    ...
    ASTTableIdentifier & db_and_table, const QualifiedName & column_prefix, ASTSelectQuery & select_query, Aliases & query_aliases)
{
...

    {
        ...
        if (!storageSupportOptimizer(storage))
            throw Exception(ErrorCodes::NOT_IMPLEMENTED, "{} table does not support optimizer", storage->getName());
    ...
    // get columns
    {
        ...
        for (const auto & column : storage->getVirtualsList())
        {
        ...
        if (storage->supportsSubcolumns())
        {
            for (const auto & column : getSubcolumnsOfAllPhysical(columns_description))
            {
        ...
        if (storage->supportsDynamicSubcolumns())
        {
            for (const auto & column : getSubcolumnsOfObjectColumns(storage_snapshot))
            {
    ...
    // get alias columns
    {
        ...
        for (const auto & column : columns_description.getAliases())
        {
            ...
            {
                throw Exception(ErrorCodes::LOGICAL_ERROR, "Alias column {} not found for table {}",column.name, full_table_name);
            }
        }
    ...
    ASTSelectQuery & select_query)
{
    // set join strictness if unspecified
    {
        ...
        if (table_join.strictness == JoinStrictness::Unspecified && !isCrossJoin(table_join))
        {
...

    {
        if (isCrossJoin(table_join))
        {
            if (table_join.strictness != JoinStrictness::Unspecified)
                throw Exception(ErrorCodes::UNKNOWN_JOIN, "CROSS/COMMA join must leave join strictness unspecified.");

            if (joinCondition(table_join))
                throw Exception(ErrorCodes::UNKNOWN_JOIN, "CROSS/COMMA join must not have join conditions.");
        }
        else if (isSemiOrAntiJoin(table_join))
        {
            if (!isInner(table_join.kind) && !isLeft(table_join.kind) && !isRight(table_join.kind))
                throw Exception(ErrorCodes::UNKNOWN_JOIN, "Illegal join kind for SEMI/ANTI join.");
        }
        else if (isAsofJoin(table_join))
        {
            if (!(isInner(table_join.kind) || isLeft(table_join.kind)))
            {
                throw Exception(ErrorCodes::UNKNOWN_JOIN, "Join kind for Asof join must be either Inner or Left");
            }
        }
    }
        ...
        if (isAsofJoin(table_join))
        {
            if (table_join.using_expression_list)
            {
                if (analysis.join_using_results.at(&table_join).left_join_fields.size() < 2)
                    throw Exception(ErrorCodes::LOGICAL_ERROR, "Join using list must have at least 2 elements for asof join");
            }
            else if (table_join.on_expression)
            {
                auto & join_on_result = analysis.join_on_results.at(&table_join);
                if (join_on_result.equality_conditions.empty())
                    throw Exception(ErrorCodes::LOGICAL_ERROR, "Asof join must have at least 1 equality condition");
                if (join_on_result.inequality_conditions.size() != 1)
                    throw Exception(ErrorCodes::LOGICAL_ERROR, "Asof join must have exact 1 inequality condition");
                if (!join_on_result.complex_expressions.empty())
                    throw Exception(ErrorCodes::LOGICAL_ERROR, "Asof join must not have complex condition");
            }
            else
                throw Exception(ErrorCodes::LOGICAL_ERROR, "Asof join must have join conditions");
        }
    }
    ...
    ASTSelectQuery & select_query)
{
    ...
    if (expr_list.empty())
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Empty join using list is not allowed.");
    ...
    if (use_ansi_semantic)
    {
        auto resolve_join_key
            ...
            if (!resolved)
                throw Exception(
                    ...
                    "Can not find column '{}' in join {} side",
                    name.toString(),
                    (left ? "left" : "right"));
        ...
        for (const auto & join_key_ast : expr_list)
        {
            ...
            if (!iden)
                throw Exception(ErrorCodes::LOGICAL_ERROR, "Expression except identifier is not allowed in join using list.");

            if (iden->compound())
                throw Exception(ErrorCodes::LOGICAL_ERROR, "Compound identifier is not allowed in join using list.");

            if (!seen_names.insert(iden->name()).second)
                throw Exception(ErrorCodes::LOGICAL_ERROR, "Duplicated join key found in join using list.");
            ...
            else if (enable_implicit_type_conversion)
            {
                ...
                catch (DB::Exception & ex)
                {
                    throw Exception(
                        ...
                        "Type mismatch of columns to JOIN by: {} at left, {} at right. Can't get supertype: {}",
                        left_type->getName(),
                        right_type->getName(),
                        ex.message());
                }
            ...
            if (!output_type)
                throw Exception(ErrorCodes::TYPE_MISMATCH, "Type mismatch for join keys: {}", serializeAST(table_join));
    ...
    else
    {
        ...
        for (size_t i = 0, true_index = 0; i < expr_list.size(); ++i)
        {
            ...
            if (!resolved)
                throw Exception(ErrorCodes::UNKNOWN_IDENTIFIER, "Can not find column {} on right side", key_name);
        ...
        bool check_identifier_begin_valid = context->getOptimizerContext()->getSettingsRef().check_identifier_begin_valid;
    ...
    if (table_join.on_expression)
    {
        ...
        auto array_join_exprs = extractExpressions(context, analysis, table_join.on_expression, false, [&](const ASTPtr & node) {
            if (const auto * func = node->as<ASTFunction>())
                if (func->name == "arrayJoin" && (!context->getOptimizerContext()->getSettingsRef().ignore_array_join_check_in_join_on_condition))
                    return true;
...
ScopePtr QueryAnalyzerVisitor::analyzeArrayJoin(ASTArrayJoin & array_join, ASTSelectQuery & select_query, ScopePtr source_scope)
{
    ...
    for (auto & array_join_expr : array_join_expression_list->children)
    {
        ...
        if (name_set.count(output_name))
            throw Exception(ErrorCodes::MULTIPLE_EXPRESSIONS_FOR_ALIAS, "Duplicate alias in ARRAY JOIN: {}", output_name);
        ...
        if (array_join_expr->as<ASTIdentifier>())
        {
            ...
            if (!matched)
                throw Exception(ErrorCodes::UNKNOWN_IDENTIFIER, "Can not resolve array join column: {}", column_name);
        }
        else
        {
            if (array_join_expr->tryGetAlias().empty())
                throw Exception(
                    ...
                    "No alias for non-trivial value in ARRAY JOIN: {}",
                    array_join_expr->tryGetAlias());
    ...
    ASTSelectQuery & select_query, ScopePtr source_scope, ASTPtr & alias_columns, Aliases & query_aliases)
{
    ...
    if (select_query.join())
        throw Exception(ErrorCodes::ILLEGAL_PREWHERE, "Prewhere is not supported in query with join");
    ...
    if (auto inner_type = removeNullable(removeLowCardinality(prewhere_filter_type)))
    {
        if (!inner_type->equals(DataTypeUInt8()) && !inner_type->equals(DataTypeNothing()))
            throw Exception(
                ...
                "Illegal type {} for PREWHERE. Must be UInt8 or Nullable(UInt8).",
                prewhere_filter_type->getName());
    }
...
void QueryAnalyzerVisitor::analyzeWhere(ASTSelectQuery & select_query, ScopePtr source_scope)
{
    ...
    if (auto inner_type = removeNullable(removeLowCardinality(filter_type)))
    {
        if (!inner_type->equals(DataTypeUInt8()) && !inner_type->equals(DataTypeNothing()))
            throw Exception(
                ErrorCodes::ILLEGAL_TYPE_OF_COLUMN_FOR_FILTER,
                "Illegal type {} for WHERE. Must be UInt8 or Nullable(UInt8).",
                filter_type->getName());
    }
}
...
ASTs QueryAnalyzerVisitor::analyzeSelect(ASTSelectQuery & select_query, ScopePtr source_scope)
{
    ...
    auto add_select_expression = [&](ASTPtr & expression) {
        ...
        auto get_output_name = [&](const ASTPtr & expr) -> String {
            ...
            if (!use_ansi_semantic)
            {
                if (expr->as<ASTFieldReferenceExt>())
                {
                    auto col_ref = analysis.tryGetColumnReference(expr);
                    if (!col_ref)
                        throw Exception(ErrorCodes::LOGICAL_ERROR, "invalid select expression");
    ...
    for (auto & select_item : select_query.refSelect()->children)
    {
        if (select_item->as<ASTAsterisk>())
        {
            for (size_t field_index = 0; field_index < source_scope->size(); ++field_index)
            {
                ...
                {
                    ASTPtr field_reference = std::make_shared<ASTFieldReferenceExt>(field_index);
        ...
        else if (select_item->as<ASTQualifiedAsterisk>())
        {
            ...
            if (!matched)
                throw Exception(ErrorCodes::UNKNOWN_IDENTIFIER, "Can not find column of {} in Scope", prefix.toString());
        }
        else if (auto * asterisk_pattern = select_item->as<ASTColumnsRegexpMatcher>())
        {
            String pattern = asterisk_pattern->getPattern();
            ...
            {
                if (source_scope->at(field_index).substituted_by_asterisk &&
                    re2::RE2::PartialMatch(source_scope->at(field_index).name, *regexp))
                {
                    ASTPtr field_reference = std::make_shared<ASTFieldReferenceExt>(field_index);
                    add_select_expression(field_reference);
                }
...
void QueryAnalyzerVisitor::analyzeGroupBy(ASTSelectQuery & select_query, ASTs & select_expressions, ScopePtr source_scope)
{
    ...
    if (select_query.groupBy())
    {
            ...
            for (ASTPtr grouping_expr : grouping_expr_list)
            {
                if (allow_group_by_position)
                        ...
                        literal->value.getType() == Field::Types::UInt64)
                    {
                        ...
                        {
                            throw Exception(ErrorCodes::BAD_ARGUMENTS, "Group by index is greater than the number of select elements");
                        }
        ...
        if (select_query.group_by_with_totals)
        {
            ...
            {
                throw Exception(ErrorCodes::NOT_IMPLEMENTED, "WITH TOTALS and ROLLUP/CUBE/GROUPING SETS are not supported together");
            }
        }
    }
        ...
        for (auto order_item : select_query.orderBy()->children)
        {
            ...
            if (context->getSettingsRef().enable_positional_arguments)
                    ...
                    literal->value.getType() == Field::Types::UInt64)
                {
                    ...
                    if (index > select_expressions.size() || index < 1)
                    {
                        throw Exception(ErrorCodes::BAD_ARGUMENTS, "Order by index is greater than the number of select elements");
                    }
        ...
        for (auto limit_item : select_query.limitBy()->children)
        {
            if (context->getSettingsRef().enable_positional_arguments)
                    ...
                    literal->value.getType() == Field::Types::UInt64)
                {
                    ...
                    if (index > select_expressions.size() || index < 1)
                    {
                        throw Exception(ErrorCodes::BAD_ARGUMENTS, "Limit by index is greater than the number of select elements");
                    }
...
void QueryAnalyzerVisitor::analyzeOutfile(ASTSelectWithUnionQuery & outfile_query)
{
    ...
    if (outfile_query.out_file)
    {
        ...
        if (outfile_query.compression)
        {
            compression_method = outfile_query.compression->as<ASTLiteral &>().value.safeGet<std::string>();
            ...
            if (outfile_query.compression_level)
            {
                ...
                {
                    throw Exception(ErrorCodes::BAD_ARGUMENTS, "Invalid compression level");
                }
            }
        }
...
namespace
{
    ...
    private:
        ...
        {
            throw Exception(code, "{} is not allowed in {}", expr_type, statement_name);
        }
    };
}
...
namespace
{
    ...
    protected:
        ...
        void visitFieldReference(ASTPtr & node, ASTFieldReferenceExt &, const Void &) override { verifyNoReferenceToNonGroupingKey(node); }
    ...
    void PostAggregateAnalyzerExpressionVisitor::verifyNoReferenceToNonGroupingKey(ASTPtr & node)
    {
        ...
        if (auto col_ref = analysis.tryGetColumnReference(node); col_ref && col_ref->scope == source_scope)
        {
            if (!grouping_field_indices.count(col_ref->local_index))
                throw Exception(ErrorCodes::NOT_AN_AGGREGATE, "{} is not a grouping key", serializeAST(*node));
        }
    }
    ...
    void PostAggregateAnalyzerExpressionVisitor::verifyNoReferenceToNonGroupingKeyInSubquery(ASTPtr & node)
    {
        ...
        for (const auto & col_ref_expr : col_ref_exprs)
        {
            if (auto col_ref = analysis.tryGetColumnReference(col_ref_expr))
                if (!grouping_field_indices.count(col_ref->local_index))
                    throw Exception(ErrorCodes::NOT_AN_AGGREGATE, "Non grouping key used in subquery: {}", serializeAST(*node));
        }
    }
...
void QueryAnalyzerVisitor::verifyAggregate(ASTSelectQuery & select_query, ScopePtr source_scope)
{
    if (!analysis.needAggregate(select_query))
    {
        ...
        if (!analysis.getGroupingOperations(select_query).empty())
        {
            ...
            throw Exception(ErrorCodes::BAD_ARGUMENTS, "Invalid grouping operation: {}", serializeAST(*representative));
        }
        ...
        for (const auto & grouping_op : grouping_ops)
            for (const auto & grouping_op_arg : grouping_op->arguments->children)
                if (!grouping_expressions.count(grouping_op_arg))
                    throw Exception(ErrorCodes::BAD_ARGUMENTS, "Invalid grouping operation: {}", serializeAST(*grouping_op));
    }
    ...
    protected:
        ...
        void visitIdentifier(ASTPtr & node, ASTIdentifier &, const Void &) override
        {
            if (auto lambda_ref = analysis.tryGetLambdaArgumentReference(node); lambda_ref && !lambda_scopes.count(lambda_ref->scope))
                throw Exception(error_code, "Free lambda argument reference found in {}: {}", function_name, serializeAST(*node));
        }
...
UInt64 QueryAnalyzerVisitor::analyzeUIntConstExpression(const ASTPtr & expression)
{
    ...
    if (!val)
        throw Exception(
            ...
            "Element of set in IN, VALUES or LIMIT is not a constant expression: {}",
