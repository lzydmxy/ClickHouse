...
public:
     ...
     *    1) identifier's last name equal to field's name;
     *    2) identifier's name prefix is a suffix of field's name prefix.
    ...
    /// parent scope of current query, null if `this` is a root scope
    ScopePtr getLocalParent() const;
    /// root scope of current query, nonnull
    ScopePtr getQueryBoundaryScope() const;
    /// first scope of outer query, null if outer query not exists
    ScopePtr getOuterQueryScope() const;
