...
#include <Query/ProtosHelper/ReadWriteProtobuf.h>
...
public:
    ...
    static ASTPtr parse(const std::string & query, ContextMutablePtr query_context)
    {
        ...
        auto size = query_context->getSettingsRef().max_query_size;
        auto depth = query_context->getSettingsRef().max_parser_depth;
        auto backtracks = query_context->getSettingsRef().max_parser_backtracks;
        auto ast = parseQuery(parser, begin, end, "", size, depth, backtracks);
        return ast;
    }
    ...
    static void SetUpTestCase()
    {
        ...
        auto dt1 = std::make_shared<const DataTypeString>();
        auto dt2 = std::make_shared<const DataTypeUInt32>();

        test_data_types.push_back(std::move(dt1));
        test_data_types.push_back(std::move(dt2));
        test_data_types.push_back(std::move(map_type));
    ...
    // static std::shared_ptr<DB::TableWriteStep::InsertTarget> generateTableWriteStepInsertTarget(std::default_random_engine & eng)
    // {
    ...
    //     auto step = std::make_shared<DB::TableWriteStep::InsertTarget>(storage, storage_id, columns, nullptr);
    //     return step;
    // }
    ...
    static Block generateBlock(std::default_random_engine & eng, bool arr = false)
    {
        ...
        for (size_t i = 0; i * sizeof(UInt64) < size_of_row_in_bytes; i++)
        {
            ...
            ColumnWithTypeAndName col(std::move(column), std::make_shared<DataTypeUInt64>(), columns[i]);
            cols.push_back(col);
        }
        if (arr)
        {
            ...
            auto column = ColumnArray::create(std::move(val), std::move(offset));
            auto tp = std::make_shared<const DataTypeArray>(std::make_shared<const DataTypeUInt64>());
            ColumnWithTypeAndName col(std::move(column), tp, "arr");
    ...
    // static Protos::Partitioning generatePartitioning(std::default_random_engine & eng)
    // {
    //     auto handle = static_cast<Protos::Partitioning::Handle>(eng() % 3);
    //     Names columns;
    //     for (int i = 0; i < 10; ++i)
    ...
    static Assignments generateAssignments(std::default_random_engine & eng)
    {
        Names columns;
        columns.emplace_back(fmt::format("text{}", eng() % 100));
        columns.emplace_back(fmt::format("text{}", eng() % 100));
    ...
    static GroupingSetsParams generateGroupingSetsParams(std::default_random_engine & eng)
    {
        ...
        for (int i = 0; i < 2; ++i)
            res.used_keys.emplace_back(std::to_string(eng() % 3));
        // generate ColumnNumbers
        for (int i = 0; i < 2; ++i)
            res.missing_keys.emplace_back(std::to_string(eng() % 3));
        return res;
    }

    // No GroupingDescription class
    ...
    static Aggregator::Params generateAggregatorParams(std::default_random_engine & eng)
    {
        ...
        auto tmp_volume = context ? context->getTempDataOnDisk() : nullptr;
        ...
        auto step = Aggregator::Params(
            // src_header,
            keys, aggregates, overflow_row, max_rows_to_group_by, group_by_overflow_mode,

            group_by_two_level_threshold, group_by_two_level_threshold_bytes, max_bytes_before_external_group_by, empty_result_for_aggregation_by_empty_set, tmp_volume,

            max_threads, min_free_disk_space, compile_aggregate_expressions, min_count_to_compile_aggregate_expression, 65409,

            true, true, true, 10 * 1024 * 1024, {});
    ...
    {
        auto header = generateBlock(eng);
        auto params = generateAggregatorParams(eng);
        auto final = eng() % 2 == 1;
        auto step = std::make_shared<AggregatingTransformParams>(header, params, final);
    ...
    static InputOrderInfoPtr generateInputOrderInfo(std::default_random_engine & eng)
    {
        ...
        auto res = std::make_shared<InputOrderInfo>(order_key_prefix_descr, direction, 1, 0);
    ...
    static WindowFunctionDescription generateWindowFunctionDescription(std::default_random_engine & eng)
    {
        ...
        AggregateFunctionProperties properties;
        res.aggregate_function = AggregateFunctionFactory::instance().get("uniq", NullsAction::EMPTY, res.argument_types,
            res.function_parameters, properties);
    ...
    {
        auto id = eng() % 1000;
        return RuntimeFilter(id, RRuntimeFilter::LOCAL);
    }
