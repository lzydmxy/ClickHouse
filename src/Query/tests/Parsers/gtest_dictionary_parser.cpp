...
TEST(ParserDictionaryDDL, SimpleDictionaryExt)
{
...
TEST(ParserDictionaryDDL, AttributesWithMultiplePropertiesExt)
{
...
TEST(ParserDictionaryDDL, CustomAttributePropertiesOrderExt)
{
...
TEST(ParserDictionaryDDL, NestedSourceExt)
{
...
TEST(ParserDictionaryDDL, FormattingExt)
{
    ...
    EXPECT_EQ(
        ...
        "SOURCE(MYSQL(HOST 'localhost' PORT 9000 USER 'default' REPLICA (HOST '127.0.0.1' PRIORITY 1) PASSWORD '')) LIFETIME(MIN 1 MAX 10) "
        "LAYOUT(CACHE(SIZE_IN_CELLS 50)) RANGE(MIN second_column MAX third_column)");
}

TEST(ParserDictionaryDDL, ParseDropQueryExt)
{
    ...
    DB::ASTReplaceVisitor::replace(ast1);
    ...
    DB::ASTReplaceVisitor::replace(ast2);
...
TEST(ParserDictionaryDDL, ParsePropertiesQueriesExt)
{
