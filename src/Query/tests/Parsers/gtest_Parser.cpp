...
TEST_P(ParserTest, parseQueryExt)
{
    ...
    if (expected_ast)
    {
        ...
        else
        {
            ...
            else if (input_text.starts_with("ATTACH"))
            {
...
INSTANTIATE_TEST_SUITE_P(
    ParserOptimizeQueryExt,
    ParserTest,
    ::testing::Combine(
        ::testing::Values(std::make_shared<ParserOptimizeQuery>()),
        ::testing::ValuesIn(
            std::initializer_list<ParserTestCase>{
...
INSTANTIATE_TEST_SUITE_P(
    ParserAlterCommand_MODIFY_COMMENTExt,
    ParserTest,
    ::testing::Combine(
        ::testing::Values(std::make_shared<ParserAlterCommand>(false)),
        ::testing::ValuesIn(
...
INSTANTIATE_TEST_SUITE_P(
    ParserCreateQuery_DICTIONARY_WITH_COMMENTExt,
    ParserTest,
    ::testing::Combine(
        ...
        ::testing::ValuesIn(
            std::initializer_list<ParserTestCase>{
                ...
                {"CREATE DATABASE db ENGINE=MaterializeMySQL('addr:port', 'db', 'user', 'pw') TABLE OVERRIDE `tbl`\n(PARTITION BY "
                 ...
                 "CREATE DATABASE db\nENGINE = MaterializeMySQL('addr:port', 'db', 'user', 'pw')\nTABLE OVERRIDE `tbl`\n(\n    PARTITION "
                 "BY toYYYYMM(`created`)\n)"},
                {"CREATE DATABASE db ENGINE=Foo TABLE OVERRIDE `tbl` (), TABLE OVERRIDE a (COLUMNS (_created DateTime MATERIALIZED "
                 "now())), TABLE OVERRIDE b (PARTITION BY rand())",
                 "CREATE DATABASE db\nENGINE = Foo\nTABLE OVERRIDE `tbl`\n(\n\n),\nTABLE OVERRIDE `a`\n(\n    COLUMNS\n    (\n        "
                 "`_created` DateTime MATERIALIZED now()\n    )\n),\nTABLE OVERRIDE `b`\n(\n    PARTITION BY rand()\n)"},
                {"CREATE DATABASE db ENGINE=MaterializeMySQL('addr:port', 'db', 'user', 'pw') TABLE OVERRIDE tbl (COLUMNS (id UUID) "
                ...
                {"CREATE DATABASE db TABLE OVERRIDE t1 (TTL inserted + INTERVAL 1 MONTH DELETE), TABLE OVERRIDE t2 (TTL `inserted` + "
                 "INTERVAL 2 MONTH DELETE)",
                 "CREATE DATABASE db\nTABLE OVERRIDE `t1`\n(\n    TTL `inserted` + toIntervalMonth(1)\n),\nTABLE OVERRIDE `t2`\n(\n    TTL "
                 "`inserted` + toIntervalMonth(2)\n)"},
                {"CREATE DATABASE db ENGINE = MaterializeMySQL('127.0.0.1:3306', 'db', 'root', 'pw') SETTINGS allows_query_when_mysql_lost "
                 "= 1 TABLE OVERRIDE tab3 (COLUMNS (_staged UInt8 MATERIALIZED 1) PARTITION BY (c3) TTL c3 + INTERVAL 10 minute), TABLE "
                 "OVERRIDE tab5 (PARTITION BY (c3) TTL c3 + INTERVAL 10 minute)",
        ...
        ::testing::ValuesIn(
            std::initializer_list<ParserTestCase>{
                {"ATTACH USER user1 IDENTIFIED WITH sha256_hash BY "
                 ...
                 "^[A-Za-z0-9]{64}$"},
                {"ATTACH USER user1 IDENTIFIED WITH sha256_hash BY "
                 "'2CC4880302693485717D34E06046594CFDFE425E3F04AA5A094C4AABAB3CB0BF'", //for users created in older releases that sha256_password has no salt
                 "^$"}})));

INSTANTIATE_TEST_SUITE_P(
    ParserRenameQueryExt,
    ParserTest,
    ::testing::Combine(
        ::testing::Values(std::make_shared<ParserRenameQuery>()),
        ::testing::ValuesIn(
            std::initializer_list<ParserTestCase>{
                {"RENAME TABLE eligible_test TO eligible_test2", "RENAME TABLE eligible_test TO eligible_test2"}})));
        ...
        ::testing::ValuesIn(
            std::initializer_list<ParserTestCase>{
                ...
                {"Customers | take 3 | order by FirstName desc",
                 "SELECT *\nFROM\n(\n    SELECT *\n    FROM Customers\n    LIMIT 3\n)\nORDER BY FirstName DESC"},
                {"Customers | sort by FirstName asc", "SELECT *\nFROM Customers\nORDER BY FirstName ASC"},
                {"Customers | sort by FirstName", "SELECT *\nFROM Customers\nORDER BY FirstName DESC"},
                {"Customers | order by LastName", "SELECT *\nFROM Customers\nORDER BY LastName DESC"},
                {"Customers | order by Age desc, FirstName asc  ", "SELECT *\nFROM Customers\nORDER BY\n    Age DESC,\n    FirstName ASC"},
                {"Customers | order by Age asc, FirstName desc", "SELECT *\nFROM Customers\nORDER BY\n    Age ASC,\n    FirstName DESC"},
                {"Customers | sort by FirstName | order by Age ", "SELECT *\nFROM Customers\nORDER BY\n    Age DESC,\n    FirstName DESC"},
                {"Customers | sort by FirstName nulls first", "SELECT *\nFROM Customers\nORDER BY FirstName DESC NULLS FIRST"},
                {"Customers | sort by FirstName nulls last", "SELECT *\nFROM Customers\nORDER BY FirstName DESC NULLS LAST"},
                {"Customers | where Occupation == 'Skilled Manual'", "SELECT *\nFROM Customers\nWHERE Occupation = 'Skilled Manual'"},
                {"Customers | where Occupation != 'Skilled Manual'", "SELECT *\nFROM Customers\nWHERE Occupation != 'Skilled Manual'"},
                ...
                {"Customers |where Education contains_cs  'Degree'", "SELECT *\nFROM Customers\nWHERE Education LIKE '%Degree%'"},
                {"Customers | where Occupation startswith_cs  'Skil'", "SELECT *\nFROM Customers\nWHERE startsWith(Occupation, 'Skil')"},
                {"Customers | where FirstName endswith_cs  'le'", "SELECT *\nFROM Customers\nWHERE endsWith(FirstName, 'le')"},
                {"Customers | where Age == 26", "SELECT *\nFROM Customers\nWHERE Age = 26"},
                {"Customers | where Age > 20 and Age < 30", "SELECT *\nFROM Customers\nWHERE (Age > 20) AND (Age < 30)"},
                ...
                {"Customers |summarize  max(Age) by Occupation",
                 "SELECT\n    Occupation,\n    max(Age) AS max_Age\nFROM Customers\nGROUP BY Occupation"},
                {"Customers | where FirstName contains 'pet'", "SELECT *\nFROM Customers\nWHERE FirstName ILIKE '%pet%'"},
                {"Customers | where FirstName !contains 'pet'", "SELECT *\nFROM Customers\nWHERE NOT (FirstName ILIKE '%pet%')"},
                {"Customers | where FirstName endswith 'er'", "SELECT *\nFROM Customers\nWHERE FirstName ILIKE '%er'"},
                ...
                {"Customers | where Education has_cs 'School'", "SELECT *\nFROM Customers\nWHERE hasToken(Education, 'School')"},
                {"Customers | where Education !has_cs 'School'", "SELECT *\nFROM Customers\nWHERE NOT hasToken(Education, 'School')"},
                {"Customers | where FirstName matches regex 'P.*r'", "SELECT *\nFROM Customers\nWHERE match(FirstName, 'P.*r')"},
                {"Customers | where FirstName startswith 'pet'", "SELECT *\nFROM Customers\nWHERE FirstName ILIKE 'pet%'"},
                {"Customers | where FirstName !startswith 'pet'", "SELECT *\nFROM Customers\nWHERE NOT (FirstName ILIKE 'pet%')"},
                {"Customers | where Age in ((Customers|project Age|where Age < 30))",
                 "SELECT *\nFROM Customers\nWHERE Age IN (\n    SELECT Age\n    FROM Customers\n    WHERE Age < 30\n)"},
                {"Customers|where Occupation has_any ('Skilled','abcd')",
                 "SELECT *\nFROM Customers\nWHERE hasTokenCaseInsensitive(Occupation, 'Skilled') OR hasTokenCaseInsensitive(Occupation, "
                 "'abcd')"},
                {"Customers|where Occupation has_all ('Skilled','abcd')",
                 "SELECT *\nFROM Customers\nWHERE hasTokenCaseInsensitive(Occupation, 'Skilled') AND hasTokenCaseInsensitive(Occupation, "
                 "'abcd')"},
                {"Customers|where Occupation has_all (strcat('Skill','ed'),'Manual')",
                 "SELECT *\nFROM Customers\nWHERE hasTokenCaseInsensitive(Occupation, concat('Skill', 'ed')) AND "
                 "hasTokenCaseInsensitive(Occupation, 'Manual')"},
                ...
                {"Customers | project split('aaa_bbb_ccc', '_', 1)",
                 "SELECT multiIf((length(if(empty('_'), splitByString(' ', 'aaa_bbb_ccc'), splitByString('_', 'aaa_bbb_ccc'))) >= 2) AND "
                 "(2 > 0), arrayPushBack([], if(empty('_'), splitByString(' ', 'aaa_bbb_ccc'), splitByString('_', 'aaa_bbb_ccc'))[2]), 2 = "
                 "0, if(empty('_'), splitByString(' ', 'aaa_bbb_ccc'), splitByString('_', 'aaa_bbb_ccc')), arrayPushBack([], "
                 "NULL[1]))\nFROM Customers"},
                {"Customers | project strcat_delim('-', '1', '2', 'A')", "SELECT concat('1', '-', '2', '-', 'A')\nFROM Customers"},
                {"Customers | project indexof('abcdefg','cde')", "SELECT position('abcdefg', 'cde', 1) - 1\nFROM Customers"},
                {"Customers | project indexof('abcdefg','cde', 2) ", "SELECT position('abcdefg', 'cde', 3) - 1\nFROM Customers"},
                {"print x=1, s=strcat('Hello', ', ', 'World!')", "SELECT\n    1 AS x,\n    concat('Hello', ', ', 'World!') AS s"},
                {"print parse_urlquery('https://john:123@google.com:1234/this/is/a/path?k1=v1&k2=v2#fragment')",
                 "SELECT concat('{', concat('\"Query Parameters\":', concat('{\"', "
                 "replace(replace(if(position('https://john:123@google.com:1234/this/is/a/path?k1=v1&k2=v2#fragment', '?') > 0, "
                 "queryString('https://john:123@google.com:1234/this/is/a/path?k1=v1&k2=v2#fragment'), "
                 "'https://john:123@google.com:1234/this/is/a/path?k1=v1&k2=v2#fragment'), '=', '\":\"'), '&', '\",\"'), '\"}')), '}')"},
                {"print strcmp('a','b')", "SELECT multiIf('a' = 'b', 0, 'a' < 'b', -1, 1)"},
                {"print parse_url('https://john:123@google.com:1234/this/is/a/path?k1=v1&k2=v2#fragment')",
                 "SELECT concat('{', concat('\"Scheme\":\"', "
                 "protocol('https://john:123@google.com:1234/this/is/a/path?k1=v1&k2=v2#fragment'), '\"'), ',', concat('\"Host\":\"', "
                 "domain('https://john:123@google.com:1234/this/is/a/path?k1=v1&k2=v2#fragment'), '\"'), ',', concat('\"Port\":\"', "
                 "toString(port('https://john:123@google.com:1234/this/is/a/path?k1=v1&k2=v2#fragment')), '\"'), ',', "
                 "concat('\"Path\":\"', path('https://john:123@google.com:1234/this/is/a/path?k1=v1&k2=v2#fragment'), '\"'), ',', "
                 "concat('\"Username\":\"', splitByChar(':', splitByChar('@', "
                 "netloc('https://john:123@google.com:1234/this/is/a/path?k1=v1&k2=v2#fragment'))[1])[1], '\"'), ',', "
                 "concat('\"Password\":\"', splitByChar(':', splitByChar('@', "
                 "netloc('https://john:123@google.com:1234/this/is/a/path?k1=v1&k2=v2#fragment'))[1])[2], '\"'), ',', concat('\"Query "
                 "Parameters\":', concat('{\"', "
                 "replace(replace(queryString('https://john:123@google.com:1234/this/is/a/path?k1=v1&k2=v2#fragment'), '=', '\":\"'), '&', "
                 "'\",\"'), '\"}')), ',', concat('\"Fragment\":\"', "
                 "fragment('https://john:123@google.com:1234/this/is/a/path?k1=v1&k2=v2#fragment'), '\"'), '}')"},
                ...
                {"print output = dynamic([1, 2, 3])", "SELECT [1, 2, 3] AS output"},
