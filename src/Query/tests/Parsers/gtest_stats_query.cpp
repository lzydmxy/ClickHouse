...
protected:
    void checkCreateStatsQuerySuccessParse(const String & input, const CreateStatsASTParseResult & expect)
    {
        ...
        ParserQueryWithOutputExt parser{end};
        auto res = parseQuery(parser, beg, end, "", 0, 0, 0);
        DB::ASTReplaceVisitor::replace(res);
        auto * ast = res->as<ASTCreateStatsQueryExt>();
        ASSERT_TRUE(ast) << input;
        EXPECT_EQ(ast->getID(' '), expect.id);
        EXPECT_EQ(ast->getType(), ASTType::ASTCreateStatsQueryExt);
    ...
    void checkFailParse(String input)
    {
        ...
        try
        {
            parseQuery(parser, beg, end, "", 0, 0, 0);
        }
        catch (Exception & e)
        {
...
{
    checkSuccessParse<ASTShowStatsQueryExt>(
        "SHOW TABLE_STATS t1",
        {
            .id = "SHOW TABLE_STATS t1",
            .type = ASTType::ASTShowStatsQueryExt,
            .kind = StatsQueryKind::TABLE_STATS,
            .any_table = false,
            .table = "t1",
        });

    checkSuccessParse<ASTDropStatsQueryExt>(
        "DROP COLUMN_STATS t1 (xx)",
        {
            .id = "DROP COLUMN_STATS t1 xx",
            .type = ASTType::ASTDropStatsQueryExt,
...
{
    std::shared_ptr<ASTShowStatsQueryExt> ptr1 = std::make_shared<ASTShowStatsQueryExt>();
    // ptr1->database = "db";
    // ptr1->table = "table";
    ...
    std::shared_ptr<ASTShowStatsQueryExt> ptr2 = std::dynamic_pointer_cast<ASTShowStatsQueryExt>(cloned);
    ASSERT_TRUE(ptr2);
...
TEST_F(StatsQueryTest, SimpleStatsASTRewrittenWithoutOnCluster)
{
    ...
    ASTPtr rewritten = ptr1->getRewrittenASTWithoutOnCluster(WithoutOnClusterASTRewriteParams{.default_database =  "new_db",.host_id=""});
    std::shared_ptr<ASTShowStatsQueryExt> ptr2 = std::dynamic_pointer_cast<ASTShowStatsQueryExt>(rewritten);
    ASSERT_TRUE(ptr2);
    EXPECT_TRUE(ptr1 != ptr2);
...
TEST_F(StatsQueryTest, CreateStatsQueryParsers)
{
    ...
    checkCreateStatsQuerySuccessParse(
        "CREATE STATS `ALL`",
        CreateStatsASTParseResult{
            .id = "CREATE STATS ALL", .kind = StatsQueryKind::ALL_STATS, .table = "ALL", .formatted_sql = "CREATE STATS ALL"});
...
{
    std::shared_ptr<ASTCreateStatsQueryExt> ptr1 = std::make_shared<ASTCreateStatsQueryExt>();
    // ptr1->database = "db";
    // ptr1->table = "table";
    ptr1->kind = StatsQueryKind::COLUMN_STATS;
    ptr1->any_table = true;
...
TEST_F(StatsQueryTest, CreateStatsASTRewrittenWithoutOnCluster)
{
    ...
    ASTPtr rewritten = ptr1->getRewrittenASTWithoutOnCluster(WithoutOnClusterASTRewriteParams{.default_database =  "new_db",.host_id=""});
    std::shared_ptr<ASTCreateStatsQueryExt> ptr2 = std::dynamic_pointer_cast<ASTCreateStatsQueryExt>(rewritten);
    ASSERT_TRUE(ptr2);
    ASSERT_TRUE(ptr1 != ptr2);
