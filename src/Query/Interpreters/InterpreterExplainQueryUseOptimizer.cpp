...
BlockIO InterpreterExplainQueryUseOptimizer::execute()
{
    ...
    const auto & ast = query->as<ASTExplainQueryExt &>();
    if ((ast.getKind() == ASTExplainQueryExt::DistributedAnalyze || ast.getKind() == ASTExplainQueryExt::LogicalAnalyze
         || ast.getKind() == ASTExplainQueryExt::PipelineAnalyze)
...
Block InterpreterExplainQueryUseOptimizer::getSampleBlock()
{
    Block block;
    const auto & ast = query->as<ASTExplainQueryExt &>();
    if (ast.getKind() == ASTExplainQueryExt::MaterializedView)
    {
...
void InterpreterExplainQueryUseOptimizer::fillColumn(IColumn & column, const std::string & str)
{
    size_t start = 0;
    size_t end = 0;
...
void InterpreterExplainQueryUseOptimizer::rewriteDistributedToLocal(ASTPtr & ast)
{
    if (!ast)
        return;
...
QueryPipeline InterpreterExplainQueryUseOptimizer::executeImpl()
{
    const auto & ast = query->as<ASTExplainQueryExt &>();
...
QueryPipeline InterpreterExplainQueryUseOptimizer::explain()
{
    const auto & ast = query->as<ASTExplainQueryExt &>();
    ...
    // if settings.enable_optimizer = true && query is supported by optimizer, print plan with optimizer.
    // if query is not supported by optimizer, settings `settings.enable_optimizer` in context will be disabled.
    if (ast.getKind() == ASTExplainQueryExt::ParsedAST)
    {
        if (ast.getSettings())
    ...
    else if (ast.getKind() == ASTExplainQueryExt::AnalyzedSyntax)
    {
        if (ast.getSettings())
    ...
    else if (ast.getKind() == ASTExplainQueryExt::QueryPlan)
    {
    ...
    else if (ast.getKind() == ASTExplainQueryExt::QueryPipeline)
    {
        if (!dynamic_cast<const ASTSelectWithUnionQuery *>(ast.getExplainedQuery().get()))
            throw Exception(ErrorCodes::INCORRECT_QUERY, "Only SELECT is supported for EXPLAIN query");
        ...
        auto builder = plan.buildQueryPipeline(
            QueryPlanOptimizationSettings::fromContext(getContext()), BuildQueryPipelineSettingsExt::fromContext(getContext()));
    ...
    else if (ast.getKind() == ASTExplainQueryExt::ExplainKindExt::QueryElement)
    {
        ...
        if (ast_ptr.children.size() != 1)
            throw Exception(ErrorCodes::LOGICAL_ERROR, "Element query not support multiple select query");
    ...
    else if (ast.getKind() == ASTExplainQueryExt::PlanSegment)
    {
        // todo: hongzhigao1, implement InterpreterDistributedStages
        // if (!dynamic_cast<const ASTSelectWithUnionQuery *>(ast.getExplainedQuery().get()))
        //     throw Exception(ErrorCodes::INCORRECT_QUERY, "Only SELECT is supported for EXPLAIN query");
        ...
        throw Exception(
            ErrorCodes::INCORRECT_QUERY, "This explain syntax is not supported, you can try to open optimizer(enable_optimizer=1).");
...
void InterpreterExplainQueryUseOptimizer::elementDatabaseAndTable(
    const ASTSelectQuery & select_query, const ASTPtr & where, WriteBuffer & buffer)
{
    if (!select_query.tables())
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Can not get database and table in element query");

    if (select_query.tables()->children.size() != 1)
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Element query not support multiple table query, such as join, etc");
    ...
    if (database_and_table)
    {
        ...
        buffer << R"("database": ")" << storage->getStorageID().getDatabaseName() << "\", ";
        buffer << R"("table": ")" << storage->getStorageID().getTableName() << "\", ";
        auto dependencies = DatabaseCatalog::instance().getLoadingDependencies(storage->getStorageID());
        buffer << "\"dependencies\": [";
...
void InterpreterExplainQueryUseOptimizer::listPartitionKeys(StoragePtr & storage, WriteBuffer & buffer)
{
...
/**
  ...
  */
void InterpreterExplainQueryUseOptimizer::listRowsOfOnePartition(
    StoragePtr & storage, const ASTPtr & group_by, const ASTPtr & where, WriteBuffer & buffer)
{
    UInt64 base_rows = 0;
    UInt64 view_rows = 0;
    if (getContext()->getOptimizerContext()->getSettingsRef().enable_element_mv_rows)
    {
        ...
        {
            {
                ...
                ParserQuery parser(end, getContext()->getSettingsRef().allow_settings_after_format_in_insert);
                auto query_ast = parseQuery(parser, begin, end, "", 0, 0, getContext()->getSettingsRef().max_parser_backtracks);

                InterpreterSelectWithUnionQuery select(query_ast, getContext(), SelectQueryOptions());
                QueryPipeline in = select.execute().pipeline;
                PullingPipelineExecutor executor(in);
...
std::optional<String> InterpreterExplainQueryUseOptimizer::getActivePartCondition(StoragePtr & storage)
{
    auto * merge_tree = dynamic_cast<MergeTreeData *>(storage.get());
    if (!merge_tree)
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Unknown engine: {}", storage->getName());

    // todo: hongzhgiao1, implement getDataPartsVector
...
void InterpreterExplainQueryUseOptimizer::elementWhere(const ASTPtr & where, WriteBuffer & buffer)
{
    buffer << "\"where\": "
           << "\"";
    if (where)
...
void InterpreterExplainQueryUseOptimizer::elementMetrics(const ASTPtr & select, WriteBuffer & buffer)
{
    buffer << "\"metrics\": [";
...
void InterpreterExplainQueryUseOptimizer::elementDimensions(const ASTPtr & select, WriteBuffer & buffer)
{
    buffer << "\"dimensions\": [";
...
void InterpreterExplainQueryUseOptimizer::elementGroupBy(const ASTPtr & group_by, WriteBuffer & buffer)
{
    buffer << "\"group_by\": [";
    if (group_by)
    {
...
QueryPipeline InterpreterExplainQueryUseOptimizer::explainUsingOptimizer()
{
    WriteBufferFromOwnString buffer;
    bool single_line = false;
    const auto & explain = query->as<ASTExplainQueryExt &>();
    auto context = getContext();

    if (explain.getKind() == ASTExplainQueryExt::AnalyzedSyntax)
    {
    ...
    else if (explain.getKind() == ASTExplainQueryExt::TraceOptimizer || explain.getKind() == ASTExplainQueryExt::TraceOptimizerRule)
    {
        if (explain.getSettings())
            throw Exception(ErrorCodes::UNKNOWN_SETTING, "Settings are not supported for EXPLAIN TRACE OPTIMIZER query.");

        context->getOptimizerContext()->initOptimizerProfile();
        try
        {
        ...
        {
            context->getOptimizerContext()->clearOptimizerProfile();
            throw;
        }

        if (explain.getKind() == ASTExplainQueryExt::TraceOptimizerRule)
            buffer << context->getOptimizerContext()->getOptimizerProfile(true);
        else
            buffer << context->getOptimizerContext()->getOptimizerProfile();
    }
    else
    {
        InterpreterSelectQueryUseOptimizer interpreter(explain.getExplainedQuery(), context, SelectQueryOptions());
        auto query_plan = interpreter.getQueryPlan();
        if (explain.getKind() == ASTExplainQueryExt::ExplainKindExt::OptimizerPlan
        ...
        else if (explain.getKind() == ASTExplainQueryExt::ExplainKindExt::Distributed)
        {
            explainDistributedWithOptimizer(explain, *query_plan, buffer, context);
        }
        else if (explain.getKind() == ASTExplainQueryExt::ExplainKindExt::QueryPipeline)
        {
...
BlockIO InterpreterExplainQueryUseOptimizer::explainAnalyze()
{
    BlockIO res;

    auto context_ptr = getContext();
    context_ptr->getOptimizerContext()->setSetting("report_segment_profiles", true);
    context_ptr->getOptimizerContext()->setSetting("log_explain_analyze_type", Field("NONE"));
    context_ptr->getOptimizerContext()->setIsExplainQuery(true);
    try
    {
...
void InterpreterExplainQueryUseOptimizer::explainPlanWithOptimizer(
    ...
    bool & /*single_line*/)
{
    ...
    else if (settings.pb_json)
    {
        ...
        if (!status.ok())
            buffer << "Error converting message to JSON: " << status.ToString();
        else
            buffer << json_msg;
    }
    else
...
void InterpreterExplainQueryUseOptimizer::explainDistributedWithOptimizer(
    const ASTExplainQueryExt & explain_ast, QueryPlanExt & plan, WriteBuffer & buffer, ContextMutablePtr & context_ptr)
{
    // todo: hongzhigao1, implement explainDistributedWithOptimizer
    // auto settings = checkAndGetSettings<QueryPlanSettings>(explain_ast.getSettings());
    // QueryPlanExt query_plan = PlanNodeToNodeVisitor::convert(plan);
    // PlanSegmentTreePtr plan_segment_tree = std::make_unique<PlanSegmentTree>();
    // // select health worker before split
    // if (context_ptr->getOptimizerContext()->getSettingsRef().scheduler_mode != SchedulerMode::SKIP
    //     && context_ptr->getOptimizerContext()->tryGetCurrentWorkerGroup())
    // {
...
QueryPipeline InterpreterExplainQueryUseOptimizer::explainMetaData()
{
    const auto & explain = query->as<ASTExplainQueryExt &>();
    ...
    if (metadata_settings.lineage || metadata_settings.lineage_use_optimizer)
    {
        ...
        {
            tryLogCurrentException(getLogger("InterpreterExplainQueryUseOptimizer::explainMetaData"), "build plan failed.");
        }
    }
    ...
    ColumnPtr settings_column = ColumnMap::create(ColumnArray::create(
        ColumnTuple::create(Columns{std::move(key_column), std::move(value_column)}), std::move(settings_offset_column)));
    ColumnPtr func_args_column = ColumnMap::create(ColumnArray::create(
        ColumnTuple::create(Columns{std::move(func_column), std::move(args_column)}), std::move(func_args_offset_column)));
    ...
    block.insert(
        {func_args_column,
         ...
         "function_const_arguments"});

    return QueryPipeline(std::make_shared<SourceFromSingleChunk>(block));
}

void InterpreterExplainQueryUseOptimizer::explainPipelineWithOptimizer(
    const ASTExplainQueryExt & explain_ast, QueryPlanExt & plan, WriteBuffer & buffer, ContextMutablePtr & context_ptr)
{
    ...
    for (auto & plan_segment : plan_segments)
    {
        auto * segment = plan_segment.getPlanSegment();
        segment->update(context_ptr);
        buffer << "\nSegment[ " << std::to_string(segment->getPlanSegmentId()) << " ] :\n";
        auto & segment_plan = segment->getQueryPlan();
        auto builder = segment_plan.buildQueryPipeline(
            QueryPlanOptimizationSettings::fromContext(context_ptr),
            BuildQueryPipelineSettingsExt::fromPlanSegment(
                segment, {.execution_address = std::make_shared<AddressInfo>(segment->getCoordinatorAddress())}, context_ptr, true));
        if (builder)
        {
            if (settings.graph)
            {
