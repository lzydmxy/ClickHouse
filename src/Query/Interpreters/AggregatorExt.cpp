...
Block AggregatorExt::getHeader(bool final) const
{
    return params.getHeader(final);
}

Block AggregatorExt::Params::getHeader(
...
void AggregatorExt::Params::explain(WriteBuffer & out, size_t indent) const
{
    Strings res;
    const auto & header = src_header ? src_header : intermediate_header;
...
void AggregatorExt::Params::explain(JSONBuilder::JSONMap & map) const
{
    const auto & header = src_header ? src_header : intermediate_header;
...
void AggregatorExt::compileAggregateFunctions()
{
...

    {
        ...
        {
            auto [compiled_function_cache_entry, _] = compilation_cache->getOrSet(
                aggregate_functions_description_hash_key,
                [&]()
                {
                    LOG_TRACE(log, "Compile expression {}", functions_description);

                    auto compiled_aggregate_functions
                        = ::DB::compileAggregateFunctions(getJITInstance(), functions_to_compile, functions_description);
...
void AggregatorExt::chooseAggregationMethodByOption(
    const ChooseMethodOption & option, Sizes & key_sizes, AggregatedDataVariants::Type & method_chosen)
{
...
AggregatedDataVariants::Type AggregatorExt::chooseAggregationMethod()
{
    ...
    AggregatorExt::chooseAggregationMethodByOption(option, key_sizes, method);
    return method;
}

template <bool skip_compiled_aggregate_functions>
void AggregatorExt::createAggregateStates(AggregateDataPtr & aggregate_data) const
{
...
void NO_INLINE AggregatorExt::executeImpl(
    Method & method,
...
void NO_INLINE AggregatorExt::executeImplBatch(
    Method & method,
    ...
    for (size_t i = 0; i < aggregate_functions.size(); ++i)
    {
        ...
        if (inst->offsets)
            inst->batch_that->addBatchArray(
                0, rows, places.get(), inst->state_offset, inst->batch_arguments, inst->offsets, aggregates_pool);
        else
            inst->batch_that->addBatch(0, rows, places.get(), inst->state_offset, inst->batch_arguments, aggregates_pool);
    }
}


void NO_INLINE AggregatorExt::executeWithoutKeyImpl(
    ...
    for (AggregateFunctionInstruction * inst = aggregate_instructions; inst->that; ++inst)
        if (inst->offsets)
            inst->batch_that->addBatchSinglePlace(
                0, inst->offsets[static_cast<ssize_t>(rows - 1)], res + inst->state_offset, inst->batch_arguments, arena);
        else
            inst->batch_that->addBatchSinglePlace(0, rows, res + inst->state_offset, inst->batch_arguments, arena);
}


void NO_INLINE AggregatorExt::executeOnIntervalWithoutKeyImpl(
    AggregatedDataWithoutKey & res, size_t row_begin, size_t row_end, AggregateFunctionInstruction * aggregate_instructions, Arena * arena)
{
    /// Adding values
    for (AggregateFunctionInstruction * inst = aggregate_instructions; inst->that; ++inst)
        if (inst->offsets)
            inst->batch_that->addBatchSinglePlace(
                inst->offsets[row_begin], inst->offsets[row_end - 1], res + inst->state_offset, inst->batch_arguments, arena);
        else
            inst->batch_that->addBatchSinglePlace(row_begin, row_end, res + inst->state_offset, inst->batch_arguments, arena);
}


void AggregatorExt::prepareAggregateInstructions(
    Columns columns,
    ...
    NestedColumnsHolder & nested_columns_holder) const
{
    ...
    for (size_t i = 0; i < params.aggregates_size; ++i)
    {
        for (size_t j = 0; j < aggregate_columns[i].size(); ++j)
        {
            ...
            auto full_column = recursiveRemoveSparse(aggregate_columns[i][j]->getPtr());
...
bool AggregatorExt::executeOnBlock(
    const Block & block,
...
void AggregatorExt::writeToTemporaryFile(AggregatedDataVariants & data_variants, size_t max_temp_file_size) const
{
    if (!tmp_data)
...
template <typename Method>
Block AggregatorExt::convertOneBucketToBlock(
    AggregatedDataVariants & data_variants, Method & method, Arena * arena, bool final, size_t bucket) const
{
    Block block = prepareBlockAndFill(
        ...
        [bucket, &method, arena, this](
            MutableColumns & key_columns, AggregateColumnsData & aggregate_columns, MutableColumns & final_aggregate_columns, bool final_)
...
Block AggregatorExt::mergeAndConvertOneBucketToBlock(
    ManyAggregatedDataVariants & variants, Arena * arena, bool final, size_t bucket, std::atomic<bool> * is_cancelled) const
{
...
template <typename Method>
void AggregatorExt::writeToTemporaryFileImpl(AggregatedDataVariants & data_variants, Method & method, TemporaryFileStream & out) const
{
    ...
    data_variants.aggregator_ext = nullptr;
...
bool AggregatorExt::checkLimits(size_t result_size, bool & no_more_keys) const
{
        ...
        switch (params.group_by_overflow_mode)
        {
            case OverflowMode::THROW:
                throw Exception(
                    ...
                    toString(result_size),
                    toString(params.max_rows_to_group_by));
...
void AggregatorExt::convertToBlockImpl(
    Method & method,
    Table & data,
    ...
    bool final) const
{
    ...
    if (final)
    {
        ...
        if (compiled_aggregate_functions_holder)
        {
            ...
            convertToBlockImplFinal<Method, use_compiled_functions>(
                method, data, std::move(raw_key_columns), final_aggregate_columns, arena);
        }
        else
#endif
        {
...
inline void AggregatorExt::insertAggregatesIntoColumns(Mapped & mapped, MutableColumns & final_aggregate_columns, Arena * arena) const
{
...
Block AggregatorExt::prepareBlockAndFill(AggregatedDataVariants & data_variants, bool final, size_t rows, Filler && filler) const
{
...
void AggregatorExt::addSingleKeyToAggregateColumns(const AggregatedDataVariants & data_variants, MutableColumns & aggregate_columns) const
{
...
void AggregatorExt::addArenasToAggregateColumns(const AggregatedDataVariants & data_variants, MutableColumns & aggregate_columns) const
{
    for (size_t i = 0; i < params.aggregates_size; ++i)
    {
        auto & column_aggregate_func = assert_cast<ColumnAggregateFunction &>(*aggregate_columns[i]);
...
void AggregatorExt::createStatesAndFillKeyColumnsWithSingleKey(
...
Block AggregatorExt::prepareBlockAndFillWithoutKey(AggregatedDataVariants & data_variants, bool final, bool is_overflows) const
{
    size_t rows = 1;
...
Block AggregatorExt::prepareBlockAndFillSingleLevel(AggregatedDataVariants & data_variants, bool final) const
{
    ...
    {
#define M(NAME) \
    else if (data_variants.type == AggregatedDataVariants::Type::NAME) convertToBlockImpl( \
        *data_variants.NAME, \
...
BlocksList AggregatorExt::prepareBlocksAndFillTwoLevel(AggregatedDataVariants & data_variants, bool final, ThreadPool * thread_pool) const
{
#define M(NAME) \
    else if (data_variants.type == AggregatedDataVariants::Type::NAME) return prepareBlocksAndFillTwoLevelImpl( \
        data_variants, *data_variants.NAME, final, thread_pool);
...
template <typename Method>
BlocksList AggregatorExt::prepareBlocksAndFillTwoLevelImpl(
    AggregatedDataVariants & data_variants, Method & method, bool final, ThreadPool * thread_pool) const
{
    ...
    try
    {
        ...
        {
            tasks[thread_id] = std::packaged_task<BlocksList()>([group = CurrentThread::getGroup(), thread_id, &converter]
                                                                { return converter(thread_id, group); });
...
BlocksList AggregatorExt::convertToBlocks(AggregatedDataVariants & data_variants, bool final, size_t max_threads) const
{
...
void NO_INLINE AggregatorExt::mergeWithoutKeyDataImpl(ManyAggregatedDataVariants & non_empty_data) const
{
...
template <typename Method>
void NO_INLINE AggregatorExt::mergeSingleLevelDataImpl(ManyAggregatedDataVariants & non_empty_data) const
{
    AggregatedDataVariantsPtr & res = non_empty_data[0];
    ...
    for (size_t result_num = 1, size = non_empty_data.size(); result_num < size; ++result_num)
    {
        ...
        if (!no_more_keys)
        {
            ...
            if (compiled_aggregate_functions_holder)
                mergeDataImpl<Method>(
                    getDataVariant<Method>(*res).data, getDataVariant<Method>(current).data, res->aggregates_pool, true, false);
            else
#endif
                mergeDataImpl<Method>(
                    getDataVariant<Method>(*res).data, getDataVariant<Method>(current).data, res->aggregates_pool, false, false);
        }
        else if (res->without_key)
        {
            mergeDataNoMoreKeysImpl<Method>(
                getDataVariant<Method>(*res).data, res->without_key, getDataVariant<Method>(current).data, res->aggregates_pool);
        }
        else
        {
        ...
        current.aggregator_ext = nullptr;
    }
}

#define M(NAME) \
    template void NO_INLINE AggregatorExt::mergeSingleLevelDataImpl<decltype(AggregatedDataVariants::NAME)::element_type>( \
...
template <typename Method>
void NO_INLINE
AggregatorExt::mergeBucketImpl(ManyAggregatedDataVariants & data, Int32 bucket, Arena * arena, std::atomic<bool> * is_cancelled) const
{
    /// We merge all aggregation results to the first.
    ...
    for (size_t result_num = 1, size = data.size(); result_num < size; ++result_num)
    {
        ...
        {
            mergeDataImpl<Method>(
                getDataVariant<Method>(*res).data.impls[bucket], getDataVariant<Method>(current).data.impls[bucket], arena, true, false);
        }
        else
#endif
        {
            mergeDataImpl<Method>(
                getDataVariant<Method>(*res).data.impls[bucket], getDataVariant<Method>(current).data.impls[bucket], arena, false, false);
        }
    }
}

ManyAggregatedDataVariants AggregatorExt::prepareVariantsToMerge(ManyAggregatedDataVariants & data_variants) const
{
    if (data_variants.empty())
        throw Exception(ErrorCodes::EMPTY_DATA_PASSED, "Empty data passed to AggregatorExt::mergeAndConvertToBlocks.");
...
void NO_INLINE AggregatorExt::mergeStreamsImplCase(
    Block & block, Arena * aggregates_pool, Method & method [[maybe_unused]], Table & data, AggregateDataPtr overflow_row) const
{
    ...
    for (size_t i = 0; i < params.aggregates_size; ++i)
    {
        ...
        // {
        //     delta_bytes_of_large_midstate_agg_inputs
        //         += typeid_cast<const ColumnAggregateFunction &>(*block.getByName(aggregate_column_name).column).byteSize();
        // }
    }
    ...
    for (size_t j = 0; j < params.aggregates_size; ++j)
    {
        ...
        aggregate_functions[j]->mergeBatch(
            0, rows, places.get(), offsets_of_aggregate_states[j], aggregate_columns[j]->data(), aggregates_pool);
    }
...
template <typename Method, typename Table>
void NO_INLINE AggregatorExt::mergeStreamsImpl(
...
void NO_INLINE AggregatorExt::mergeWithoutKeyStreamsImpl(
    Block & block, AggregatedDataVariants & result, std::atomic<bool> & is_cancelled [[maybe_unused]]) const
{
    AggregateColumnsConstData aggregate_columns(params.aggregates_size);
...
bool AggregatorExt::mergeOnBlock(Block block, AggregatedDataVariants & result, bool & no_more_keys, std::atomic<bool> & is_cancelled) const
{
    /// `result` will destroy the states of aggregate functions in the destructor
    result.aggregator_ext = this;
...
#define M(NAME, IS_TWO_LEVEL) \
    ...
    APPLY_FOR_AGGREGATED_VARIANTS(M)
#undef M
    else if (result.type != AggregatedDataVariants::Type::without_key) throw Exception(
        ErrorCodes::UNKNOWN_AGGREGATED_DATA_VARIANT, "Unknown aggregated data variant.");
...
void AggregatorExt::mergeBlocks(
    BucketToBlocks bucket_to_blocks, AggregatedDataVariants & result, size_t max_threads, std::atomic<bool> & is_cancelled)
{
...
#undef M
    ...
    if (has_two_level)
    {
        ...
        auto merge_bucket
            = [&bucket_to_blocks, &result, this, &is_cancelled](Int32 bucket, Arena * aggregates_pool, ThreadGroupPtr thread_group)
        {
            if (thread_group)
                CurrentThread::attachToGroupIfDetached(thread_group);
...
#undef M
        ...
        for (const auto & bucket_blocks : bucket_to_blocks)
        {
            ...
            auto task = [group = CurrentThread::getGroup(), bucket, &merge_bucket, aggregates_pool]
            { return merge_bucket(bucket, aggregates_pool, group); };
...
Block AggregatorExt::mergeBlocks(BlocksList & blocks, bool final, std::atomic<bool> & is_cancelled)
{
    if (blocks.empty())
        return {};
...
#undef M
    ...
    LOG_DEBUG(
        log,
        "Merged partially aggregated blocks. {} rows, {}. in {} sec. ({:.3f} rows/sec., {}/sec.)",
        rows,
        ReadableSize(bytes),
        elapsed_seconds,
        rows / elapsed_seconds,
...
template <typename Method>
void NO_INLINE AggregatorExt::convertBlockToTwoLevelImpl(
...
std::vector<Block> AggregatorExt::convertBlockToTwoLevel(const Block & block) const
{
    if (!block)
        return {};
...
template <typename Method, typename Table>
void NO_INLINE AggregatorExt::destroyImpl(Table & table) const
{
    table.forEachMapped(
...
void AggregatorExt::destroyWithoutKey(AggregatedDataVariants & result) const
{
...
void AggregatorExt::destroyAllAggregateStates(AggregatedDataVariants & result) const
{
    if (result.empty())
        return;
