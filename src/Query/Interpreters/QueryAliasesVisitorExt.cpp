...
bool QueryAliasesWithSubqueriesExt<allow_ambiguous_>::needChildVisit(const ASTPtr & node, const ASTPtr &)
{
...
template <bool allow_ambiguous_>
bool QueryAliasesNoSubqueriesExt<allow_ambiguous_>::needChildVisit(const ASTPtr & node, const ASTPtr & child)
{
    if (node->as<ASTSubquery>())
        return false;
    return QueryAliasesWithSubqueriesExt<allow_ambiguous_>::needChildVisit(node, child);
}

template <typename T>
void QueryAliasesMatcherExt<T>::visit(const ASTPtr & ast, Data & data)
{
...
template <typename T>
void QueryAliasesMatcherExt<T>::visit(const ASTSelectQuery & select, const ASTPtr &, Data &)
{
...
template <typename T>
void QueryAliasesMatcherExt<T>::visit(const ASTArrayJoin &, const ASTPtr & ast, Data & data)
{
    visitOther(ast, data);
...
template <typename T>
void QueryAliasesMatcherExt<T>::visit(const ASTSubquery & const_subquery, const ASTPtr & ast, Data & data)
{
    auto & aliases = data;
...
template <typename T>
void QueryAliasesMatcherExt<T>::visitOther(const ASTPtr & ast, Data & data)
{
    auto & aliases = data;
    String alias = ast->tryGetAlias();
    if (!alias.empty())
    {
        if (aliases.count(alias) && ast->getTreeHash(false) != aliases[alias]->getTreeHash(false))
        {
            if constexpr (T::allow_ambiguous)
                aliases.emplace(alias, ast);
            else
                throw Exception(ErrorCodes::MULTIPLE_EXPRESSIONS_FOR_ALIAS, "{}", wrongAliasMessage(ast, aliases[alias], alias));
        }
        else
            aliases[alias] = ast;
    }
...
template struct QueryAliasesWithSubqueriesExt<false>;
template struct QueryAliasesWithSubqueriesExt<true>;
template struct QueryAliasesNoSubqueriesExt<false>;
template struct QueryAliasesNoSubqueriesExt<true>;

template class QueryAliasesMatcherExt<QueryAliasesWithSubqueriesExt<false>>;
template class QueryAliasesMatcherExt<QueryAliasesWithSubqueriesExt<true>>;
template class QueryAliasesMatcherExt<QueryAliasesNoSubqueriesExt<false>>;
template class QueryAliasesMatcherExt<QueryAliasesNoSubqueriesExt<true>>;
}
