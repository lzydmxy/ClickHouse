        ...
        for (const auto & child : node.children)
        {
            ...
            else if (child->as<ASTQualifiedAsterisk>())
            {
                ...
                if (child->children.size() != 1)
                    throw Exception(ErrorCodes::LOGICAL_ERROR, "Logical error: qualified asterisk must have exactly one child");
                auto & identifier = child->children[0]->as<ASTTableIdentifier &>();

                data.addTableColumns(identifier.name());
            }
            else if (auto * columns_matcher = child->as<ASTColumnsRegexpMatcher>())
            {
                has_asterisks = true;
...
bool JoinToSubqueryTransformMatcherExt::needChildVisit(ASTPtr & node, const ASTPtr &)
{
    return !node->as<ASTSubquery>();
}

void JoinToSubqueryTransformMatcherExt::visit(ASTPtr & ast, Data & data)
{
    if (auto * t = ast->as<ASTSelectQuery>())
        visit(*t, ast, data);
}
...
void JoinToSubqueryTransformMatcherExt::visit(ASTSelectQuery & select, ASTPtr & ast, Data & data)
{
    ...
    for (auto & expr : on_aliases)
    {
        ...
        if (check.dependency)
            throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Cannot rewrite JOINs. Alias '" + expr.first +
    ...
    for (auto * ident : using_identifiers)
        if (on_aliases.count(ident->name()))
            throw Exception(ErrorCodes::NOT_IMPLEMENTED, "Cannot rewrite JOINs. Alias '" + ident->name() + "' appears both in ON and USING");
...
ASTPtr JoinToSubqueryTransformMatcherExt::replaceJoin(ASTPtr ast_left, ASTPtr ast_right, ASTPtr subquery_template)
{
