...
{
    if (settings.empty())
        settings.set_cache_policy(context->getOptimizerContext()->getSettingsRef().statistics_cache_policy);
...
void StatisticsCollector::writeToCatalog()
{
    if (!catalog)
    {
        throw Exception(ErrorCodes::LOGICAL_ERROR, "catalog is NULL");
    }
    StatsData data;
...
PlanNodeStatisticsPtr expandToCurrent(ContextPtr context, PlanNodeStatisticsPtr plan_node_stats, time_t stats_time)
{
    auto [input_now, input_skip] = [&] {
        time_t now = context->getOptimizerContext()->getSettingsRef().statistics_current_timestamp.value;
        time_t skip_seconds = context->getOptimizerContext()->getSettingsRef().statistics_expand_to_current_threshold_days * 3600 * 24;
        time_t skip_time;
    ...
    for (auto & [col_name, symbol] : plan_node_stats->getSymbolStatistics())
    {
        ...
        if (is_date || is_datetime)
        {
            ...
            if (histogram.empty())
            {
                ...
                symbol_stats->setNdv(static_cast<UInt64>(symbol_stats->getNdv() * (1 + expand_factor)));
                symbol_stats->setMax(helper.now);
                delta_count = static_cast<UInt64>(expand_factor * full_count);
            }
            else
            {
                double hist_ratio = context->getOptimizerContext()->getSettingsRef().statistics_expand_to_current_histogram_ratio;
                ...
                for(auto i = bucket_id; i < histogram.getBucketSize(); ++i)
                {
                    ...
                    tail_hist_count += static_cast<UInt64>(bucket.getCount());
                    tail_hist_ndv += static_cast<UInt64>(bucket.getNumDistinct());
                }
                ...
                delta_count = static_cast<UInt64>(local_count);
                symbol_stats->setMax(helper.now);
                symbol_stats->setNdv(static_cast<UInt64>(symbol_stats->getNdv() + local_ndv));
            }
...
std::optional<PlanNodeStatisticsPtr> StatisticsCollector::toPlanNodeStatistics() const
{
    ...
    for (const auto & [col, stats] : columns_stats)
    {
        ...
        if (stats.basic)
        {
            ...
            if (stats.ndv_buckets_result)
            {
                ...
                if (context->getOptimizerContext()->getSettingsRef().statistics_simplify_histogram)
                {
                    auto is_integer = isInteger(stats.ndv_buckets_result->getSerdeDataType());

                    auto ndv_thres = context->getOptimizerContext()->getSettingsRef().statistics_simplify_histogram_ndv_density_threshold;
                    auto range_thres = context->getOptimizerContext()->getSettingsRef().statistics_simplify_histogram_range_density_threshold;
            ...
            symbol->avg_len = static_cast<UInt64>(getAverageLength(decay_type, total_length, nonnull_count));
    ...
    if (context->getOptimizerContext()->getSettingsRef().statistics_expand_to_current)
    {
