...
TableOptions CatalogAdaptor::getTableOptions(const StatsTableIdentifier & table)
{
    ...
    const auto & pattern = context->getOptimizerContext()->getSettingsRef().statistics_exclude_tables_regex.value;
    if (!pattern.empty())
    {
        try
        {
        ...
        {
            throw Exception(ErrorCodes::BAD_ARGUMENTS, "statistics_exclude_tables_regex match error: {}", e.what());
        }
    }
    return options;
}
    ...
    const StatsTableIdentifier & table, const std::vector<String> & target_columns, bool exception_on_unsupported)
{
    ...
    for (const auto & col_name : target_columns)
    {
        ...
        if (QueryStatistics::isCollectableType(col.type))
        {
    ...
    {
        throw Exception(ErrorCodes::BAD_ARGUMENTS, "columns ({}) is not collectable", fmt::join(unsupported_columns, ", "));
    }
...
ColumnDescVector CatalogAdaptor::getAllCollectableColumns(const StatsTableIdentifier & identifier)
{
    ...
    auto snapshot = storage->getInMemoryMetadataPtr();
    auto col_names = getNamesOfOrdinary(snapshot->getColumns());
