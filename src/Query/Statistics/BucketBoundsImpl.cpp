...
String BucketBoundsImpl<T>::serializeToJson() const
{
    ...
    {
        throw Exception(ErrorCodes::LOGICAL_ERROR, "serializeToJson in BucketBoundsImpl.h is not successful, because the Type define is not in type_string set");
    }
...
void BucketBoundsImpl<T>::deserializeFromJson(std::string_view blob)
{
    ...
    {
        throw Exception(ErrorCodes::LOGICAL_ERROR, "the blob is not a array, deserialize failure");
    }
}
...
void BucketBoundsImpl<T>::checkValid() const
{
    ...
    {
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Buckets is not initialized, maybe corrupted blob");
    }

    if (bounds_.size() < 2)
    {
        throw Exception(ErrorCodes::LOGICAL_ERROR, "Buckets too few");
    }
}
...
std::pair<bool, bool> BucketBoundsImpl<T>::getBoundInclusive(size_t bucket_id) const
{
    ...
    {
        throw Exception(ErrorCodes::LOGICAL_ERROR, "out of bucket id");
    }
    auto i = bucket_id;
