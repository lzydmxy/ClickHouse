...
#include <Query/Statistics/TableHandler.h>
...
public:
    ...
    void generateSqls()
    {
        ...
        // to estimate ndv
        LOG_INFO(getLogger("FirstFullColumnHandler"),
            "col info: col={} && sqls={}",
            col_name,
            fmt::join(sqls, ", "));
    }
    ...
    void parse(const Block & block, size_t index_offset_begin) override
    {
        ...
        if (nonnull_count != 0)
        {
            ...
            if (config.need_histogram)
            {
                ...
                LOG_INFO(getLogger("FirstFullColumnHandler"),
                    "col info: col={} && context raw data: full_count={} && column raw data: nonnull_count={}, ndv= {} && cast data: result.nonnull_count={}, result.is_ndv_reliable={}, result.ndv_value={}",
                    col_name,
                    full_count,
                    nonnull_count,
                    ndv,
