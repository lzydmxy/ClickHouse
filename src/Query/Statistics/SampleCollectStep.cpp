...
#include <Query/Statistics/ScaleAlgorithm.h>
#include <Query/Statistics/StatsHllSketch.h>
#include <Query/Statistics/StatsNdvBucketsExtend.h>
...
public:
    ...
    void generateSqls()
    {
        ...
        // to estimate ndv
        LOG_INFO(getLogger("FirstSampleColumnHandler"),
            "col info: col={} && sqls={}",
            col_name,
            fmt::join(sqls, ", "));
    }
    ...
    void parse(const Block & block, size_t index_offset_begin) override
    {
        ...
        /// todo: bc, remove this hack when snapshot is ready
        ...
        if (sample_nonnull_count != 0)
        {
            ...
            LOG_INFO(getLogger("ThirdSampleColumnHandler"),
                "estimated_ndv={}, estimated_ndv_low_bound={}, estimated_ndv_upper_bound={}",
                estimated_ndv,
...
public:
    ...
    void parse(const Block & block, size_t index_offset) override
    {
        ...
        {
            throw Exception(ErrorCodes::LOGICAL_ERROR, "previous result not found");
        }
        ...
        {
            throw Exception(ErrorCodes::LOGICAL_ERROR, "mismatched bucket size");
        }
...
public:
    ...
    String getSampleTail(bool for_uniq_exact)
    {
        ...
        {
            sample_row_count = std::max<UInt64>(settings.sample_row_count(), static_cast<UInt64>(row_count * settings.sample_ratio()));
        }
        ...
        for (auto & col_desc : cols_desc)
        {
            ...
            auto proc_block = [num_buckets, &sample_ndvs, &sample_block_ndvs, &sample_counts, &sample_null_count](Block & block) {
                ...
                for (size_t index = 0; index < block.rows(); ++index)
                {
                    ...
                    if (tag >= num_buckets)
                    {
                        throw Exception(ErrorCodes::LOGICAL_ERROR, "unexpected tag");
                    }
    ...
    {
        // todo: bc, split them into several columns step
        collectTable();

        auto cols_desc_groups = split(all_cols_desc, context->getOptimizerContext()->getSettingsRef().statistics_batch_max_columns);
        ...
        auto unhandled_cols_groups = split(all_unhandled_cols, context->getOptimizerContext()->getSettingsRef().statistics_batch_max_columns);
