...
void AutoStatsManagerSettings::applySettingsChanges(const SettingsChanges & changes, bool throw_if_override)
{
    ...
    for (auto change : changes)
    {
        ...
        {
            throw Exception(ErrorCodes::SYNTAX_ERROR, "duplicate key {}", change.name);
        }
    ...
    auto insert_key = [&]<typename SettingFieldType>(const String & key) {
        if (changes_map.count(key))
        {
            ...
            {
                throw Exception(ErrorCodes::INVALID_SETTING_VALUE, "`{}` overrides previous settings, not allowed", key);
            }
...
#undef INSERT_KEY
    ...
    {
        throw Exception(ErrorCodes::INVALID_SETTING_VALUE, "unknown setting key {}", changes_map.begin()->first);
    }
}
...
void CreateStatsSettings::fromContextSettings(const OptimizerSettings & settings)
{
...
// only for statistics
void applyStatisticsSettingsChanges(OptimizerSettings & settings, SettingsChanges settings_changes)
{
