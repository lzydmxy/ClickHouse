    ...
    PlanSegment * plan_segment_ptr, NodeSelectorResult * result, std::map<PlanSegmentInstanceID, std::vector<UInt32>> & read_partitions)
{
    ...
    for (auto & plan_segment_input : plan_segment_ptr->getPlanSegmentInputs())
    {
        ...
        if (plan_segment_input->getPlanSegmentType() != RIPlanSegment::EXCHANGE)
        {
            continue;
        }
        ...
        if (auto it = dag_graph_ptr->id_to_segment.find(input_plan_segment_id); it != dag_graph_ptr->id_to_segment.end())
        {
            ...
            if (isLocalExchange(mode))
            {
                if (enable_local_input)
                {
                    ...
                    {
                        PlanSegmentPartitionSource source = {.exchange_id = exchange_id, .address = local_addr, .partition_ids = {0}};
                ...
                else
                {
                    ...
                    std::vector<PlanSegmentPartitionSource> sources;
            ...
            else if (mode == RExchangeMode::BROADCAST)
            {
                ...
                for (UInt32 parallel_id = 0; parallel_id < current_parallel_size; parallel_id += 1)
                {
                    ...
                    {
                        auto source = PlanSegmentPartitionSource{.exchange_id = exchange_id, .address = addr, .partition_ids = {}};
...
std::map<PlanSegmentInstanceID, std::vector<UInt32>> NodeSelector::splitReadPartitions(PlanSegment * plan_segment_ptr)
{
    ...
    /// 2. gather segment
    size_t disk_shuffle_advisory_partition_size = optimizer_context->getSettingsRef().disk_shuffle_advisory_partition_size;
        ...
        && !plan_segment.hasLocalOutput()) // TODO wujianchao remove it
    {
        ...
        /// TODO: Coalesced small partitions will be computed in every worker node
    ...
    else
    {
        ...
        for (auto & plan_segment_input : plan_segment_ptr->getPlanSegmentInputs())
        {
            ...
            {
                auto mode = plan_segment_input->getExchangeMode();
                size_t exchange_parallel_size = optimizer_context->getSettingsRef().exchange_parallel_size;
                ...
                if (!isLocalExchange(mode) && mode != RExchangeMode::BROADCAST)
                {
    ...
    NodeSelectorResult & result)
{
    if (payloads.empty() || parallel_size == 0)
        throw Exception(ErrorCodes::LOGICAL_ERROR,
            "Invalid argument for divideSourceTaskByBucket payloads.size:{} parallel_size:{}", payloads.size(), parallel_size);
    ...
    NodeSelectorResult & result)
{
    if (payloads.empty() || parallel_size == 0)
        throw Exception(ErrorCodes::LOGICAL_ERROR,
            "Invalid argument for divideSourceTaskByPart payloads.size:{} parallel_size:{}", payloads.size(), parallel_size);
...
NodeSelectorResult LocalityNodeSelector::select(PlanSegment * plan_segment_ptr, ContextPtr query_context, DAGGraph * dag_graph_ptr)
{
    ...
    if (local_input)
    {
        ...
        if (address_it == dag_graph_ptr->finished_address.end())
            throw Exception(ErrorCodes::LOGICAL_ERROR,
                "Logical error: address of segment {} not found", local_input->getPlanSegmentId());
...
PlanSegmentInputPtr NodeSelector::tryGetLocalInput(PlanSegment * plan_segment_ptr)
{
    ...
    auto it = std::find_if(inputs.begin(), inputs.end(), [](PlanSegmentInputPtr input) {
        return input->getExchangeMode() == RExchangeMode::LOCAL_NO_NEED_REPARTITION
            || input->getExchangeMode() == RExchangeMode::LOCAL_MAY_NEED_REPARTITION;
    });
