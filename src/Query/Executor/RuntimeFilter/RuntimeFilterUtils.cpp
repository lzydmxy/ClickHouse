...
std::pair<ASTs, ASTs> RuntimeFilterUtils::extractExecutableRuntimeFilters(const ASTPtr & conjuncts)
{
    ASTs runtime_filters;
    ASTs static_filters;
    if (!conjuncts)
...
ASTs RuntimeFilterUtils::createRuntimeFilterForFilter(const RuntimeFilterDescription & description, const String & query_id, bool only_bf)
{
    if (only_bf)
    {
...
ASTs RuntimeFilterUtils::generateFunctionArgs(const RuntimeFilterDescription & description, const String & query_id)
{
    ASTs args;
...
ASTs RuntimeFilterUtils::createRuntimeFilterForTableScan(
