...
void MPPScheduler::genBatchTasks()
{
    ...
    for (auto leaf_id : dag_graph_ptr->leaf_segments)
    {
        ...
        if (it == dag_graph_ptr->id_to_segment.end())
            throw Exception(ErrorCodes::LOGICAL_ERROR, "Logical error: source segment can not be found");
...
PlanSegmentExecutionInfo MPPScheduler::schedule()
{
    ...
    {
        auto curr = timeInMilliseconds(std::chrono::system_clock::now());
        if (curr > query_expiration_ms && !stopped.load(std::memory_order_relaxed))
        {
            throw Exception(ErrorCodes::TIMEOUT_EXCEEDED, "schedule timeout, current ts {} expire ts {}",
                curr, query_expiration_ms);
        }
