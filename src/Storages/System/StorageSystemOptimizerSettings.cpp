...
namespace DB
{
ColumnsDescription StorageSystemOptimizerSettings::getColumnsDescription()
{
...
void StorageSystemOptimizerSettings::fillData(MutableColumns & res_columns, ContextPtr context, const ActionsDAG::Node *, std::vector<UInt8>) const
{
    ...
    const auto fill_data_for_setting = [&](std::string_view setting_name, const auto & setting)
    {
        ...
        if (!min.isNull())
            min = OptimizerSettings::valueToStringUtil(setting_name, min);
        if (!max.isNull())
            max = OptimizerSettings::valueToStringUtil(setting_name, max);
    ...
    const auto & settings_to_aliases = OptimizerSettings::Traits::settingsToAliases();
