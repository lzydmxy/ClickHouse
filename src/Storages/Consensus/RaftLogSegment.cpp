...
#include <Storages/Consensus/RaftTools.h>
...
//8+6
//20230614113301
static constexpr char FILE_TIME_FMT[] = "%Y%m%d%H%M%S";
...
bool compareSegment(LogSegmentPtr & seg1, LogSegmentPtr & seg2)
{
...
std::string LogSegment::getFinishFileName()
{
    char buf[1024];
...
std::string LogSegment::getOpenPath()
{
...
std::string LogSegment::getFinishPath()
{
    std::string path(log_dir);
...
std::string LogSegment::getFileName()
{
    if (!file_name.empty())
        return file_name;
...
std::string LogSegment::getPath()
{
    return log_dir + "/" + getFileName();
}

//Open file for read/write
void LogSegment::openFile()
{
    if (seg_fd > 0) return;
    std::string full_path = getPath();
    if (!Poco::File(full_path).exists())
        throw Exception(DB::ErrorCodes::FILE_DOESNT_EXIST, "File path {} is not exists.", full_path);

    errno = 0;
    seg_fd = ::open(full_path.c_str(), O_RDWR);
    if (seg_fd < 0)
        throw Exception(DB::ErrorCodes::CANNOT_OPEN_FILE, "Fail to open {}, fd {}, error:{}",
            full_path, seg_fd, strerror(errno));

    //Set create time
    auto time_str = full_path.substr(full_path.rfind("_")+1);
    int tzd;
    create_dt = Poco::DateTimeParser::parse(FILE_TIME_FMT, time_str, tzd);

    LOG_INFO(log, "Open segment for read, path {}, time {}", full_path, time_str);
}

//create new open segment for read/write
void LogSegment::create()
{
    if (!is_open)
    {
    ...
    create_dt = Poco::DateTime();
    create_time = Poco::DateTimeFormatter::format(create_dt, std::string(FILE_TIME_FMT));

    std::string full_path = getOpenPath();

    if (Poco::File(full_path).exists())
        throw Exception(DB::ErrorCodes::FILE_DOESNT_EXIST, "File path {} is exists.", full_path);

    errno = 0;
    seg_fd = ::open(full_path.c_str(), O_RDWR | O_CREAT | O_TRUNC, 0644);
    if (seg_fd < 0)
        throw Exception(DB::ErrorCodes::CANNOT_OPEN_FILE, "Created new segment {} failed, fd {}, error:{}",
            full_path, seg_fd, strerror(errno));
...
void LogSegment::writeFileHeader()
{
    union
    {
        uint64_t magic_num;
    ...
    if (::write(seg_fd, &magic_num, 8) != 8)
        throw DB::Exception(DB::ErrorCodes::CANNOT_WRITE_TO_FILE_DESCRIPTOR, "Cannot write magic to file descriptor");

    if (::write(seg_fd, &version_uint8, 1) != 1)
        throw DB::Exception(DB::ErrorCodes::CANNOT_WRITE_TO_FILE_DESCRIPTOR, "Cannot write version to file descriptor");

    file_size.fetch_add(sizeof(uint64_t) + sizeof(uint8_t), std::memory_order_release);
}

//load open/close segment
void LogSegment::load()
{
    openFile();
    ...
    {
        closeFile();
        throw Exception(DB::ErrorCodes::CANNOT_STAT, "Fail to get the stat, error:{}", strerror(errno));
    }
    // load entry index
    ...
    for (; entry_off < file_size;)
    {
        ...
        // rc == 0
        const UInt64 skip_len = LogEntryHeader::HEADER_SIZE + header.data_length;
        ...
        {
            LOG_DEBUG(log, "Load log segment, entry_off {}, skip_len {}, file_size {}, actual_last_index {}",
                entry_off, skip_len, file_size, actual_last_index);
        }
    }
    ...
    if (ret == 0 && !is_open)
    {
        ...
        {
            LOG_ERROR(log, "Data lost in a full segment, directory {}, first index {}, expect last index {}, actual last index {}",
                log_dir, first_index, curr_last_index, actual_last_index);
            ret = -1;
        }
        else if (actual_last_index > curr_last_index)
        {
            LOG_ERROR(log, "Found garbage in a full segment, directory {}, first index {}, expect last index {}, actual last index {} ",
                log_dir, first_index, last_index, actual_last_index);
            ret = -1;
        }
    }
    ...
    {
        LOG_INFO(log, "Truncate last uncompleted write entry, directory {}, first_index {}, old size {}, new size {} ",
            log_dir, first_index, file_size, entry_off);
...
off_t LogSegment::loadVersion()
{
    if (seg_fd < 0)
        throw DB::Exception(ErrorCodes::LOGICAL_ERROR, "File not open yet");

    /// "RaftSvs" + version. 9 bytes
    NuBufferPtr buf = buffer::alloc(9);
    buf->pos(0);
    errno = 0;
    ssize_t ret = pread(seg_fd, buf->data(), 9, 0);
    if (ret < 0 || ret != 9)
        throw DB::Exception(ErrorCodes::CANNOT_READ_FROM_FILE_DESCRIPTOR, "Cannot read from file descriptor");
...
void LogSegment::close(bool is_full)
{
    std::lock_guard write_lock(log_mutex);
    if (!is_open) return;
    ...
    if (is_full)
    {
        ...
        LOG_INFO(log, "Close a full segment. Current first index {}, last index {}, renamed {} to {}.",
            first_index, last_index, old_path, new_path);
...
UInt64 LogSegment::appendEntry(NuLogEntryPtr entry, std::atomic<UInt64> & last_log_index)
{
    LogEntryHeader header;
    NuBufferPtr entry_buf;
    char * entry_str;
    size_t buf_size = 0;
    struct iovec vec[2];
    {
        ...
        entry_str = NuLogBuilder::serializeEntry(entry, entry_buf, buf_size);
        ...
        header.data_length = static_cast<UInt32>(buf_size);
        // adler32 is faster 5x than crc32 with big size value
        header.data_crc = getChecksum(entry_str, header.data_length);
    ...
    LOG_TRACE(log, "Append term {}, index {}, length {}, crc {}, file {}, entry type {}.",
        header.term, header.index, header.data_length, header.data_crc, file_size, entry->get_val_type());
    return header.index;
}

int LogSegment::getMeta(UInt64 log_index, LogMeta * meta) const
{
    if (last_index == first_index - 1 || log_index > last_index.load(std::memory_order_relaxed) || log_index < first_index)
    {
        LOG_WARNING(log, "current_index={}, last_index={}, first_index={}", log_index,
            last_index.load(std::memory_order_relaxed), first_index);
        return -1;
    }

    UInt64 meta_index = log_index - first_index;
    UInt64 entry_offset = offset_term[meta_index].first;
    UInt64 next_offset;
    if (log_index < last_index.load(std::memory_order_relaxed))
...
int LogSegment::loadHeader(int fd, off_t offset, LogEntryHeader * header) const
{
    if (header == nullptr)
        return -1;
    auto buf = buffer::alloc(LogEntryHeader::HEADER_SIZE);
    buf->pos(0);
    errno = 0;
    ssize_t ret = ::pread(fd, buf->data(), LogEntryHeader::HEADER_SIZE, offset);
    if (ret < 0 || ret != LogEntryHeader::HEADER_SIZE)
    {
        LOG_ERROR(log, "Read log entry header failed, offset {}, header size {}, ret:{}, error:{}.",
            offset, LogEntryHeader::HEADER_SIZE, ret, strerror(errno));
        return -1;
    }
    buffer_serializer bs(buf);
    bs.pos(0);
...
//Header
//Entry
int LogSegment::loadEntry(int fd, off_t offset, LogEntryHeader * header, NuLogEntryPtr & entry) const
{
    ...
    if (ret < 0 || ret != static_cast<ssize_t>(header->data_length))
    {
    ...
    if (!verifyChecksum(entry_str, header->data_length, header->data_crc))
    {
        LOG_ERROR(log, "Found corrupted data at offset {}, term {}, index {}, length {}, crc {}, file {}",
            offset, header->term, header->index, header->data_length, header->data_crc, file_name);
        delete[] entry_str;
        return -1;
    }
    //LOG_INFO(log, "Verify crc32, length {}, crc {}.", header->data_length, header->data_crc);
    entry = NuLogBuilder::parseEntry(entry_str, header->term, header->data_length);
...
//--Entry
//---Protobuf Message
NuLogEntryPtr LogSegment::getEntry(UInt64 log_index)
{
    {
        std::lock_guard write_lock(log_mutex);
        openFile();
    ...
    do
    {
        ...
        {
            LOG_WARNING(log, "Get entry failed, path {}, index {}, offset {}.", getPath(), log_index, offset);
            ok = false;
            break;
        }
    } while (0);
...
UInt64 LogSegment::getTerm(UInt64 index) const
{
    LogMeta meta;
...
void LogSegment::truncate(const UInt64 last_index_kept)
{
    ...
    UInt64 first_truncate_in_offset = 0;
    {
        ...
        LOG_INFO(log, "Truncating {}, offset {}, first_index {}, last_index from {} to {}, truncate_size to {} ",
            getFileName(), first_truncate_in_offset, first_index, last_index, last_index_kept, truncate_size);
    }
    ...
    if (!is_open)
    {
        ...
        LOG_INFO(log, "Truncate segment closed and reopen. Current first index {}, last index {}, renamed {} to {}.",
            first_index, last_index, old_path, new_path);
    ...
    if (ret != 0)
        throw Exception(DB::ErrorCodes::CANNOT_TRUNCATE_FILE, "Truncate failed errno {}, msg {}", errno, strerror(errno));
    ...
    if (ret_off < 0)
        throw Exception(DB::ErrorCodes::CANNOT_SEEK_THROUGH_FILE, "Fail to lseek fd {} to size {}, path {}.",
            seg_fd, truncate_size, getOpenPath());
...
LogSegmentStorePtr LogSegmentStore::segment_store = nullptr;

LogSegmentStorePtr LogSegmentStore::getInstance(const std::string & log_dir_, const SettingsPtr & settings_, bool force_new)
{
    if (segment_store == nullptr || force_new)
        segment_store = RaftNew<LogSegmentStore>(log_dir_, settings_);
    return segment_store;
}

LogSegmentStore::LogSegmentStore(const std::string & log_dir_, const SettingsPtr & settings_)
    : log_dir(log_dir_), settings(settings_), first_log_index(1), last_log_index(0), log(&(Poco::Logger::get("LogSegmentStore")))
{
...
void LogSegmentStore::init()
{
    ...
    LOG_INFO(log, "Init log segment store, first log index {}, last log index {},"
        " logfile segment size {} MB, logfile keep count {}, logfile keep minutes {}",
...
void LogSegmentStore::close()
{
    if (open_segment)
    {
        ...
        LOG_DEBUG(log, "Close all log segments");
...
void LogSegmentStore::openSegment()
{
    {
        std::shared_lock read_lock(seg_mutex);
        if (open_segment && static_cast<int>(open_segment->getFileSize()) <= settings->logfile_segment_mb * 1000 * 1000
            && open_segment->getVersion() >= CURRENT_LOG_VERSION)
            return;
    }
    std::lock_guard write_lock(seg_mutex);
    if (open_segment)
    ...
    UInt64 next_idx = last_log_index.load() + 1;
    LOG_INFO(log, "Open segment, last log index {}, next index {}", last_log_index.load(), next_idx);
    open_segment = RaftNew<LogSegment>(log_dir, next_idx);
    open_segment->create();
}

void LogSegmentStore::getSegment(UInt64 index, LogSegmentPtr & seg)
{
    seg = nullptr;
...
UInt64 LogSegmentStore::appendEntry(NuLogEntryPtr entry)
{
    try
    {
        openSegment();
    ...
    {
        LOG_WARNING(log, "Append entry exception : {}", ex.displayText());
        return -1;
    }
    //std::lock_guard write_lock(seg_mutex);
    std::shared_lock read_lock(seg_mutex);
...
UInt64 LogSegmentStore::writeAt(UInt64 index, const NuLogEntryPtr entry)
{
    truncateLog(index - 1);
...
NuLogEntryPtr LogSegmentStore::getEntry(UInt64 index)
{
    LogSegmentPtr seg;
    std::shared_lock read_lock(seg_mutex);
    getSegment(index, seg);
...
void LogSegmentStore::getEntries(UInt64 start_index, UInt64 end_index, ptr<std::vector<NuLogEntryPtr>> & entries)
{
    if (entries == nullptr)
    {
...
void LogSegmentStore::getEntriesExt(
    UInt64 start_index, UInt64 end_index, int64 batch_size_hint_in_bytes, ptr<std::vector<NuLogEntryPtr>> & entries)
{
    if (entries == nullptr)
    {
        LOG_ERROR(log, "Entry vector is nullptr.");
...
void LogSegmentStore::removeSegment(UInt64 first_index_kept)
{
    LOG_INFO(log, "Remove log segment, first_log_index {}, first_index_kept {}",
        first_log_index.load(std::memory_order_relaxed), first_index_kept);

    if (first_log_index.load(std::memory_order_acquire) >= first_index_kept)
    {
        LOG_INFO(log, "Nothing is going to happen since first_log_index {} >= first_index_kept {}",
            first_log_index.load(std::memory_order_relaxed), first_index_kept);
        return;
    }

    {
        ...
        LOG_INFO(log, "Remove segment, directory {}, file count {}", log_dir, remove_vec.size());
        ...
        if (last_log_index == 0 || (last_log_index - 1) < first_log_index)
        {
            ...
            LOG_INFO(log, "Reset last log index {}", first_log_index);
        }
    }
}

void LogSegmentStore::removeSegmentThread()
{
    while (!shutdown_called)
    {
        LOG_INFO(log, "Remove old log segments {}", log_dir);
        if (segments.size() == 0) return;
        std::lock_guard write_lock(seg_mutex);
        LogSegments remove_vec;

        //check log file count limit
        auto remove_count = static_cast<int>(segments.size()) + 1 - settings->logfile_keep_count;
        //check creat time limit
        auto curr_datetime = Poco::DateTime();
        ...
        for (auto i = 0; i < static_cast<int>(segments.size()); i++)
        {
            ...
            LOG_DEBUG(log, "Remove index {}, count {}, minutes {}",
                i, remove_count, (curr_datetime - segment->getCreateTime()).totalMinutes());

            if (i < remove_count ||
                (curr_datetime - segment->getCreateTime()).totalMinutes() > settings->logfile_keep_minutes)
            {
                remove_vec.push_back(segment);
        ...
        remove_vec.clear();
        LOG_INFO(log, "Sleep {} seconds", 10);
        sleepForSeconds(10);
    }
}

void LogSegmentStore::truncateLog(UInt64 last_index_kept)
{
    if (last_log_index.load(std::memory_order_acquire) <= last_index_kept)
    {
        LOG_INFO(log, "Nothing is going to happen since last_log_index {} <= last_index_kept {}",
            last_log_index.load(std::memory_order_relaxed), last_index_kept);
        return;
    }

    LogSegments remove_vec;
    LogSegmentPtr last_segment = nullptr;
    {
        std::lock_guard write_lock(seg_mutex);
...
void LogSegmentStore::reset(UInt64 next_log_index)
{
    if (next_log_index <= 0)
        return;
...
void LogSegmentStore::listSegments()
{
    Poco::File file_dir(log_dir);
    if (!file_dir.exists())
        throw Exception(DB::ErrorCodes::DIRECTORY_DOESNT_EXIST, "Log directory {} is not exists.", log_dir);
    ...
    for (auto file_name : files)
    {
        ...
        match = ::sscanf(file_name.c_str(), LOG_FINISH_FILE_NAME, &first_index, &last_index, create_time);
        if (match == 3)
        {
            LOG_INFO(log, "Restore closed segment, directory {}, first index {}, last index {}", log_dir, first_index, last_index);
            auto segment = RaftNew<LogSegment>(log_dir, first_index, last_index, file_name);
            segments.push_back(segment);
            continue;
        }
        match = ::sscanf(file_name.c_str(), LOG_OPEN_FILE_NAME, &first_index, create_time);
        if (match == 2)
        {
            LOG_INFO(log, "Restore open segment, directory {}, first index {}, file name {}", log_dir, first_index, file_name);
            if (!open_segment)
            {
                open_segment = RaftNew<LogSegment>(log_dir, first_index, file_name, std::string(create_time));
                LOG_INFO(log, "Create open segment, directory {}, first index {}, file name {}", log_dir, first_index, file_name);
                continue;
            }
            else
                throw Exception(DB::ErrorCodes::KEEPER_EXCEPTION,
                    "Open segment conflict, directory {}, first index {}, file name {}", log_dir, first_index, file_name);
        }
    }
    ...
    // check segment
    // last_log_index = 0;
    LogSegmentPtr prev_seg = nullptr;
    LogSegmentPtr segment;
    for (auto it = segments.begin(); it != segments.end();)
    {
        segment = *it;
        LOG_INFO(log, "first log index {}, last log index {}, current segment first index {}, last index {}",
            first_log_index.load(std::memory_order_relaxed), last_log_index.load(std::memory_order_relaxed),
            segment->firstIndex(), segment->lastIndex());

        if (segment->firstIndex() > segment->lastIndex())
            throw Exception(DB::ErrorCodes::KEEPER_EXCEPTION,
                "Closed segment is bad, directory {}, current segment first index {}, last index {}",
                log_dir, segment->firstIndex(), segment->lastIndex());
        ...
        if (prev_seg && segment->firstIndex() != prev_seg->lastIndex() + 1)
            throw Exception(DB::ErrorCodes::KEEPER_EXCEPTION,
                ...
                log_dir, prev_seg->lastIndex(), segment->firstIndex());
    ...
    if (open_segment)
        if (prev_seg && open_segment->firstIndex() != prev_seg->lastIndex() + 1)
            throw Exception(DB::ErrorCodes::KEEPER_EXCEPTION,
                ...
                log_dir, prev_seg->lastIndex(), open_segment->firstIndex());
}

void LogSegmentStore::loadSegments()
{
    // closed segments
    ThreadPool load_thread_pool(CurrentMetrics::RaftThreads, CurrentMetrics::RaftThreadsActive, CurrentMetrics::RaftThreadsScheduled,
        LOAD_THREAD_NUM);

    for (UInt32 thread_idx = 0; thread_idx < LOAD_THREAD_NUM; thread_idx++)
    {
        load_thread_pool.trySchedule([this, thread_idx] {
            auto thread_log = &(Poco::Logger::get("LoadLogThread"));
            for (size_t seg_idx = 0; seg_idx < this->getSegments().size(); seg_idx++)
            {
                if (seg_idx % LOAD_THREAD_NUM == thread_idx)
                {
                    auto segment = this->getSegments()[seg_idx];
                    segment->load();
    ...
    if (open_segment)
    {
        ...
        {
            LOG_WARNING(log, "open segment need discard, file {}, first_log_index {}, first_index {}, last_index {} ",
                open_segment->getFileName(), first_log_index.load(), open_segment->firstIndex(), open_segment->lastIndex());
            open_segment = nullptr;
        }
        else
        {
